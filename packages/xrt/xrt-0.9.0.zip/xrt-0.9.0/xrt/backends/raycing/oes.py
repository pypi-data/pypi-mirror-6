...
import __init__ as raycing
import sources as rs
import materials as rm
import stages as rst
...
#direction:
            ...
            if toWhere in [0, 2]:  # reflect, straight
                ...
                if useAsymmetricNormal:
                    normalDotSurfaceNormal = oeNormal[0]*oeNormal[-3] +\
                        oeNormal[1]*oeNormal[-2] + oeNormal[2]*oeNormal[-1]
                    gNormalCryst = np.asarray((
                        oeNormal[0] - normalDotSurfaceNormal*oeNormal[-3],
                        oeNormal[1] - normalDotSurfaceNormal*oeNormal[-2],
                        oeNormal[2] - normalDotSurfaceNormal*oeNormal[-1]),
                        order='F') / (matSur.d * 1e-7) *\
                        np.sqrt(abs(1 - normalDotSurfaceNormal**2)) *\
                        matSur.get_dtheta_symmetric_Bragg(lb.E[goodN])
