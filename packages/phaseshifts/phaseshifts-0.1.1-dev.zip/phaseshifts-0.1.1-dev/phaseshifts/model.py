...
class Model(object):
    ...
    def check_coordinates(self):
        ...
        Raises
        ------
        CoordinateError if duplicate positions found.
        """
...
class MTZ_model(Model):
    ...
    def load_from_file(self, filename):
        ...
        TypeError if a input line cannot be parsed correctly.
        """
        try:
    ...
    def create_atorbs(self, **kwargs):
        ...
        if 'output_dir' in kwargs:
            ...
            if 'library_dir' in kwargs:
                output_dir = kwargs.get('library_dir')
            else:
        ...
        for element in set([str(atom.element.symbol) for atom in self.atoms]):
            ...
            atorb = os.path.join(output_dir, 'Atorb', 'atorb_%s.i' % element)
            if not os.path.isfile(atorb):  # create new atorb input file
                atorb = atorb.Atorb.gen_input(element, filename=atorb)
            
            at = os.path.join(output_dir, 'Atorb', 'at_' + element + '.i')
            if not os.path.isfile(at):
                at = atorb.Atorb.calculate_Q_density(input=atorb, 
                        output_dir=os.path.join(output_dir, 'Atorb'))
            
            # update lists
            atorb_files.append(atorb)
            at_files.append(at)
    ...
    def gen_atomic(self, **kwargs):
        ...
        Raises
        ------
        IOError if either input directory or files do not exist. 
        ...
        if 'output_file' in kwargs:
            output_file = os.path.abspath(kwargs.get('output_file'))
        else:
            output_file = os.path.abspath('atomic.i')
        
        # get list of input
        if 'input_files' in kwargs:
            files = kwargs.get('input_files')
    ...
    def calculate_MTZ(self, **kwargs):
        ...
        if 'cluster_file' in kwargs:
            cluster_file = os.path.abspath(kwargs.get('cluster_file'))
        else:
        ...
        if 'atomic_file' in kwargs:
            atomic_file = os.path.abspath(kwargs.get('atomic_file'))
            if not os.path.isfile(atomic_file):
                raise IOError("Appeneded radial charge densities file " 
                              "'%s' does not exist!" % atomic_file)
        else:  # generate on the fly
            input_dir = os.path.abspath(os.path.basename(cluster_file))
            self.create_atorbs(output_dir=input_dir)
            self.gen_atomic(input_dir=input_dir)
            
        if 'output_file' in kwargs:
            output_file = os.path.abspath(kwargs.get('mtz_file'))
        else:
            output_file = '%s.i' % fid
        ...
        libphsh.cavpot(mtz_string, int(slab), atomic_file, cluster_file,
                       output_file)
    ...
    def gen_input(self, **kwargs):
        ...
        Raises
        ------
        CoordinatesError if the model atoms have duplicate coordinates and 
        the 'pos_check' kwarg is set to True.
        """
        ...
        if 'filename' in kwargs:
            ...
            filename = 'cluster{0}.i'.format(fid)

        # determine if coordinate duplicates 
        if 'pos_check' in kwargs:
            if bool(kwargs.get('pos_check')):
        ...
        with open(filename, 'w') as f:
            ...
            if 'nineq_atoms' in kwargs:
                try:
                    nineq_atoms = int(kwargs.get('nineq_atoms'))
                except TypeError:
