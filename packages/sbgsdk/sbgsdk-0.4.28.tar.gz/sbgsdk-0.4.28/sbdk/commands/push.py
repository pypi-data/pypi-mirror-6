...
def push(project, message):
    ...
    config = {'Entrypoint': project.sdk_runner_command}
...
def push_schema(project, container):
    ...
    #, '--output', '/wrapper/.sbdk/schema.json'])
    #docker.start(container=c)

    # if not cont.is_success():
    #     cont.print_log()
    #     raise SbdkError("Error while fetching schema.")

    with open('.sbdk/schema.json') as fp:
