...
class DocspadClient:
    ...
    def delete(self, doc_id, delete_doc=True):
        if delete_doc:
            returned_message = self._post(self._get_url(self._DELETE_PATH), {'docId':doc_id})
        else:
            returned_message = self._post(self._get_url(self._DELETE_PATH), {'sessionId':doc_id})
        if str(returned_message) != '1':
...
class DocspadDocument:
    ...
    def delete(self):
        self.docspadClient.delete(self.doc_id)
