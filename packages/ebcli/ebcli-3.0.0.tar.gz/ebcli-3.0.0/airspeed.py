...
class Template:
    ...
    def ensure_compiled(self):
        if not self.root_box:
            self.root_box = TemplateBody(self.content)

    def merge_to(self, namespace, fileobj, loader=None):
        if loader is None: loader = NullLoader()
        self.ensure_compiled()
        self.root_box.evaluate(fileobj, namespace, loader)
...
class TemplateSyntaxError(TemplateError):
    def __init__(self, box, expected):
        self.box = box
        self.text_understood = box.full_text()[:box.end]
        self.line = 1 + self.text_understood.count('\n')
        self.column = len(self.text_understood) - self.text_understood.rfind('\n')
        got = box.next_text()
        if len(got) > 40:
            got = got[:36] + ' ...'
        Exception.__init__(self, "line %d, column %d: expected %s in %s, got: %s ..." % (self.line, self.column, expected, self.box_name(), got))

    def get_position_strings(self):
        error_line_start = 1 + self.text_understood.rfind('\n')
        if '\n' in self.box.next_text():
            error_line_end = self.box.next_text().find('\n') + self.box.end
        else:
            error_line_end = len(self.box.full_text())
        error_line = self.box.full_text()[error_line_start:error_line_end]
        caret_pos = self.column
        return [error_line, ' ' * (caret_pos - 1) + '^']

    def box_name(self):
        return re.sub('([A-Z])', lambda m: ' ' + m.group(1).lower(), self.box.__class__.__name__).strip()
...
class _Box:
    ...
    def next_box(self, box_spec):
        if callable(box_spec):
            box = box_spec(self._full_text, self.end)
            self.end = box.end
            return box
        else:
            for box_class in box_spec:
                try: box = box_class(self._full_text, self.end)
                except NoMatch: pass
                else:
                    self.end = box.end
                    return box
    ...
    def require_next_box(self, box_spec, expected):
        if callable(box_spec):
            try: box = box_spec(self._full_text, self.end)
            ...
            for box_class in box_spec:
                try: box = box_class(self._full_text, self.end)
                    ...
                    self.end = box.end
                    return box
            expected = ', '.join([cls.__name__ for cls in box_spec])
...
class FallthroughHashText(_Box):
    ...
    # because of earlier boxes, this will always start with a hash
...
class FloatingPointLiteral(_Box):
...
class Range(_Box):
    ...
    def parse(self):
        self.value1 = self.next_box((FormalReference, IntegerLiteral))
        self.identity_match(self.MIDDLE)
        self.value2 = self.next_box((FormalReference, IntegerLiteral))
...
class ValueList(_Box):
    COMMA = re.compile(r'\s*,\s*(.*)$', re.S)

    def parse(self):
        self.values = []
        try: value = self.next_box(Value)
        except NoMatch:
            pass
        else:
            self.values.append(value)
            while self.optional_match(self.COMMA):
                value = self.require_next_box(Value, 'value')
                self.values.append(value)
...
class ArrayLiteral(_Box):
    ...
    def parse(self):
        ...
        try:
            self.values = self.next_box((Range, ValueList))
        except NoMatch:
            pass
        self.require_match(self.END, ']')
        self.calculate = self.values.calculate

class DictionaryLiteral(_Box):
    START = re.compile(r'{[ \t]*(.*)$', re.S)
    END =   re.compile(r'[ \t]*}(.*)$', re.S)
    ...
    def parse(self):
        ...
        if self.optional_match(self.END):
            ...
            return
        while(True):
            key = self.next_box(Value)
            self.require_match(self.KEYVALSEP, ':')
            value = self.next_box(Value)
...
class Value(_Box):
    def parse(self):
        self.expression = self.next_box((FormalReference, FloatingPointLiteral, IntegerLiteral,
...
class SubExpression(_Box):
    DOT = re.compile('\.(.*)', re.S)

    def parse(self):
        self.identity_match(self.DOT)
        self.expression = self.next_box(VariableExpression)
...
class VariableExpression(_Box):
    subexpression = None

    def parse(self):
        self.part = self.next_box(NameOrCall)
        try: self.subexpression = self.next_box(SubExpression)
        except NoMatch: pass
...
class ParameterList(_Box):
    ...
    def parse(self):
        self.identity_match(self.START)
        try: self.values = self.next_box(ValueList)
        except NoMatch: pass
        self.require_match(self.END, ')')
...
class FormalReference(_Box):
    START = re.compile(r'\$(!?)(\{?)(.*)$', re.S)
    CLOSING_BRACE = re.compile(r'\}(.*)$', re.S)

    def parse(self):
        self.silent, braces = self.identity_match(self.START)
        self.expression = self.require_next_box(VariableExpression, 'expression')
...
class Expression(_Box):

    def parse(self):
        self.expression = [self.next_box(Value)]
        while(True):
            try:
                binary_operator = self.next_box(BinaryOperator)
                value = self.require_next_box(Value, 'value')
...
class ParenthesizedExpression(_Box):
    START = re.compile(r'\(\s*(.*)$', re.S)
    END = re.compile(r'\s*\)(.*)$', re.S)

    def parse(self):
        self.identity_match(self.START)
        expression = self.next_box(Expression)
        self.require_match(self.END, ')')
        self.calculate = expression.calculate


class Condition(_Box):
    def parse(self):
        expression = self.next_box(ParenthesizedExpression)
...
class ElseBlock(_Box):
    START = re.compile(r'#(?:else|{else})(.*)$', re.S + re.I)

    def parse(self):
        self.identity_match(self.START)
        self.block = self.require_next_box(Block, 'block')
        self.evaluate = self.block.evaluate


class ElseifBlock(_Box):
    START = re.compile(r'#elseif\b\s*(.*)$', re.S + re.I)

    def parse(self):
        self.identity_match(self.START)
        self.condition = self.require_next_box(Condition, 'condition')
        self.block = self.require_next_box(Block, 'block')
        self.calculate = self.condition.calculate
        self.evaluate = self.block.evaluate


class IfDirective(_Box):
    START = re.compile(r'#if\b\s*(.*)$', re.S + re.I)
    else_block = Null()

    def parse(self):
        self.identity_match(self.START)
        self.condition = self.next_box(Condition)
        self.block = self.require_next_box(Block, "block")
        self.elseifs = []
        while True:
            try: self.elseifs.append(self.next_box(ElseifBlock))
            except NoMatch: break
        try: self.else_block = self.next_box(ElseBlock)
        except NoMatch: pass
        self.require_next_box(End, '#else, #elseif or #end')
...
class Assignment(_Box):
    ...
    def parse(self):
        ...
        self.value = self.require_next_box(Expression, "expression")
        self.require_match(self.END, ')')
...
class MacroDefinition(_Box):
    ...
    def parse(self):
        ...
        self.optional_match(WHITESPACE_TO_END_OF_LINE)
        self.block = self.require_next_box(Block, 'block')
        self.require_next_box(End, 'block')
    ...
    def execute_macro(self, stream, namespace, arg_value_boxes, loader):
        if len(arg_value_boxes) != len(self.arg_names):
            raise Exception("expected %d arguments, got %d" % (len(self.arg_names), len(arg_value_boxes)))
        macro_namespace = LocalNamespace(namespace)
        for arg_name, arg_value in zip(self.arg_names, arg_value_boxes):
            macro_namespace[arg_name] = arg_value.calculate(namespace, loader)
        self.block.evaluate(stream, macro_namespace, loader)


class MacroCall(_Box):
    ...
    def parse(self):
        ...
        if not self.optional_match(self.OPEN_PAREN):
            ...
            raise NoMatch()
        while True:
            try: self.args.append(self.next_box(Value))
            except NoMatch: break
...
class IncludeDirective(_Box):
    ...
    def parse(self):
        self.identity_match(self.START)
        self.require_match(self.OPEN_PAREN, '(')
        self.name = self.require_next_box((StringLiteral, InterpolatedStringLiteral, FormalReference), 'template name')
...
class UserDefinedDirective(_Box):
    DIRECTIVES = []

    def parse(self):
        self.directive = self.next_box(self.DIRECTIVES)

    def evaluate(self, stream, namespace, loader):
        self.directive.evaluate(stream, namespace, loader)


class SetDirective(_Box):
    START = re.compile(r'#set\b(.*)', re.S + re.I)

    def parse(self):
        self.identity_match(self.START)
        self.assignment = self.require_next_box(Assignment, 'assignment')

    def evaluate(self, stream, namespace, loader):
        self.assignment.evaluate(stream, namespace, loader)


class ForeachDirective(_Box):
    ...
    def parse(self):
        ...
        self.value = self.next_box(Value)
        self.require_match(self.CLOSE_PAREN, ')')
        self.block = self.next_box(Block)
        self.require_next_box(End, '#end')
...
class TemplateBody(_Box):
    def parse(self):
        self.block = self.next_box(Block)
        if self.next_text():
            raise self.syntax_error('block box')
...
class Block(_Box):
    def parse(self):
        self.children = []
        while True:
            try: self.children.append(self.next_box((Text, FormalReference, Comment, IfDirective, SetDirective,
