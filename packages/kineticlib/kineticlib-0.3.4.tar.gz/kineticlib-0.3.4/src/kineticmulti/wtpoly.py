...
def Y_poly_norm(T, T1, molecule, F):
    ...
    F - a constant
    """
    return molecule.vibr_avg_energy_sq(T, T1, 0) + molecule.rot_avg_energy_sq(T, 0)\
                                                 + molecule.avg_i_sq(T, T1) * (F ** 2)\
                                                 + 2 * molecule.vibr_avg_energy(T, T1, 0)\
                                                     * molecule.rot_avg_energy(T, 0)\
                                                 - 2 * molecule.vibr_avg_energy_i(T, T1, 0) * F\
                                                 - 2 * molecule.rot_avg_energy(T, 0) * molecule.W(T, T1) * F\
...
def Y_simple_avg(T, T1, molecule, F):
    ...
    F - a constant
    """
    return molecule.full_avg_energy(T, T1, 0) - molecule.W(T, T1) * F