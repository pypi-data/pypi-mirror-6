...
class TimeConverter(object):  # this is a generic time converter skeleton
    ...
    def COMDate(self,obj):
        ...
        if isinstance(obj, datetime.time):
            ...
            obj = datetime.datetime.now().replace(hour=obj.hour, minute=obj.minute,
                second=obj.second, microsecond=obj.microsecond)
        if hasattr(obj, 'timetuple'):
            tt=obj.timetuple()
            try:
                ms=obj.microsecond // 1000  # convert microseconds to milliseconds -- 2.5.0 Bjarke Ebert
            except:
                ms=0
        ...
        elif hasattr(obj, 'COMDate'): # is mxdate like object?
            try:
                return obj.COMDate()
    ...
    def DateObjectToIsoFormatString(self, obj):
        ...
        try:  # most likely, a datetime.datetime
            ...
            if isinstance(obj, datetime.date):
                ...
                if hasattr(obj, 'microsecond'):
                    s += '.%s' % (s.microsecond // 1000) # convert to millisecond
        return s
...
typeMap= { memoryViewType : adc.adVarBinary,
           ...
           basestring : adc.adBSTR, # this line will be altered by 2to3 to 'str:'
...
def pyTypeToADOType(d):
    """
    Return the ADO type for the type of the provided Python value.
    """
    from . import dateconverter
    ado_type = None
    py_type = type(d)
    if py_type in typeMap:
        ado_type = typeMap[py_type]
    elif py_type in dateconverter.types:
        ado_type = adc.adDate
    else:
        # convert subclasses of types that don't exactly fit in the map
        for map_type, map_ado_type in typeMap.iteritems():
            if issubclass(py_type, map_type):
                ado_type = map_ado_type
                break
    if ado_type is None:
        raise DataError('cannot convert "%s" (type=%s) to ADO' % (repr(d), py_type))
    return ado_type
...
def changeNamedToQmark(op, parms):  #convert from 'named' paramstyle to ADO required '?'mark parameters
    if type(parms) != type({}):
