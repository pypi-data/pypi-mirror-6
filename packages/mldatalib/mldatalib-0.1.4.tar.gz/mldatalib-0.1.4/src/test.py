...
def realstuff(attempt, predict=True):
    ...
    # print 'dunloading'
    #
    pkl_pth = res('pca.pkl.csv')
    # pca_c = PCA(n_components=40)
    # pca_c.fit(a)
    # pca_c.transform()
    # whatev = joblib.dump(pca_c, pkl_pth, compress=3)
    mypca = joblib.load(pkl_pth)
    traindata.extract_feature(pca_transform, force_extraction=False, verbose=1000, mypca)
    ...
    testdata.extract_feature(pca_transform, force_extraction=False, verbose=1000, mypca)
    ...
    if predict is True:
        for i in enumerate(class_lengths):
            ...
            clf = GradientBoostingClassifier(n_estimators=130, max_depth=5)
