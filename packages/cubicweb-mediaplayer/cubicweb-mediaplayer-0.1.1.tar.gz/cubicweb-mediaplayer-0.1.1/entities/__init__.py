...
def _get_repo(appobject):
    if media.safe_hasattr(appobject._cw, 'repo'):
        return appobject._cw.repo
    elif media.safe_hasattr(appobject._cw.cnx, '_repo'):
...
class SoundFile(MediaMixIn, AnyEntity):
    ...
    def update_entity(self, outputs):
        for _format, stream in outputs.iteritems():
...
class VideoFile(MediaMixIn, AnyEntity):
    ...
    def update_entity(self, results):
        attrs = {}
        for attr, (fmt, stream) in results.iteritems():
            attr = 'data_%s' % attr
            attrs[attr] = Binary(stream.read())
