...
class AssetBin(gameduino2.base.GD2):
    ...
    def make(self):
        ...
        if self.asset_file is None:
            print >>hh, "static const PROGMEM prog_uchar __assets[%d] = {" % (len(commandblock))
            print >>hh, textwrap.fill(", ".join(["%d" % ord(c) for c in commandblock]))
            print >>hh, "};"
            print >>hh, "#define %sLOAD_ASSETS()  GD.copy(__assets, sizeof(__assets))" % (self.prefix)
        else:
            open(self.asset_file, "wb").write(commandblock)
            print >>hh, '#define %sLOAD_ASSETS()  GD.load("%s");' % (self.prefix, self.asset_file)
