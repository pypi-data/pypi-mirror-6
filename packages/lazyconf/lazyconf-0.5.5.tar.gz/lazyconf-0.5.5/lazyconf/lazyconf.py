...
class Lazyconf():
    ...
    @property
    def data_file(self):
        """ Gets the full path to the file in which to 
            save/load configured data. """
    ...
    def __init__(self):
        """ Creates the Prompt instance, and initialises the object
            to hold configured data. """
    ...
    def choose_schema(self, out_file):
        """ Finds all schema templates and prompts to choose one.
            Copies the file to self.lazy_folder. """

        path = os.path.dirname(sys.modules[__name__].__file__) + '/schema/'
        ...
        while val is 0 or val > i:
            ...
            if val is 0 or val > i:
                lbl = 'Please choose a value between 1 and '
                self.prompt.error(lbl + str(i) + '.')
    ...
    def configure_data(self, data, key_string = ''):
        ...
        # If we are have any key string or label, 
        ...
        # If this section has an '_enabled' key, process it first,
        # as it could enable or disable this whole section.
        if '_enabled' in data.keys():
            s = self.data.get_key_string(key_string, '_enabled')

            #Prompt whether to enable this section. 
            # Use the existing value as the default.
            lbl = prefix + self.data.get_label(s)
            data['_enabled'] = self.prompt.bool(lbl, None, data['_enabled'])

            # Return if this section is now disabled.
            if data['_enabled'] is False:
                return

        # Loop through the rest of the dictionary and prompt for every key.
        # If the value is a dictionary, call this function again
        # for the next level.
        for k, v in data.iteritems():

            # If we hit the '_enabled' key, we've already processed it
            # (but still need it in the dictionary for saving). Ignore it.
            if k == '_enabled':
                continue
            
            # Get the type of the value at this key, 
            # and the dot-noted format of this key.
            t = type(v)
    ...
    def configure(self):
        """ The main configure function. Uses a schema file 
            and an optional data file, and combines them
            with user prompts to write a new data file. """
        ...
        try:
            ...
            lbl_noschema = "Could not find schema in "
            lbl_dflt = " - Choosing from default templates..."
            self.prompt.error(lbl_noschema + schema_file + lbl_dflt)
        ...
        try:
            ...
            lbl = 'Could not find data file. Copying from schema...'
            self.prompt.error(lbl)
        else:
        ...
        if data.data:
            ...
            # Create a new Merge instance using the data
            # from the schema and data files.
            ...
            for k,m in mods['modified']:
                notice = 'Modified ' + k + ': '
                notice += m[0] + ' became ' + m[1] + '.' 
                self.prompt.notice(notice)
    ...
    def parse_value(self, inner_dict, label, key, value, default):
        """ Parses a single value and sets it in an inner dictionary.
            ...
            default    -- The default value in the prompt.
                          This is taken from the schema and defines the type of the value.
        """
        t = type(default)
        ...
        if select:
            inner_dict[k] = self.prompt.select(label, select,
                                               value, default = default)

        # If the value type is a boolean, prompt a boolean.
        elif t is bool:
            inner_dict[k] = self.prompt.bool(label, value,
                                             default = default)

        # If the value is an int, prompt and int.
        elif t is int:
            inner_dict[k] = self.prompt.int(label, value,
                                            default = default)

        # If someone has put a list in data, we default it to an empty string.
        # If it had come from the schema, it would already be a string.
        elif t is list:
            inner_dict[k] = self.prompt.prompt(label + ':', value,
                                               default = '')
