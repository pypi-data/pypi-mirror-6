...
#        random.seed(seed) - has no effect!
        ...
        if self.card.backend.startswith('shadow'):
            self.alarmQueue.put([])
            ret = backends.shadow.run_process(
...
#                    self.card.stop.set()
            ...
            dummy_output = backends.dummy.run_process()
            self.alarmQueue.put([])
        elif self.card.backend.startswith('raycing'):
            raycing_output = backends.raycing.run.run_process(
                self.card.beamLine)
        ...
        for plot, queue in zip(self.plots, self.outPlotQueues):
            ...
            if self.card.backend.startswith('shadow'):
                ...
                x, y, intensity, cData, locNrays, locAlive, locGood, locOut,\
                    ...
                    backends.raycing.get_output(plot, raycing_output)
