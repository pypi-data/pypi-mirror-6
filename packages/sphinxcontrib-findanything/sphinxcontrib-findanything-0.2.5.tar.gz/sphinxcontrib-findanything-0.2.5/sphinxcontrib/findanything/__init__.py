...
if sphinx_version.startswith('1.0'):
    sphinx_1_0 = True
else:
    sphinx_1_2 = True
...
def astext(x):
    if hasattr(x, 'astext'):
        ...
        return ''.join([x for x in x.itertext() ])
...
def env_updated(app, env):
    ...
    def indexentries(entry, links, cap = ''):
        if links:
            # in newer sphinx versions links is a list of tuples (ismain, link)
            href = links[0]
            ...
            for i, thing in enumerate(links[1:], start=1):
                try:
                    ismain, link = thing
                except:
                    link = thing
...
#            x = ref.astext()
#            #if "Changing the " in x:
#            if 1:
#                import rpdb2 ; rpdb2.start_embedded_debugger()

            docs[refuri] = astext(ref)
            path = "/"
            if "/" in refuri:
            ...
            for entry, (links, subitems) in char_list:
                ...
                if subitems:
                    ...
                    for subname, sublinks in subitems:
                        if subname.startswith('(') and subname.endswith(')'):
                            subname = main_name + ' ' + subname
...
def print_default_css():
    '''Convenience function to get default CSS for easier styling.

    Usage::
        python -c "from sphinxcontrib.findanything import print_default_css; print_default_css()"

    '''
    fn = os.path.join(os.path.dirname(__file__), 'css', 'findanything.css')
