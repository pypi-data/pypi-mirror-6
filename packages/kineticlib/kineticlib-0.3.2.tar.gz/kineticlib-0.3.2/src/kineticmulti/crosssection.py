...
def vv_collisions(T, T1_1, T1_2, idata, molecule1, molecule2, nokt=False):
    ...
    result[3] - <dE_c>_{cd}
    result[4] - <di>_{cd} (should be negligibly small, since, from a theoretical POV, it is precisely equal to 0)
    result[5] - <di ^ 2>_{cd}
    result[6] - <di * dk>_{cd}
    result[7] - <di * dE_d>_{cd}
    ...
    for k in xrange(molecule2.num_vibr):
        for i in xrange(molecule1.num_vibr):
            ...
            tmp_arr[3] += (vibr_energy[i + 1] - vibr_energy[i]) * vibr_exp[i] * vibr_exp2[k + 1] * cs
            tmp_arr[4] += vibr_exp[i] * vibr_exp2[k + 1] * cs
            tmp_arr[5] += vibr_exp[i] * vibr_exp2[k + 1] * cs  # i, k -> i + 1, k - 1
            tmp_arr[6] -= vibr_exp[i] * vibr_exp2[k + 1] * cs
            tmp_arr[7] += (vibr_energy2[k] - vibr_energy2[k + 1]) * vibr_exp[i] * vibr_exp2[k + 1] * cs
            ...
            tmp_arr[3] += (vibr_energy[i] - vibr_energy[i + 1]) * vibr_exp[i + 1] * vibr_exp2[k] * cs  # <Ev_i>
            tmp_arr[4] -= vibr_exp[i + 1] * vibr_exp2[k] * cs  # <i>
            tmp_arr[5] += vibr_exp[i + 1] * vibr_exp2[k] * cs  # <i ^ 2>
            tmp_arr[6] -= vibr_exp[i + 1] * vibr_exp2[k] * cs  # <i k>
            tmp_arr[7] -= (vibr_energy2[k + 1] - vibr_energy2[k]) * vibr_exp[i + 1] * vibr_exp2[k] * cs  # <i Ev_k>
...
def dE_rot_sq(T, molecule):
    ...
    tmp *= 1.0 / (molecule.Z_rot(T) * ((constants.k * T) ** 2)) / molecule.num_rot
    return tmp
