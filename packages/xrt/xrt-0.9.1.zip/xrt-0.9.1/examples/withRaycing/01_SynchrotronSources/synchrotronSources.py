...
Synchrotron sources
-------------------
The images below are produced by the test suit of
`\\backends\\raycing\\sources.py` and by
...
Undulator
~~~~~~~~~
The module :mod:`~xrt.backends.raycing.sources` has built in functions for
...
def build_beamline(nrays=100000):
    ...
    beamLine.fsm1 = rsc.Screen(beamLine, 'FSM1', (0, 20000, 0))
    return beamLine
...
#        return
    ...
    xrtr.run_ray_tracing(plots, repeats=360, beamLine=beamLine,
                         afterScript=afterScript, processes='all')
