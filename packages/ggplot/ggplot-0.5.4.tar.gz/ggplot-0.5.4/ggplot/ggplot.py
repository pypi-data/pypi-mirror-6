    ...
    def _make_plot_data(self, data=None, aes=None):
        ...
        for ae, key in aes.items():
            if isinstance(key, list) or hasattr(key, "__array__"):
                ...
                mapping[ae] = data[key]
            elif key in data.index.names:
                pass
            elif key==data.index.name:
                pass
            else:
...
def _apply_transforms(data, aes):
    ...
    for ae, name in aes.items():
        existing_col = (name in data or name in data.index.names or name==data.index.name)
        if (isinstance(name, six.string_types) and (existing_col==False)):
