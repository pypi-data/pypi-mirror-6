...
from cubes.mediaplayer.utils import safe_hasattr, TemporaryDir
...
def reencode_sound(sound, outputs):
    ...
    avconv -y -i toto.aiff -f aac -acodec aac -strict experimental -ab 128k output.mp3
    """
    for attr, value in outputs.iteritems():
        ...
        # output to stdout
        cmd.extend(['-'])
        input_stream = _ensure_file(sound)
        output_stream = TemporaryFile()
        stderr = TemporaryFile()
        return_code = call(cmd, stdin=input_stream,  stdout=output_stream, stderr=stderr)
        if return_code:
            stderr.seek(0)
            raise RuntimeError('Unable to convert  %s.\n%s  returned with status %i and stderr:%s' % (sound.data_name, ' '.join(cmd), return_code, stderr.read()))
        # check the output_stream is a valide File
        output_stream.seek(0)
        output_stream = _ensure_file(output_stream)
        outputs[attr] = output_stream
    return outputs
...
def reencode_video(video, outputs):
    ...
    if not video.data.getvalue():
        output_stream = _ensure_file(video)
        for _, _fmt in outputs.iteritems():
            results[_fmt[1]] = (_fmt[0], output_stream)
        return results
    ...
    with TemporaryDir() as tmp:
        ...
        for input_path, fmt in tmp_outputs.items():
            ...
            output_stream = open(output_path, 'rb')
            results[fmt[1]] = (fmt[0], output_stream)
        return results
