...
from ._session_start_info import SessionStartInfo
...
class Eyes(object):
    ...
    def open(self, driver, app_name, test_name, viewport_size=None,
             match_level_=match_level.EXACT, failure_reports_=failure_reports.ON_CLOSE):
        """
        Starts a test.
        ...
        self._failure_reports = failure_reports_
        self._match_level = match_level_
    ...
    def _start_session(self):
        ...
        app_env = AppEnvironment(self.host_os, self.host_app, self._viewport_size,
                                 self._get_inferred_environment())
        self._start_info = SessionStartInfo(Eyes.AGENT_ID, self._app_name, self._test_name,
                                            self.batch, app_env, self._match_level, None,
                                            self.branch_name, self.parent_branch_name)
    ...
    def check_window(self, tag=None):
        ...
        if not as_expected:
            ...
            #         TODO log("mismatch #{ tag ? '' : "(#{tag})" })
                if self._failure_reports == failure_reports.IMMEDIATE:
    ...
    def close(self):
        ...
        should_save = (is_new_session and self.save_new_tests) \
            or ((not is_new_session) and self.save_failed_tests)
        ...
        if is_new_session:
            ...
            message = "'%s' of '%s'. %s" % (self._start_info.scenario_id_or_name,
                                            self._start_info.app_id_or_name, instructions)
