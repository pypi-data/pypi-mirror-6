...
def dynamically_loaded_scripts(request):
    from simplejson import load, loads, dump
    ...
    scripts = loads(request.params.get('scripts', '[]'))

    css = [s for s in scripts if s.endswith('.css')]

    # ExtJS sources
    extjs = [s for s in scripts if s.startswith('/desktop/extjs/src')]

    # metapensiero.extjs.desktop sources
    med = [s for s in scripts if s.startswith('/desktop/js/')]

    # app specific sources
    app = [s for s in scripts if s.startswith('/static/app')]

    done = set(css + extjs + med + app)

    # other sources
    other = [s for s in scripts if s not in done]

    newlist = css + extjs + med + other + app
...
def catalog_view(request):
    ...
    try:
        ...
        # Usually the "en" language is native, no external catalog
        # exists for it
        if name != 'en':
