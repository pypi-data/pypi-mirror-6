...
def check_authorized_request(event,
    ...
    get_request_logger(request, logger).debug(
        t(_('Unauthorized access to $path', mapping=dict(path=request.path))))
...
def auth_user(request):
    ...
    if username and password:
        ...
        adminuser = settings['sol.admin.user']
        adminpwd = settings['sol.admin.password']
        guestuser = settings['sol.guest.user']
        guestpwd = settings['sol.guest.password']
        ...
        get_request_logger(request, logger).info(
            t(_('New session for $fullname', mapping=dict(fullname=fullname))))
...
def logout(request):
    ...
    get_request_logger(request, logger).info(t(_('Session terminated')))
    ...
    if s['user_id'] or s['is_admin']: # not for guest users
        ...
        if bckdir and isdir(bckdir):
            get_request_logger(request, logger).info(
                t(_('Performing a database backup, just in case...')))
