...
MAXIMUM_SENDING_STREAMS_NUMBER = 2
...
class Notifier():
    ...
    def streams(self):
        ...
        for keyID, streams in _SendingStreams.items():
            for stream in streams:
                sendings.append(keyID+':'+stream.state+':'+str(stream.totalBytesSent))
        for keyID, streams in _ReceivingStreams.items():
            for stream in streams:
                receivings.append(keyID+':'+stream.state+':'+str(stream.totalBytesReceived))
        # logger().info('%s %s' % (str(sendings), str(receivings)))
        controlCommand('streams', (sendings, receivings))
