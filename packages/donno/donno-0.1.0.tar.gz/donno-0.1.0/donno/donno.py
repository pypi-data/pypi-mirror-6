import argparse, sys
import notes, evernote, cloudsync
...
def complex_search(args):
    if args.tag != None:
        print 'search tags:', args.tag
    if args.title != None:
        print 'search titles:', args.title
    if args.content != None:
        print 'search content:', args.content
    if args.notebook != None:
        print 'search in notebook:', args.notebook
...
def deletenote(args):
    print 'Delete the no.', args.listno, 'note'
...
def def_parser():
    parser = argparse.ArgumentParser(prog='an',
            epilog='Use "an <cmd> -h" for the exact help of <cmd>.',
            description='ANote is a personal CLI note-taking app.')
    ...
    parser_listnote.add_argument('listcnt', nargs='?', type=int, default=25,
    ...
    parser_complexsearch.add_argument('-t', '--tag', nargs='*',
            help='search in the note tags')
    parser_complexsearch.add_argument('-l', '--title', nargs='*',
            help='search in the note titles')
    parser_complexsearch.add_argument('-c', '--content', nargs='*',
    ...
    parser_deletenote = subparsers.add_parser('d', help='Delete the note')
    ...
    parser_syncnote = subparsers.add_parser('y',
            help='Synchronize notes with cloud server')
