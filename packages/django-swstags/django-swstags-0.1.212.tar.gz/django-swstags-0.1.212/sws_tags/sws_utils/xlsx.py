...
def ExportFile(param_export_file):
	...
	def writeBillInStream():
		...
		# border = book.add_format({'bold': 1,'font_color': 'red','border_color':'blue'})
		border = book.add_format({'bold': True, 'border': 7,'color':'red'})



		queryset =OrderedDict([('Destination', 'Afghanistan'), ('Total Minutes', 523), ('EUR/min', 0.35), ('Total', 183.05)]), OrderedDict([('Destination', 'Mexico'), ('Total Minutes', 837), ('EUR/min', 0.4), ('Total', 334.8)])

		headers = ['Destination','Total Minutes','EUR/min','Total']
		col_name = ['Destination','Total Minutes','EUR/min','Total']
		...
		try:
			...
			sheet.write(0,5,'FACTURA',border)
			sheet.write(1,5,'Numero: 123123123')
		except:
			print traceback.format_exc()
		...
		for trr in queryset:
			for c in col_name:
				...
				sheet.write(row, col, trr[c])
				if c =='Total':
					total = total + float(trr[c])
					# print 'rrrrrrrrrrrr',float(trr[c])
		...
		sheet.write(row, col_max-2, 'Total',bold)	
		sheet.write(row, col_max-1, total,bold)	

		row = row +1

		sheet.write(row+1, 1, 'Bank Name:',bold)	
		sheet.write(row+2, 1, 'Account:',bold)
		sheet.write(row+3, 1, 'Beneficiary:',bold)
		sheet.write(row+4, 1, 'Bank addres:',bold)

		sheet.write(row+1, 2, 'Santander')	
		sheet.write(row+2, 2, '123123123')
		sheet.write(row+3, 2, 'Sultan')
		sheet.write(row+4, 2, 'Alcala')


		sheet.write(row+1, 5, 'IBAN:',bold)	
		sheet.write(row+2, 5, 'SWIFT:',bold)
		sheet.write(row+3, 5, 'Company:',bold)
		sheet.write(row+4, 5, 'Company Number:',bold)
		sheet.write(row+5, 5, 'Email:',bold)
		
		sheet.write(row+1, 6, 'Santander')	
		sheet.write(row+2, 6, '123123123')
		sheet.write(row+3, 6, 'Sultan')
		sheet.write(row+4, 6, 'Alcala')
		sheet.write(row+5, 6, 'safe@safecalls.net')

		# marge_format = book.add_format({'align':center})

		# sheet.merge_range('B3:D4','Merged Cell',merge_format)
