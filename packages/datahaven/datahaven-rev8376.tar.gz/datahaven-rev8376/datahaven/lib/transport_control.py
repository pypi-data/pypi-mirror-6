...
def register_transfer(proto=None, send_or_receive=None, remote_address=None, callback=None, filename=None, size=None, description=None, transfer_id=None):
    global _LiveTransfers
    ...
    dhnio.Dprint(14, '>>>>>> {%d} %s via %s %s %s at %s' % (
        transfer_id, fti.send_or_receive, fti.proto.upper(), tofrom, nameurl.GetName(fti.remote_idurl), fti.proto_address))
    return transfer_id
...
def unregister_transfer(transfer_id):
    ...
    dhnio.Dprint(14, '<<<<<< {%d} %s via %s %s %s at %s with %d bytes at %s KB/s.' % (
        transfer_id, info.send_or_receive, info.proto.upper(), tofrom, nameurl.GetName(info.remote_idurl), info.proto_address, b, speed))
    del info
    return True
