...
#        xyRGB[1::2,1::2,0]=1
        ...
        if self.contourLevels is not None:
            ...
            if dy == 0:
                dy = 1.
            x = np.linspace(self.xaxis.limits[0], self.xaxis.limits[1],
                            self.xaxis.bins)
            y = np.linspace(self.yaxis.limits[0], self.yaxis.limits[1],
                            self.yaxis.bins)
            X, Y = np.meshgrid(x, y)
    ...
    def _pow10(self, x, digits=1):
        ...
        coeff = round(x / float(10**exponent), digits)
        ...
        if self.textGoodrays:
            if (runner.runCardVals.backend == 'shadow'):
                ...
                self.textGoodrays.set_text(
                    ''.join([r'$N_{\rm ', strDict[self.rayFlag], r'} = $%s'])
                    % self.nRaysNeeded)
                ...
                if (self.fluxUnit is None) or (self.nRaysSeeded == 0):
                    ...
                    if self.fluxKind.startswith('power'):
                        ...
                        if (self.nRaysAll > 0) and (self.nRaysSeeded > 0):
                            flux = self.intensity / self.nRaysAll *\
                                self.nRaysAcceptedTimesI / self.nRaysSeeded
                            if isPowerOfTen:
                                intensityStr = self._pow10(
                                    flux, powerOfTenDecN)
                            else:
                                intensityStr = self.fluxFormatStr % flux
                            intensityStr = \
