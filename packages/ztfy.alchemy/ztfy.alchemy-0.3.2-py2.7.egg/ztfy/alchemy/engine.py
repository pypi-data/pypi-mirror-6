...
def getSession(engine, join=True, status=STATUS_ACTIVE, request=None, alias=None, twophase=True):
...
def getUserSession(engine, join=True, status=STATUS_ACTIVE, request=None, alias=None, twophase=True):
    """Shortcut function to get user session"""
    if isinstance(engine, basestring):
        session = getSession(engine, join=join, status=status, request=request, alias=alias, twophase=twophase)
    else:
        session = engine
    return session
