...
The module also provides visualization routines (see the test suit at the end)
for displaying spatial and energy distributions of synchrotron sources in 2D
and 3D.
...
calculations and based on external codes Urgent [Urgent]_ and WS [WS]_. These
codes have some drawbacks, as demonstrated in the section
...
.. [US] R. J. Dejus, US: Program to calculate undulator spectra. Part of XOP.
...
_DEBUG = 1  # if non-zero, some diagnostics is printed out
...
#discover which side of the footprint corresponds to which side of divergence
        ...
        badNum = beam.state != 1
        if badNum.sum() > 0:
            raise "cannot shrink the source!"
...
#origin coordinates:
            if isinstance(self, BendingMagnetWS):
                ...
                bot.y[:] += np.random.normal(
                    0, ((self.eSigmaX**2 + self.eSigmaZ**2)**0.5)*1e-3,
                    npassed)
            bot.x[:] += np.random.normal(0, self.eSigmaX*1e-3, npassed)
            bot.z[:] += np.random.normal(0, self.eSigmaZ*1e-3, npassed)
...
class BendingMagnet(object):
    def __init__(self, bl, name='BM', center=(0, 0, 0), nrays=raycing.nrays,
        ...
        *distE*: 'eV' or 'BW'
            The resulted flux density is per 1 eV or 0.1% bandwidth.
        *xPrimeMax*, *zPrimeMax*:
        ...
        while length < self.nrays:
            ...
            if self.isMPW:
                ...
                bot.x[:] = self.X0 * np.sin(PI2 * bot.y / self.L0) +\
                    np.random.normal(0., self.dx, npassed)
                bot.y[:] -= 0.25 * self.L0
...
class Undulator(object):
    def __init__(self, bl, name='und', center=(0, 0, 0), nrays=raycing.nrays,
                 eE=6.0, eI=0.1, eEspread=0., eSigmaX=None, eSigmaZ=None,
                 eEpsilonX=1., eEpsilonZ=0.01, betaX=20., betaZ=5.,
                 period=5, n=50, K=10., Kx=0, Ky=0., phaseDeg=0,
                 eMin=5000., eMax=15000., eN=51, distE='eV',
                 xPrimeMax=0.5, zPrimeMax=0.5, nx=25, nz=25,
        ...
        *betaX*, *betaZ*:
            betatron function (m). Alternatively, beam size can be specified.
        *period*, *n*:
            magnetic period length and number of periods.
        *K*, *Kx*, *Ky*:
        ...
        *gp*:
        ...
        while gau_int_error > self.gp:
            ii += 1
            self.gau = int(ii * 2)
            I1 = self.build_I_map(self.E_max, self.Theta_max, self.Psi_max)[1]
            self.gau = int(ii * 2 + 1)
            I2 = self.build_I_map(self.E_max, self.Theta_max, self.Psi_max)[1]
...
#            print "G =",self.gau, gau_int_error, np.abs(I2)
        ...
        if precalc:
            ...
            def int_fun(x):
                return -1. * (self.build_I_map(x[0] * 1e5 + E0,
                                               x[1] * 1e-4,
                                               x[2] * 1e-4)[0]) / f_max
            for nn in range(10):
                a_max = np.argmax(DistI)
                DistI[a_max] = 0
                initial_x = [(rE[a_max]-E0) * 1e-5, (rTheta[a_max]) * 1e4,
                             (rPsi[a_max]) * 1e4]
                bounds_x = [
                    ((self.E_min-E0) * 1.e-5, (self.E_max-E0) * 1.e-5),
                    (self.Theta_min * 1.e4, self.Theta_max * 1.e4),
                    (self.Psi_min * 1.e4, self.Psi_max * 1.e4)]
                ...
                res = optimize.fmin_slsqp(int_fun, initial_x,
                                          ...
                                          full_output=1,
                                          iprint=0)
                tmp_Imax = -1. * int_fun(res[0]) * f_max
                if tmp_Imax > self.Imax:
                    self.Imax = tmp_Imax
        ...
        xE, xTheta, xPsi = np.mgrid[
            ...
            self.Theta_min:self.Theta_max+0.5*self.dTheta:self.dTheta,
            self.Psi_min:self.Psi_max+0.5*self.dPsi:self.dPsi]
        self.Itotal, self.Is, self.Ip =\
            self.build_I_map(xE, xTheta, xPsi)
        self.Imax = max(self.Imax, np.amax(self.Itotal))
    ...
    def build_I_map(self, w, ddtheta, ddpsi):
        ...
        dstep = PI / float(2.**self.intervals)
...
#            print "{0} NaN rays in {1}!".format(nanSum, strName)
    ...
    def shine(self, toGlobal=True):
#mcRays=4e6: repeats=4, processes=1: 2164 s
#mcRays=1e6: repeats=4, processes=1: 1959 s
#mcRays=1e5: repeats=4, processes=1: 1866 s
#mcRays=1e4: repeats=4, processes=1: 1567 s
#mcRays=3e3: repeats=4, processes=1: 1810 s
#mcRays=1e3: repeats=4, processes=1: 2441 s
        """Returns the source beam. If *toGlobal* is True, the output is in
        the global system."""
        if _DEBUG > 10:
        ...
        while length < self.nrays:
            ...
            bot.x[:] = np.random.normal(0, self.dx, npassed)
            bot.z[:] = np.random.normal(0, self.dz, npassed)
            if self.Ky:
                bot.y[:] = np.random.normal(0, self.dx * self.gamma / self.Ky,
                                            npassed)
            elif self.Kx:
                bot.y[:] = np.random.normal(0, self.dz * self.gamma / self.Kx,
                                            npassed)

            mJs = mJs[I_pass]
            mJp = mJp[I_pass]
            sSP = np.abs(mJs * np.conj(mJs) + mJp * np.conj(mJp))
            bot.Jsp[:] = np.where(sSP, mJs * np.conj(mJp) / sSP, sSP)
            bot.Jss[:] = np.where(sSP, np.abs(mJs * np.conj(mJs)) / sSP, sSP)
            bot.Jpp[:] = np.where(sSP, np.abs(mJp * np.conj(mJp)) / sSP, sSP)
...
#            mayavi.add_module(o)
        ...
        ipwY.ipw.slice_index = data.shape[1] - 1
        ...
        if isZplane:
            ...
            ipwZ.ipw.slice_index = data.shape[2] - 1
...
#        eMin=1500, eMax=31500, eN=3000, nx=20, nz=10)
###by Urgent:
##    Source = UndulatorUrgent
##by xrt:
#    kwargs['distE'] = 'BW'
