...
class TestMVPolyRudinShapiro(unittest.TestCase) :
    ...
    def test_rudin_shapiro_maxmod(self) :
        ...
        # they have rather small maximum modulu
        for C in classes :
            eps = 1e-10
            for m in range(2,7) :
    ...
    def test_rudin_shapiro_L4_norm(self) :
        # for norm tests : Littlewood, if n = 2^k - 1, P the Rudin-Shapiro
        # polynomial order n, then 
        #
        #       ||P||_L4^4  =  (1/3)((4n+1)^2 - (-1)^k (n+1))
        # 
        # to be done when norm_L(p) is implemented
        pass
...
class TestMVPolyLehmer(unittest.TestCase) :
    ...
    def test_lehmer_size(self) :
        for C in classes :
            ...
            self.assertTrue(p.degrees == (10,), "rudin-shapiro degrees")

    def test_lehmer_dtype(self) :
        for C in classes :
            for dt in [int, float, np.double] :
                p = C.lehmer(dtype=dt)
                self.assertTrue(p.dtype == dt, "rudin-shapiro dtype")
    ...
    def test_lehmer_explicit(self) :
        for C in classes :
            ...
            self.assertTrue(p == q, "rudin-shapiro explicit")
            

class TestMVPolyBerenstein(unittest.TestCase) :

    def test_berenstein_size(self) :
        for C in classes :
            p = C.berenstein((3, 2), (5, 7), dtype=int)
            self.assertTrue(p.degrees == (5,7), "berenstein degrees")

    def test_berenstein_badargs(self) :
        for C in classes :
            self.assertRaises(ValueError, C.berenstein, (3, 8), (5, 7))
            self.assertRaises(ValueError, C.berenstein, (1, 2, 3), (5, 7))

    def test_berenstein_explicit_1d(self) :
        for C in classes :
            p = C.berenstein(3, 4, dtype=int)
            x = C.monomial(0, 1, dtype=int)
            q = 4 * x**3 * (1 - x)
            self.assertTrue(p == q, "berenstein explicit 1d")

    def test_berenstein_dtype(self) :
        for C in classes :
            for dt in [int, float, np.double] :
                p = C.berenstein((3,2), (5,7), dtype=dt)
                self.assertTrue(p.dtype == dt, "rudin-shapiro dtype")
