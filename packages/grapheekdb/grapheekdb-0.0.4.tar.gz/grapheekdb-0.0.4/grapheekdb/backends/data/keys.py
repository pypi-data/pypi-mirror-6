...
def build_entity_index_values(kind, index_id, *values):
    return '%s/%s/%s/%s' % (KIND_INDEX, kind, index_id, repr(list(values)))


def build_entity_index_prefix(kind, index_id):
    return '%s/%s/%s/' % (KIND_INDEX, kind, index_id)
