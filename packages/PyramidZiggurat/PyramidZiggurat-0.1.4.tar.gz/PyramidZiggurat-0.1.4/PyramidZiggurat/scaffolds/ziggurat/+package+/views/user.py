...
session_add_failed = 'user add failed'
session_edit_failed = 'user edit failed'
...
def form_validator(form, value):
    ...
    if 'id' in form.request.matchdict:
        user = User.get_by_id(form.request.matchdict['id'])
    else:
        user = None
    found = User.get_by_email(value['email'])
    if user:
    ...
    if request.POST:
        if 'save' in request.POST:
            ...
            try:
                ...
                request.session[session_add_failed] = e.render()               
                return HTTPFound(location=request.route_url('user-add'))
            save_request(dict(controls), request)
        return route_list(request)
    elif session_add_failed in request.session:
        return session_failed(request, session_add_failed)
...
def query_id(request):
    return User.query().filter_by(id=request.matchdict['id'])
    ...
    if request.POST:
        if 'save' in request.POST:
            ...
            try:
                ...
                request.session[session_edit_failed] = e.render()               
                return HTTPFound(location=request.route_url('user-edit',
                                  id=row.id))
            save_request(dict(controls), request, row)
        return route_list(request)
    elif session_edit_failed in request.session:
        return session_failed(request, session_edit_failed)
    values = row.to_dict()
