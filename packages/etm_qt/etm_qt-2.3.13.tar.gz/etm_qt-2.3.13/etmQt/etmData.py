...
if has_qt:
    ...
    def d_to_str(qlcl, d, s):
        return(unicode(qlcl.toString(QDate(d), s).toUtf8().data(), encoding))

    def dt_to_str(dt, s):
        return(unicode(QDateTime(dt).toString(s), encoding))
    ...
    def d_to_str(qlcl, d, s):
        for key, val in qt2dt:
            s = s.replace(key, val)
        # print('fmt', s, d, s2or3(d.strftime(s)))
        return(s2or3(d.strftime(s)))

    def dt_to_str(dt, s):
        for key, val in qt2dt:
            s = s.replace(key, val)
        # print('fmt', s, dt, s2or3(dt.strftime(s)))
...
def reportDT(dt, include, options={}):
    ...
    if dt.hour == 0 and dt.minute == 0:
        ...
        if options['ampm']:
            if include:
                return(dt_to_str(dt, "%s h:mma" % include))
            else:
                return(dt_to_str(dt, "h:mma"))
        else:
            if include:
                return(dt_to_str(dt, "%s hh:mm" % include))
            else:
                return(dt_to_str(dt, "hh:mm"))
        ...
        if groupby['report'] == 'c':
            if fmts.count(u"hsh['t']"):
                ...
                for tag in hsh['t']:
                    rowcpy = deepcopy(row)
                    rowcpy[position] = tag
                    rowcpy.append((tup[-1], tup[-4], setSummary(hsh, parse(dt)), dt, etmdt))
                    rows.append(rowcpy)
            else:
                row.append((tup[-1], tup[-4], setSummary(hsh, parse(dt)), dt, etmdt))
                rows.append(row)
        else:
...
import cmd
import readline  # this is used by cmd whatever pep8 thinks
...
""")
        ...
        while msg:
            os.system(cmd)
            # check the item
            fo = codecs.open(self.tmpfile, 'r', encoding)
    ...
    def safe_save(self, file, str):
        ...
        """
        try:
            fo = codecs.open(self.tmpfile, 'w', encoding)
            fo.write(str)
            fo.close()
        except:
            print('error writing to file - aborted')
            return(False)
    ...
    def get_itemhash(self, arg_str):
        try:
            ...
            print(tr('an integer argument is required'))
            return()
...
""")
    ...
    def do_d(self, arg_str):
        ...
        print(hsh['entry'])
        print("-" * self.line_length)
        print(tr('WARNING: THIS ACTION CANNOT BE UNDONE!'))
            ...
            if 'r' in hsh:
                ...
                if choice == 1:
                    ...
                    try:
                        ...
                        print(tr("Could not parse"), "{0}".format(res))
                        print(e)
                        return(False)
                    old_dt = parse(
                        hsh['_dt']).replace(
                    ...
                    print(s2or3("Ready to replace {0} with {1}").format(fmt_datetime(old_dt, self.options), fmt_datetime(new_dt, self.options)))
                    rep = raw_input(tr("Continue? [Yn] "))
                    if rep.lower() == 'n':
                        print(tr('canceled'))
                        return(False)
...
""".format(hsh['_summary']))
        ...
        if not rep:
            ...
            if dtstr:
                ...
                # print("could not parse '{0}'".format(rep))
                return(False)
        done, next, following = getDoneAndTwo(hsh)
        if next:
...
""")
    ...
    def do_i(self, arg_str):
        ...
        print("\n{0:-^{4}}\n{1}\n{2:-^{4}}\n{3}".format(tr(' item '),
              hsh['entry'], tr(" file "), filetext, self.line_length))
        print("-" * self.line_length)
...
the file {0}:""".format(f))
    ...
    def do_n(self, arg_str='', itemstr=""):
        ...
        items = [unicode(u'%s') % x[0].rstrip() for x in old_items if x[0].strip()]
        old_len = len(items)
        ...
        print("{0:-^{4}}\n{1}\n{2:-^{4}}\n{3}".format(tr(' Ready to append '),
              new_item, tostr, self.currfile, self.line_length))
        print("-" * self.line_length)
        rep = raw_input("Continue? [Yn] ")
        if rep.lower() == 'n':
            print(tr('canceled'))
...
Commands:
    ...
    v       display the etm version number.
