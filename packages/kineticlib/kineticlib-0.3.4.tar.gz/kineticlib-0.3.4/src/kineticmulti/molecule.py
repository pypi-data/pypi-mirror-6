...
class Molecule:
    ...
    def __init__(self, name, vmodel='anharmonic'):
        ...
        if vmodel == 'harmonic':
            self.num_vibr = int(raw_d[2])
            self.max_vibr = self.num_vibr
        elif vmodel == 'anharmonic' or vmodel == 'table':
            self.num_vibr = int(raw_d[3])
            self.max_vibr = self.num_vibr
        self.inertia = raw_d[4] * (10 ** (-46))
        self.rot_const = (constants.h ** 2) / (8.0 * (constants.pi ** 2) * self.inertia)
        self.hvc = raw_d[5] * 100 * constants.h * constants.c
        self.avc = raw_d[6]
        self.diss = raw_d[7] * constants.physical_constants['electron volt'][0]
        self.form = raw_d[8] * constants.physical_constants['electron volt'][0]
        self.LJe = raw_d[9]  # stored as eps/kboltz
        self.LJs = raw_d[10] * constants.physical_constants['Angstrom star'][0]
    ...
    def renorm(self, T, T1, n):
        ...
        self.current_vae_dim = self.vibr_avg_energy(T, T1, 1)
        self.current_vae = self.vibr_avg_energy(T, T1, 0)
        self.current_rae_dim = self.rot_avg_energy(T, 1)
        self.current_rae = self.rot_avg_energy(T, 0)
        self.current_vaesq = self.vibr_avg_energy_sq(T, T1, 0)
        self.current_vaesq_dim = self.vibr_avg_energy_sq(T, T1, 1)
        self.current_raesq = self.rot_avg_energy_sq(T, 0)
        self.current_raesq_dim = self.rot_avg_energy_sq(T, 1)
        self.current_w = self.W(T, T1)
        ...
        self.current_vae_i = self.vibr_avg_energy_i(T, T1, 0)
        self.current_vae_i_dim = self.vibr_avg_energy_i(T, T1, 1)

        a = np.arange(0, self.num_vibr + 1, 1)
        self.current_v_exp = self.vibr_exp(a, T, T1)
        self.current_ni = self.ni(a, T, T1, n)
        self.current_n = n
        self.current_T = T
        self.current_T1 = T1
    ...
    def rot_exp(self, j, T):
        ...
        T - the temperature of the mixture
        """
        return (2.0 * j + 1.0) * np.exp(-self.rot[j] / (constants.k * T))
    ...
    def Z_rot(self, T):
        ...
        """
        if self.current_T != T:
            return 4.0 * constants.pi**2 * self.inertia * constants.k * T / (constants.h**2)
        else:
    ...
    def rot_avg_energy(self, T, dim=1):
        ...
        if dim == 1:
            return kT * (1.0 + np.exp(-B * snr) * (-B * snr - 1.0)) / (self.Z_rot(T) * B)
        else:
            return (1.0 + np.exp(-B * snr) * (-B * snr - 1.0)) / (self.Z_rot(T) * B)

    def vibr_avg_energy(self, T, T1, dim=1):
    ...
    def full_avg_energy(self, T, T1, dim=1):
        ...
        dimensionless energy
        """
        return self.vibr_avg_energy(T, T1, dim) + self.rot_avg_energy(T, dim)

    def vibr_avg_energy_sq(self, T, T1, dim=1):
    ...
    def rot_avg_energy_sq(self, T, dim=1):
        ...
        if self.current_T != T:
            ...
            if dim == 1:
                return (2.0 + np.exp(-B * snr) * (-B * snr * (B * snr + 2) - 2)) * (kT ** 2) / (self.Z_rot(T) * B)
            else:
                return (2.0 + np.exp(-B * snr) * (-B * snr * (B * snr + 2) - 2)) / (self.Z_rot(T) * B)
        else:
            if dim == 1:
    ...
    def vibr_avg_energy_i(self, T, T1, dim=1):
    ...
    def ni(self, i, T, T1, n):
        ...
        """
        if (self.current_T != T) or (self.current_T1 != T1) or (self.current_n != n):
            return n * self.vibr_exp(i, T, T1) / self.Z_vibr(T, T1)
        else:
    ...
    def W_dT(self, T, T1):
        ...
        if self.vibr_model == 'harmonic':
            ...
            if (self.current_T != T) or (self.current_T1 != T1):
                return (self.vibr_avg_energy_i(T, T1) - self.vibr[1] * self.avg_i_sq(T, T1)
                        - self.W(T, T1) * self.vibr_avg_energy(T, T1) + self.vibr[1] * (self.W(T, T1) ** 2))\
    ...
    def W_dT1(self, T, T1):
        ...
        """
        if (self.current_T != T) or (self.current_T1 != T1):
            return self.vibr[1] * (self.avg_i_sq(T, T1) - (self.W(T, T1) ** 2))\
    ...
    def E_vibr(self, T, T1):
        ...
        T1 - the vibrational temperature of the molecular species in question
        """
        return self.vibr_avg_energy(T, T1) / self.mass
    ...
    def E_vibr_dT(self, T, T1):
        ...
        if (self.current_T != T) or (self.current_T1 != T1):
            if self.vibr_model == 'harmonic':
                ...
                return ((self.vibr_avg_energy_sq(T, T1) - (self.vibr_avg_energy(T, T1) ** 2))
                        + self.vibr[1] * (self.W(T, T1) * self.vibr_avg_energy(T, T1) - self.vibr_avg_energy_i(T, T1)))\
                        / (constants.k * T * T * self.mass)
        else:
    ...
    def E_vibr_dT1(self, T, T1):
        ...
        """
        if (self.current_T != T) or (self.current_T1 != T1):
            return (self.vibr_avg_energy_i(T, T1) - self.W(T, T1) * self.vibr_avg_energy(T, T1))\
                    * self.vibr[1] / (self.mass * constants.k * T1 * T1)
        else:
            return self.current_edt1

    def vibr_avg_f(self, T, T1, f, *args):
