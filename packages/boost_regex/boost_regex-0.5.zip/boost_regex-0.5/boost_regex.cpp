...
static int BoostRegex_init(BoostRegexObject *self, PyObject *args, PyObject *kwds)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwds, "|u#", kwlist, &pattern, &len)) {
		return -1;
	}

  	self->rx = new boost::wregex(pattern, boost::regex_constants::normal | boost::regex_constants::no_except);
	return 0;
};
...
static PyObject *BoostRegex_match(BoostRegexObject *self, PyObject *args)
{
    ...
    try {
        match = boost::regex_match(text, *self->rx);
        if (match)
        {
            Py_INCREF(Py_True);
            return Py_True;
...
static PyObject *BoostRegex_search(BoostRegexObject *self, PyObject *args)
{
    ...
    try {
        match = boost::regex_search(text, *self->rx);
        if (match)
        {
            Py_INCREF(Py_True);
            return Py_True;
