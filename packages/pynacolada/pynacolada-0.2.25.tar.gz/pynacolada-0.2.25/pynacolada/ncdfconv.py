...
def csv2netcdf(infile,outfile,sep=None,nalist = [''],formatlist=[],refdat = 
        datetime(2000,1,1),tunits = 'hours',readcolnames=True, customnames=[], timestep=None):
    '''
    ...
    outfile.variables['time'][:] = np.arange(lenout)
    ...
    for iline,eline in enumerate(infile):
        ...
        if tunits == 'hours':
            ...
            if timestep == None:
                dt = dt + dt.timedelate(hours=1)
        if tunits == 'days':
            outfile.variables['time'][iline] = \
                    (dt-refdat).total_seconds()/3600./24.
            # automatic timestepping
            if timestep == None:
                dt = dt + dt.timedelate(days=1)
        if tunits == 'seconds':
            outfile.variables['time'][iline] = \
                    (dt-refdat).total_seconds()
            # automatic timestepping
            if timestep == None:
                dt = dt + dt.timedelate(seconds=1)
        if tunits == 'minutes':
            outfile.variables['time'][iline] = \
                    (dt-refdat).total_seconds()/60.
            # automatic timestepping
            if timestep == None:
                dt = dt + dt.timedelate(minutes=1)
        if timestep != None:
            dt = dt + timestep
