...
def meta_merge(meta_df_list):
    """Turn a list of SAS meta DataFrames into an unduplicated set
    """
    merged_meta = pd.concat(meta_df_list).reset_index()
    augmented = meta_augment(merged_meta)
    
    #sort_index using length before de-duplication. This will ensure that the longest value is used.
    aug_sorted = augmented.sort_index(by=['field','length'], ascending=False)
    aug_dedup = aug_sorted.drop_duplicates(cols='field')
    
    #must not end up with a different number of fields vs the count of unique fields in the original merge
    assert len(aug_dedup) == len(merged_meta.field.unique()), "De-duplicated column count (%d) does not match count of unique fields among original meta_df_list (%d)" % (len(aug_dedup), len(merged_meta.field.unique()))
