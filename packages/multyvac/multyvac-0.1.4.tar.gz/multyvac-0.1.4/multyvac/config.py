...
class MultyvacConfigError(MultyvacError):
    pass
...
class ConfigModule(MultyvacModule):
    ...
    def get_auth(self):
        ...
        Otherwise, raises a MultyvacConfigError."""
        if not self.api_key:
            raise MultyvacConfigError('Api Key is not set')
    ...
    def _load_credential_from_disk(self, api_key):
        ...
        if os.path.exists(credential_path + '.json'):
            ...
            raise MultyvacConfigError('Could not find credentials for api_key '
                                      '%s on disk' % api_key)
