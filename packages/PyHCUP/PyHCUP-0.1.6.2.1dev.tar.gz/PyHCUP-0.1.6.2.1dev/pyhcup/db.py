...
def long_table_sql(table_name, category, schema=None, ine=True):
...
def pg_shovel(cnxn, tbl_source, tbl_destination, field_lst, preserve_source=True):
    ...
    fields = ', '.join(field_lst)
    
    stmt = '''
        INSERT INTO %s (%s)
            SELECT %s FROM %s;
        ''' % (str(tbl_destination), fields, fields, str(tbl_source))
