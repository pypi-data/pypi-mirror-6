...
cdef char *_mssql_last_opened_server = <char *>PyMem_Malloc(PYMSSQL_MSGSIZE)
...
cdef int err_handler(DBPROCESS *dbproc, int severity, int dberr, int oserr,
    ...
    if _mssql_last_opened_server and _mssql_last_opened_server[0]:
        sprintf(mssql_lastdberrstr, "%s (%s)", dberrstr, _mssql_last_opened_server)
    else:
        strncpy(mssql_lastdberrstr, dberrstr, PYMSSQL_MSGSIZE)
...
cdef class MSSQLConnection:
    ...
    def __init__(self, server="localhost", user="sa", password="",
        ...
        strcpy(_mssql_last_opened_server, server_cstr)
