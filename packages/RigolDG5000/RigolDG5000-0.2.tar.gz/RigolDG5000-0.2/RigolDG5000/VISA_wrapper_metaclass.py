    ...
    if in_test is None:
        ...
        def set_val(self, args):
            if isinstance(args, tuple):
                ...
                param = in_test(args)
            cmd_nameb = self.get_cmd_name(cmd_name)
            self.write('%s %s'%(cmd_nameb, param))
...
class GenericCommandClass(object):
    ...
    def to_dict(cls,name):
        return {name:_generic_command(cls.cmd, doc=cls.get_the_doc())}
    @classmethod
    def get_the_doc(cls):
...
def _make_doc(txt, title):
    txt = _clean_doc(txt)
    fmt = "* **{title}**\n\n  {txt}\n\n".format(title=title, txt = txt.strip().replace('\n','\n  '))
    fmt = ".. py:attribute:: RigolDG5000.{title}\n\n   {txt}\n\n".format(title=title, txt = txt.strip().replace('\n','\n   '))
    return fmt
...
class GenericGetCommandClass(GenericCommandClass):
    ...
    def to_dict(cls, name):
        ...
        return {name:property(get_cmd,doc=cls.get_the_doc()), 
...
class GenericSetCommandClass(GenericCommandClass):
    ...
    def to_dict(cls, name):
        ...
        return {name:property(lambda self:None, set_cmd,doc=cls.get_the_doc()), 
...
class GenericGetSetCommandClass(GenericCommandClass):
    ...
    def to_dict(cls, name):
        get_cmd = _generic_get_command(cls.cmd, cls.out_conversion, doc=cls.__doc__)
        set_cmd = _generic_set_command(cls.cmd, cls.in_test, doc=cls.__doc__)
        return {name:property(get_cmd,set_cmd, doc=cls.get_the_doc()), 
...
class InstrumentCommand(object):
    ...
    def get_the_doc(cls):
        ...
        for elm in cls._subgroups:
            ...
            out += getattr(cls,elm).get_the_doc()
        return out
...
class Group(object):
    ...
    def __init__(self, parent):
        self._parent = parent
    def write(self, s):
        return self._parent.write(s)
    def ask(self, s):
        return self._parent.ask(s)
    def get_cmd_name(self, cmd_name):
        return self._parent.get_cmd_name(cmd_name)
    @classmethod
    def get_the_doc(cls):
        out = ""
        for elm in cls._subgroups:
...
class IndexedGroup(Group):
    ...
    def __getitem__(self, i):
         ...
         return out        
    def get_cmd_name(self, cmd_name):
        new_cmd = self._parent.get_cmd_name(cmd_name).replace(self.var, str(self._item))
        return new_cmd
