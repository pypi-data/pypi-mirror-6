...
class TestEnsureSSHKeys(
    ...
    def patch_check_ssh_keys(self, return_value=False):
        mock_check_ssh_keys = mock.Mock(return_value=return_value)
        return mock.patch('quickstart.app.check_ssh_keys', mock_check_ssh_keys)
    ...
    def test_error_starting_agent(self, mock_print):
        with self.assert_program_exit('foo'):
            with mock.patch('quickstart.utils.start_ssh_agent',
...
class TestCheckSSHKeys(
    ...
    def test_no_agent(self):
        with self.patch_call(2) as mock_call:
            have_keys = app.check_ssh_keys()
        mock_call.assert_called_once_with('/usr/bin/ssh-add', '-l')
        self.assertFalse(have_keys)
...
class TestBootstrap(
    ...
    def test_success_local_provider(self, mock_print):
        ...
        mock_print.assert_has_calls([
            mock.call('sudo privileges required to bootstrap the environment'),
...
class TestGetApiUrl(
    ...
    def test_success(self):
        ...
        with self.patch_call(0, output=api_addresses) as mock_call:
