...
from sqlserver_ado.adodbapi import process_connect_string
from sqlserver_ado.adodbapi import ado_consts as adc
from sqlserver_ado.adodbapi import apibase as api
...
def _configure_parameter(p, value, adotype, settings_known):
    ...
    elif isinstance(value, api.StringTypes):            #v2.1 Jevon
        L = len(value)
    ...
    elif api.onIronPython and isinstance(value, api.longType): # Iron Python Long
...
class Connection(object):
    ...
    def connect(self, kwargs, connection_maker=make_COM_connecter):
        ...
        try:
            ...
            self._raiseConnectionError(KeyError,'Error in connection string->')
        ...
        if self.supportsTransactions:
            ...
            if 'autocommit' in kwargs:
                self._autocommit = bool(kwargs['autocommit'])
            if not self._autocommit:
                self.connector.BeginTrans() #Disables autocommit
        else:
    ...
    def close(self):
        ...
        try:
            ...
            self._raiseConnectionError(api.InternalError,e)
    ...
    def commit(self):
        ...
        try:
            ...
            self._raiseConnectionError(api.Error, e)
        ...
        if name == 'autocommit':  # extension: allow user to turn autocommit on or off
            if self.supportsTransactions:
                ...
                if value: # turn autocommit on
                    self.commit()  # turning on implies an automatic commit
                    value = True
                else:
                    self._rollback()
                    value = False
            else:
                return # silently ignore autocommit switch
        elif name == 'paramstyle':
            if value not in api.accepted_paramstyles:
                self._raiseConnectionError(api.NotSupportedError,
                        'paramstyle="'+value+'" not in:'+repr(api.accepted_paramstyles))
...
##   allows the programmer to override the connection's default error handler
    ...
    def _new_command(self, command_text, command_type=adc.adCmdText):
        ...
        if self.connection is None:
            self._raiseCursorError(api.Error, None)
            return
        try:
            ...
            self._raiseCursorError(api.DatabaseError, 'Error creating new ADODB.Command object for "%s"' % repr(command_text))
    ...
    def callproc(self, procname, parameters=None):
        ...
        self._buildADOparameterList(procname, parameters, sproc=True)
        if verbose > 2:
    ...
    def _reformat_operation(self,operation,parameters):
        if self.paramstyle == 'format': # convert %s to ?
            operation = api.changeFormatToQmark(operation)
        elif self.paramstyle == 'named' or (self.paramstyle == 'auto' and isinstance(parameters, Mapping)):
            operation, parameters = api.changeNamedToQmark(operation,parameters) # convert :name to ?
        return operation,parameters

    def _buildADOparameterList(self, operation, parameters, sproc=False):
        ...
        if sproc:  # needed only if we are calling a stored procedure
            try: # attempt to use ADO's parameter list
                ...
                parameters_known = len(parameters) == len(self.cmd.Parameters)-1 
            except:
                if verbose:
                    print 'ADO Parameter Refresh failed'
                pass

        if parameters != None or sproc:
            i = 0
