...
def ps(v, rx, ngrid, rad, e, phs, nl):
    '''
    Desciption
    ----------
...
def phsh_wil(mufftin_file='mtz.i', phasout_file='phasout.o', 
    ...
    e1, e2, ne, ix, neuo, nr, nl, z, r, v, y, f, ilst, nl2, ip, nrr  # common
    ...
    for kkk in range(nrr):
        ...
        #remove discontinuities by multiples of pi
                ...
                if abs(deldif) < 0.7:
                    ...
                    pass
                ls += 1
                delta[lp] = delta[lp] - sign(pi, deldif)
                if ls < 5:
                    # go to 111
                    pass
        ...
        #write phase shifts as def of energy for plotting 
            for kk in range(1, nl + 1):  # 145
                dataph.write('"l = %2i\n' % kk - 1)  # format(3h"l = ,i2)   
...
2     continue
       ...
       phasout.write('be careful about the order of the elements\n')
...
def s5(e):
    ...
    for i in range(5, nr + 1):
        ...
        for j in range(1, nj + 1, 2):
            ...
            flp1 = lp1
            f[j][ip1] = (flp1 * y[j][ip1] + r[i] * y[jp1][ip1]) * t1
            fjp1][ip1] = (vme[lp1] * y[j][ip1] - flp1 * y[jp1][ip1]) * t1
    return

      
def s10(e, cmrv, r, v, nr, nl, z, cm5, y, f, ilst):
    '''
 ...
 3    format    ( / / / ,10x,a6, / / /  / ,10x,18hmuffin-tin radius = ,
  ...
  9   format (1x,a1,i3,3d15.7,22x,a3)
   ...
   17 format (1h1, / ,10x,'relativistic phase shifts for ',a30, /  / ,10x,'exca  = ',f10.6,4x,'excb  = ',f10.6,4x,'exco  = ',f10.6 /  / ,10x,'lattice constant  = ',f10.6,' ,',f10.6,' ,',f10.6)
    ...
    inpdat.write('%4i%10.6f%4i  %10.6f' %
