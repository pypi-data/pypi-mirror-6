...
def shutdown():
    ...
    result = Deferred()
#    def _killed(x, reslt):
#        dhnio.Dprint(2, 'webcontrol.shutdown._killed')
#        reslt.callback(x)
#        return x
    def _kill(x, reslt):
        dhnio.Dprint(2, 'webcontrol.shutdown._kill')
        # d = threads.deferToThread(kill)
        # d.addBoth(_killed, reslt)
        # return x
        res = kill()
        result.callback(res)
        return res
    ...
    def getfsitem(self):
        ...
        if iter_path:
            ...
            if self.fsitem.type == backup_fs.PARENT: 
                self.size = dirsize.getInBytes(self.localPath)
            else:
...
class BackupPage(Page, BackupIDSplit):
    ...
    def _renderRunningPage(self, request, backupObj):
        ...
        if dirSizeBytes == 0:
            ...
            src += html_comment('  %s is unknown' % folder_or_file)
