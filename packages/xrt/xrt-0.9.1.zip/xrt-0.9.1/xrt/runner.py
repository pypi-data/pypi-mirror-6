    ...
    if runCardVals.iteration == 0:
        for plot in _plots:
            ...
            cLimitsDefined = (plot.caxis.limits is not None) and\
                (not isinstance(plot.caxis.limits, str))
...
def run_ray_tracing(
    ...
    Parameters are all optional except the 1st one.
        ...
        *generator*: generator object
            A generator for running
            :ref:`complex ray-tracing studies <complex>`. It must modify the
            optics, specify the graph limits, define the output file names etc.
            in a loop and return to xrt by ``yield``.
...
#        if plot.rayFlag == 10:
    ...
    if backend.startswith('shadow'):
        ...
        backends.shadow.check_shadow_dirs(cpus, runCardVals.cwd)
        runCardVals.fWiggler, runCardVals.fPolar, runCardVals.blockNRays = \
            backends.shadow.init_shadow(cpus, runCardVals.cwd, energyRange)
