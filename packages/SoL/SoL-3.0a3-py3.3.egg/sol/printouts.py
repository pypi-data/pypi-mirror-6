...
bfn = rl_settings.canvas_basefontname = 'DejaVuSans'
...
try:
    for variant in ('', '-Bold', '-Oblique', '-BoldOblique'):
        pdfmetrics.registerFont(TTFont(bfn + variant, bfn + "%s.ttf" % variant))
except TTFError:
    from reportlab import rl_config

    logger.error('Could not find the "%s" font, using PDF default fonts', bfn)

    rl_config.canvas_basefontname = rl_settings.canvas_basefontname = 'Helvetica'
    BOLD_ITALIC_FONT_NAME = 'Times-BoldItalic'
    ITALIC_FONT_NAME = 'Times-Italic'
else:
    BOLD_ITALIC_FONT_NAME = bfn + '-BoldOblique'
    ITALIC_FONT_NAME = bfn + '-Oblique'

    from reportlab.lib.fonts import addMapping
    addMapping(bfn, 0, 0, bfn)
    addMapping(bfn, 0, 1, bfn + '-Oblique')
    addMapping(bfn, 1, 0, bfn + '-Bold')
    addMapping(bfn, 1, 1, bfn + '-BoldOblique')
...
class TourneyPrintout(BasicPrintout):
    ...
    def getArgumentsFromRequest(klass, session, request):
        ...
        if 'idtourney' in kw:
            try:
                ...
                raise OperationAborted(
                    t(_(u'Invalid idtourney: $idtourney',
        ...
        raise OperationAborted(
            t(_(u'Missing request argument "idtourney" for class $classname',
    ...
    def getElements(self):
        ...
        if stringWidth(title, fname, fsize) > maxwidth:
            ...
            fsize = tstyle.fontSize
            if stringWidth(title, fname, fsize) > maxwidth:
                maxwidth -= stringWidth(u'…', fname, fsize)
...
class Participants(TourneyPrintout):
    ...
    def getElements(self):
        ...
        for element in super(Participants, self).getElements():
            yield element
...
class Ranking(TourneyPrintout):
    ...
    def getElements(self):
        for element in super(Ranking, self).getElements():
            yield element
        ranking = [(i+1, c.description, c.nationality,
...
class NationalRanking(TourneyPrintout):
    ...
    def getElements(self):
        ...
        for element in super(NationalRanking, self).getElements():
            yield element
...
class Results(TourneyPrintout):
    ...
    def getElements(self):
        for element in super(Results, self).getElements():
            yield element
...
class Matches(TourneyPrintout):
    ...
    def getElements(self):
        for element in super(Matches, self).getElements():
            yield element
    ...
    def getArgumentsFromRequest(klass, session, request):
        ...
        if 'idchampionship' in kw:
            try:
                ...
                raise OperationAborted(
                    t(_(u'Invalid idchampionship: $idchampionship',
                        mapping=dict(idchampionship=repr(kw['idchampionship'])))))
            entity = session.query(Championship).get(idchampionship)
            if entity is None:
                raise OperationAborted(
                    t(_(u'Championship $idchampionship does not exist',
                        mapping=dict(idchampionship=kw['idchampionship']))))
            return [entity]

        raise OperationAborted(
            t(_(u'Missing request argument "idchampionship" for class $classname',
                mapping=dict(classname=str(klass)))))
...
class SinglesRanking(ChampionshipRanking):
    ...
    def getElements(self):
        for element in super(SinglesRanking, self).getElements():
            yield element
    ...
    def getArgumentsFromRequest(klass, session, request):
        ...
        if 'idrating' in kw:
            try:
                ...
                raise OperationAborted(
                    t(_(u'Invalid idrating: $idrating',
        ...
        raise OperationAborted(
            t(_(u'Missing request argument "idrating" for class $classname',
                mapping=dict(classname=str(klass)))))
