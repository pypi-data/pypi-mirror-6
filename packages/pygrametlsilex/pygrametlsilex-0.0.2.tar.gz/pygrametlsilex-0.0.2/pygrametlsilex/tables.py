...
import pygrametlsilex
import pygrametlsilex.parallel
from pygrametlsilex.FIFODict import FIFODict
...
class Dimension(object):
    ...
    def __init__(self, name, key, attributes, lookupatts=(), 
        ...
        if targetconnection is None:
            targetconnection = pygrametlsilex.getdefaulttargetconnection()
            if targetconnection is None:
        ...
        pygrametlsilex._alltables.append(self)
...
class CachedDimension(Dimension):
    ...
    def _after_insert(self, row, namemapping, newkeyvalue):
        ...
        if self.cacheoninsert:
            ...
            if self.cachefullrows:
                tmp = pygrametlsilex.project(self.all, row, namemapping)
...
class SlowlyChangingDimension(Dimension):
    ...
    def __init__(self, name, key, attributes, lookupatts, versionatt, 
                 ...
                 srcdateatt=None, srcdateparser=pygrametlsilex.ymdparser,
        ...
        """Arguments:
           ...
           - fromfinder: a function(targetconnection, row, namemapping) 
             ...
             pygrametlsilex.today is used as fromfinder. If fromfinder is None
             and srcdateatt is not None, 
             pygrametlsilex.datereader(srcdateatt, srcdateparser) is used.
           ...
           - srcdateparser: a function that takes one argument (a date in the
             ...
             Default: pygrametlsilex.ymdparser (i.e., the default value is a
        ...
        if fromfinder is not None:
            ...
            self.fromfinder = pygrametlsilex.datereader(srcdateatt, srcdateparser)
        else:                        #fromfinder is None and srcdateatt is None
            self.fromfinder = pygrametlsilex.today
        self.toatt = toatt
        if tofinder is None:
    ...
    def _after_insert(self, row, namemapping, newkeyvalue):
        ...
        if self.__cachesize:
            self._after_lookup(row, namemapping, newkeyvalue)
            tmp = pygrametlsilex.project(self.all[1:], row, namemapping)
            tmp[self.key] = newkeyvalue
            self._after_getbykey(newkeyvalue, tmp)
...
class BatchFactTable(FactTable):
    ...
    def _before_insert(self, row, namemapping):
        self.__batch.append(pygrametlsilex.project(self.all, row, namemapping))
...
class BulkFactTable(object):
    ...
    def _insertwithnulls(self, row, namemapping={}):
        ...
        data = [pygrametlsilex.getstrornullvalue(val, self.nullsubst) \
                for val in rawdata]
        self.__count += 1
...
class SubprocessFactTable(object):
    ...
    called (and endload() is called from pygrametlsilex.commit()).
    """
...
class DecoupledDimension(pygrametlsilex.parallel.Decoupled):
    ...
    def __init__(self, dim, returnvalues=True, consumes=(), attstoconsume=(),
        ...
        """Arguments:
           ...
           - queuesize: the maximum amount of waiting batches. Infinite if
             ...
             Default: 200
        """
        pygrametlsilex.parallel.Decoupled.__init__(
            self, dim, returnvalues, consumes,
            tuple([(0, a) for a in attstoconsume]),
            batchsize, queuesize, False)
        if dim in pygrametlsilex._alltables:
            pygrametlsilex._alltables.remove(dim) # We add self instead...
        pygrametlsilex._alltables.append(self)
...
class DecoupledFactTable(pygrametlsilex.parallel.Decoupled):
    ...
    def __init__(self, facttbl, returnvalues=True, consumes=(),
        ...
        pygrametlsilex.parallel.Decoupled.__init__(
            ...
            tuple([(0, a) for a in attstoconsume]),
            batchsize, queuesize, False)
        if facttbl in pygrametlsilex._alltables:
            pygrametlsilex._alltables.remove(facttbl) # We add self instead
        pygrametlsilex._alltables.append(self)
