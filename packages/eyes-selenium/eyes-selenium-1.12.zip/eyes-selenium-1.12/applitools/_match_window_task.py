...
from applitools.utils._image_utils import png_image_from_bytes
...
class MatchWindowTask(object):
    ...
    def _prepare_match_data(self, region, tag, ignore_mismatch):
    ...
    def _match(self, region, tag, ignore_mismatch=False):
        data = self._prepare_match_data(region, tag, ignore_mismatch)
        as_expected = self._agent_connector.match_window(self._running_session, data)
        return as_expected

    def _run_once(self, region, tag, wait_before_run=None):
        if wait_before_run:
            time.sleep(wait_before_run)
        return self._match(region, tag)

    def _run_with_intervals(self, region, tag, total_run_time):
        start = time.time()
        match_retry = total_run_time
        while match_retry > 0:
            time.sleep(self._MATCH_INTERVAL)
            if self._match(region, tag, True):
                return True
            match_retry -= (time.time() - start)
        # One last try
        return self._match(region, tag)

    def match_window(self, region, tag, run_once_after_wait=False):
        if not self._max_window_load_time:
            result = self._run_once(region, tag)
        elif run_once_after_wait:
            result = self._run_once(region, tag, self._max_window_load_time)
        else:
            result = self._run_with_intervals(region, tag, self._max_window_load_time)
