...
class PoissonDistribution(Distribution):
    ...
    def __call__(self,N):
        ...
        logProb=math.log(self.Numerator)-math.log(self.Denominator)
        logProb*=N
        logProb-=(self.Numerator/self.Denominator)
        logProb-=math.lgamma(N+1)
...
class BayesianModel(object):
    ...
    def PriorProbs(self,Observations,PriorDist=None):
        ...
        if PriorDist==None:
            PriorDist=self.Prior
        return Distribution(dict(((state,PriorDist(state)*sum((self.Conditionals[state](outcome)*Observations(outcome) for outcome in Observations.States()))) for state in self.Prior.States())))
    ...
    def Outcomes(self):
        ...
        seen=set()
        for state in self.Conditionals:
            for outcome in self.Conditionals[state].States():
