...
if sys.platform.startswith('java'):
    ...
    import pygrametlsilex.jythonmultiprocessing as multiprocessing
else:
...
def _getexitfunction():
    """Return a function that halts the execution of pygrametlsilex.

       pygrametlsilex uses the function as excepthook in spawned processes such that
...
def _getexcepthook():
    ...
    def excepthook(exctype, excvalue, exctraceback):
        import traceback
        sys.stderr.write(
            "An uncaught exception occured. Terminating pygrametlsilex.\n")
...
class Decoupled(object):
    ...
    def shutdowndecoupled(self):
        ...
        try:
            pygrametlsilex._alltables.remove(self)
        except ValueError:
            pass
...
class SharedConnectionWrapperClient(object):
    ...
    def __init__(self, toserver, fromserver, freelines, connectionmodule,
        ...
        if pygrametlsilex._defaulttargetconnection is None:
            pygrametlsilex._defaulttargetconnection = self
    ...
    def execute(self, stmt, arguments=None, namemapping=None, translate=True):
        ...
        if namemapping and arguments:
            arguments = pygrametlsilex.copy(arguments, **namemapping)
        elif arguments:
...
def shareconnectionwrapper(targetconnection, maxclients=10, userfuncs=()):
    ...
    Arguments:
    - targetconnection: a pygrametlsilex ConnectionWrapper
