...
def DescriptiveStatistics(the_list):
    ...
    mode = getMode(the_list)
    stdDeviation = np.std(the_list,  ddof=1)
    printBlock(('Mean', mean), ('Median', median), ('Mode', mode), ('Range', range), ('Variance', variance), ('Standard Deviation', stdDeviation))
...
def drawOneLine():
    printLeftRightAdj(itemwid * '-', outputwid * '-')


def printBlock(*paraList):
    drawOneLine()
    for a in paraList:
        printLeftRightAdj(a[0], a[1])
    drawOneLine()
    print ''


def getMode(the_list):
...
def IntervalEstimation(the_list, ci=0.95):
    ...
    printBlock(('constrct a ' + str(ci), '$(' + str(lower.round(3)) + ', ' + str(upper.round(3)) + ')'))
...
def SimpleCorrelationAnalysis(xarray, yarray):
    ...
    printBlock(('correlation coefficient', str(r)), ('Specify H' + '₀' + ' and ' + 'H' + '₀', 'H' + '₀'), ('Determine the p-value', 'P= ' + str(p) + ' > α = 0.05'))


def showUsage(fun, *args):
...
def stathelp():
    showUsage(DescriptiveStatistics, [14, 20, 18, 17, 15])
    showUsage(IntervalEstimation, [110.6, 111.2, 110.4, 112.2, 111.3, 110.2, 110.3, 112.5])
    showUsage(SimpleCorrelationAnalysis, [35, 90, 37], [34, 54, 64])
