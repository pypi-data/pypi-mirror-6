...
def main():
	...
	# the below combination will allow '/' on its own to be used, thus sync with root folder
	if not args.redid_dir.endswith( '/' ):
		common.log( "The redid-directory is expected to end in a /" )
		sys.exit(1)
	if args.redid_dir.startswith( '/' ):
		args.redid_dir = args.redid_dir[1:]
...
def get_file_type( redid_config, filename ):
	"""
		Determine the type of the file, and it's basename (less extensions as appropriate)
	"""
	ct = None
	ext = None
	...
	if ct == None:
		ct, ext = common.guess_mime_type( filename )
	
	# strip extension of image types, to match server handling
	if ct.startswith( 'image/' ):
		return ct, filename[:-len(ext)]
