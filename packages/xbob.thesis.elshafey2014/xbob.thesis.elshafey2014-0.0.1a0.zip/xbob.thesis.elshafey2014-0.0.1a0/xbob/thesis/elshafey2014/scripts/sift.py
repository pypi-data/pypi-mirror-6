...
def command_line_arguments(command_line_parameters):
  ...
  parser.add_argument('-p', '--preprocessing', default = 'default-keypoint', help = 'Image preprocessing configuration.')
  # - the features configuration
  parser.add_argument('-f', '--features', default = 'default-sift', help = 'Feature extraction configuration.')
  ...
  parser.add_argument('-b', '--sub-directory', metavar = 'DIR', default = 'sift', help = 'The sub-directory where the files of the current experiment should be stored. Please specify a directory name with a name describing your experiment.')
  # - the temp directory
...
config_dir = os.path.join(thesis_dir, 'xbob/thesis/elshafey2014/configurations')
script = os.path.join(bin_dir, 'faceverify.py')


def main(command_line_parameters = sys.argv):
  """This script extracts SIFT descriptors extracted at keypoints on a regular grid"""

  # Collect command line arguments
  args = command_line_arguments(command_line_parameters[1:])

  facereclib.utils.info("Executing 'SIFT-keypoint' extraction")
  ...
  if args.temp_directory:
    ...
    command.extend(['--user-directory', args.temp_directory]) # No score computed ...
  ...
  # Add skips
  command.extend(['--skip-projector-training', '--skip-projection', '--skip-enroller-training', '--skip-enrollment', '--skip-score-computation', '--skip-concatenation'])
