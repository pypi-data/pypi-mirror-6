...
void IClpSimplex::setStatusArray(char* status, int n){
    for (int i = 0 ; i < n ; i++)
        setStatus(i, static_cast<Status>(status[i]));
}
...
CoinWarmStart* IClpSimplex::getWarmStart(){
    OsiClpSolverInterface osi(this, false);
    return osi.getWarmStart();
}

bool IClpSimplex::setWarmStart(const CoinWarmStart* warmstart){
    OsiClpSolverInterface osi(this, false);
