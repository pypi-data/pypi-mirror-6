...
def with_sleep(method, *args):
    ...
    if self.target_time is None:
        ...
        if delta > 0:
            dummy_logger('Sleep'.format(delta))
            time.sleep(self.CORRECTION_FACTOR * delta)


def dummy_logger(msg):
    with open('/tmp/spring.log', 'a') as fh:
        fh.write('{}\n'.format(msg))
...
class QueryWorker(Worker):
    ...
    def do_batch(self):
        ...
        for _ in xrange(self.BATCH_SIZE):
            ...
            dummy_logger('Query: {}'.format(query))
    ...
    def run(self, sid, lock, curr_queries, curr_items, deleted_items):
        ...
        try:
            dummy_logger('Started: query-worker-{}'.format(self.sid))
            ...
            dummy_logger('Interrupted: query-worker-{}'.format(e))
        else:
            dummy_logger('Finished: query-worker-{}'.format(self.sid))
...
class WorkloadGen(object):
    ...
    def start_query_workers(self, curr_items, deleted_items):
        ...
        dummy_logger('Starting {} workers'.format(self.ws.query_workers))
        for sid in range(self.ws.query_workers):
            ...
            worker_process.start()
            dummy_logger('Started {}'.format(self.ws.query_workers))
