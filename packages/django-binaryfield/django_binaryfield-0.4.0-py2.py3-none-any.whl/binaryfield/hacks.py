...
def hack_engine(engine_module, ):
    try:
        ...
        # try to fallback to the .base
        creation = importlib.import_module(engine_module + '.base')

    byte_type_mapping = {
        'django.db.backends.sqlite3': 'BLOB',
        'django.db.backends.oracle': 'BLOB',
        'django.db.backends.postgresql_psycopg2': 'bytea',
        'django.db.backends.mysql': 'longblob',
    }

    # a place in settings to define your own type mappings for custom backends
    EXTRA_TYPE_MAPPING = getattr(settings, 'BINARYFIELD_EXTRA_TYPE_MAPPING', {})
    byte_type_mapping.update(EXTRA_TYPE_MAPPING)
