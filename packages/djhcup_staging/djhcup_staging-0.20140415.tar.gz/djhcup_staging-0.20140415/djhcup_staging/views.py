...
def src_gen_master_table(request, source_id):
    ...
    try:
        ...
        print "cursor.execute() without error"
        tx.commit(using=DJHCUP_DB_ENTRY)
        print "tx.commit() without error"
    except:
        e = sys.exc_info()[0]
    ...
    table_record = ImportTable(source=source, name=db_table, database=DJHCUP_DB_ENTRY, schema=settings[DJHCUP_DB_ENTRY]['schema'])
    table_record.save()
