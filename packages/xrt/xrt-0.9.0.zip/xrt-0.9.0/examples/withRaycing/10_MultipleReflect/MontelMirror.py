...
class MontelMirror(roe.SimpleVCM):
    def local_z(self, x, y):
        dz = abs(x)
        return roe.SimpleVCM.local_z(self, x, y) * sqrt2 + dz
...
class MontelMirrorP(roe.SimpleVCM, roe.SurfaceOfRevolution):
    _h = 100

    def local_r(self, s, phi):
        z = -roe.SimpleVCM.local_z(self, 0, s) + self._h
        r = np.empty_like(phi)
    ...
    def local_n(self, s, phi):
        a, b, c = roe.SimpleVCM.local_n(self, 0, s)
