...
class NailgunTask(Task):
  ...
  def __init__(self, context, minimum_version=None, jdk=False):
    ...
    self._java = (NailgunExecutor(workdir, classpath, distribution=self._dist)
                  if context.options.nailgun_daemon else SubprocessExecutor(self._dist))
  ...
  def runjava(self, classpath, main, args=None, jvm_args=None):
    ...
    """
    try:
      return self.java_executor.execute(classpath, main, args=args,
                                        jvm_args=jvm_args or self.jvm_args)
  ...
  def profile_classpath(self, profile):
    ...
    """
    updated, classpath = profile_classpath(profile,
