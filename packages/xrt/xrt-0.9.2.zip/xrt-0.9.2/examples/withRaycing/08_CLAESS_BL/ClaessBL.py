...
def build_beamline(nrays=raycing.nrays):
    ...
    rs.GeometricSource(
        beamLine, 'Source', (xWiggler, yWiggler, height), nrays=nrays,
        distE='flat', energies=(2000, 25000), polarization='horizontal')
    ...
    beamLine.filter1 = roe.Plate(
        beamLine, 'Filter1', (-42740.918, -17456.772, height), pitch=np.pi/2,
    ...
    beamLine.dcm = roe.DCMOnTripodWithOneXStage(
        beamLine, 'DCM', [-45342.09, -14855.6, 1415.],
        surface=('Si311', 'Si111'), material=(si311_1, si111_1),
    ...
    beamLine.BSBlock = ra.RectangularAperture(
        beamLine, 'BSBlock', -45988.52, -14209.17, ('bottom',), (22,),
        alarmLevel=0.)
...
#= make narrow energy band before DCM =========================================
    if shouldShrinkEnergy:
        beamVCMglobal.project_energy_to_band(beamLine.dcm.energyMin,
                                             beamLine.dcm.energyMax)
        rs.make_polarization(beamLine.sources[0].polarization,
                             beamVCMglobal.Jss, beamVCMglobal.Jpp,
                             beamVCMglobal.Jsp)
#= -end- make narrow energy band before DCM ===================================
...
def print_beamline_positions(
    ...
    pitch=None, nameDCMcrystal='Si111', bragg=None, energy=9000.,
    fixedExit=20., nameDiagnFoil='top-edge', nameVFMcylinder='Rh',
        heightVFM=None):
    ...
    print ('beamLine.slitEH.opening (l, r, b, t) = [{0[0]:.3f},' +
           r' {0[1]:.3f}, {0[2]:.3f}, {0[3]:.3f}]').format(
        beamLine.slitEH.opening)
