...
def compruebaFranjaHoraria2(dates,actual_tz_request):
    
    madrid = timezonepytz(settings.TIME_ZONE)
...
def preCompruebaFranjaHoraria (part_where,actual_tz):
    ...
    aux_fecha0,aux_fecha1=compruebaFranjaHoraria2(part_where[dim_time],actual_tz)
...
def requestCube(ip_connect,name_bbdd,name_cube,col_cube,med_cube,request_data,ifDate,dimension_filters,database_cube_dict,total_data = False,where_tupla=('Date','Id','Id'),request_date_previous = False,complete_name=False,format_cube=False):
    ...
    where=preCompruebaFranjaHoraria(where,request_data['django_timezone'])
