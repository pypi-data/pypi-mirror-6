...
class EngagementComputer(object):
    ...
    def __init__(self, coursesStartYearsArr, dbHost, dbName, tableName, openMySQLDB=None, mySQLUser=None, mySQLPwd=None, courseToProfile=None):
        '''
    ...
    def run(self):
        ...
        try:
            ...
            if self.courseToProfile is None:
                # Profile all courses:
                queryIterator = self.db.query('use %s; SELECT course_display_name, anon_screen_name, time, isVideo FROM %s ORDER BY course_display_name, anon_screen_name, time;' % 
                                              self.dbName, self.tableName)
            else:
                queryIterator = self.db.query('use %s; SELECT course_display_name, anon_screen_name, time, isVideo FROM %s WHERE course_display_name LIKE "%s" ORDER BY anon_screen_name, time;' % 
                                              (self.dbName, self.tableName, self.courseToProfile))
            ...
            if self.db is not None:
                try:
                    # If creator of this instance passed an open
                    # db into this instance, don't close that db.
                    # But if we opened one ourselves, close it:
                    if self.openOwnDB:
                        self.db.close()
                except Exception as e:
