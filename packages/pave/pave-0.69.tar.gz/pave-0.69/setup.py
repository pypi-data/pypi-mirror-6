...
try:
    with open('readme.rst') as f:
        long_description = ['|\n', '|\n', '|\n','\n'] # push logo under login
        for line in f:
            if line.startswith('.. toctree:'):  # rm sphinx directives at end
                break
...
setup(
    name              = name,
    version           = __version__,
    description       = ( u'Simple push-based configuration and deployment ' +
                          u'tool, leveraging fabric.  ' +  # nbsp at end
                          u'No servers, few dependencies.'),
    author            = 'Mike Miller',
