...
def disconnect(host, port):
    global _OpenedConnections
    for connection in _OpenedConnections.get((host, int(port)), []):
...
if __name__ == "__main__":
    ...
    if cmd == 'receive':
        ...
        reactor.callLater(5, send, 'c:\.buffer\N.zip', '127.0.0.1', 6661)
        reactor.callLater(5.01, send, 'c:\.buffer\N.zip', '127.0.0.1', 6661)
        reactor.callLater(5.02, send, 'c:\.buffer\N.zip', '127.0.0.1', 6661)
        reactor.callLater(5.03, send, 'c:\.buffer\N.zip', '127.0.0.1', 6661)
        reactor.callLater(5.04, send, 'c:\.buffer\N.zip', '127.0.0.1', 6661)
        reactor.run()
    elif cmd == 'send':
