...
try:
    ...
    try:
        ...
        has_sip = 1
        print('using QVariant 1')
except:
    print('could not import sip')
    SIP_VERSION_STR = 0
    has_sip = 0
    use_qt = 0

# for testing use_qt can be set to 5 (the default), 4 or 0
use_qt = 5

qt_version = 0  # this will be overridden below
if use_qt == 5:
    try:
        from PyQt5.QtCore import (QProcess, PYQT_VERSION_STR, QT_VERSION_STR)
        ...
        use_qt = 4   # since 5 didn't work, try 4

if use_qt == 4:
    try:
        from PyQt4.QtCore import (QProcess, PYQT_VERSION_STR, QT_VERSION_STR)
...
if qt_version == 5:
    ...
    def d_to_str(qlcl, d, s):
        return(qlcl.toString(QDate(d), s))

    def dt_to_str(qlcl, dt, s):
        return(qlcl.toString(QDateTime(dt), s))
    ...
    def dt_to_str(qlcl, dt, s):
        if python_version == 2:
            return(unicode(qlcl.toString(QDateTime(dt), s).toUtf8().data(), encoding))
        else:
            return(qlcl.toString(QDateTime(dt), s))
...
the file {0}:""".format(f))
    ...
    def do_n(self, arg_str='', itemstr=""):
        if type(arg_str) == unicode:
            ...
            item = unicode(arg_str.strip(), encoding)
        if item:
            new_hsh, msg = str2hsh(item, options=self.options)
            if msg:
                print("\n".join(msg))
