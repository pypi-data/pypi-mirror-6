...
from slickloghandler.util import u
from slickloghandler.util import new_queue
from slickloghandler.util import queue_mod
from slickloghandler.util import httplib_mod
from slickloghandler.util import quote_p
...
class SlickLogRemoteLogger:
    ...
    try:
      ...
      while size < max_size and not self._timeout_passed(conn, start_time):
        try:
          ...
          message = self.log_queue.get(True, SlickLogRemoteLogger.__MAX_QUEUE_WAIT)
          log('pushing message {0}', message)
  ...
  def _send_buffer(self, conn, buf):
    try:
      bytes = ''.join(buf).encode(SlickLogRemoteLogger.__CHAR_ENCODING)
      ...
      with self.log_queue.mutex:
        self.log_queue.queue.appendleft(''.join(buf))
      raise ex
