...
###############################################################################
"""Checbox widget
$Id: checkbox.py 3934 2014-03-17 07:38:52Z roger.ineichen $
"""
...
class NoWidgetLabelMixin(object):
    """Skip widget label, don't show tiwce (widget and item provide the same)"""
...
class CheckBoxWidget(WidgetMixin, z3c.form.browser.widget.HTMLInputWidget,
    ...
    def items(self):
        ...
        for count, term in enumerate(self.terms):
            ...
            if not self.showItemLabel:
                # skip label, but don't use None, support correct markup
                # otherwise javascript based solutions will not work
                label = '&nbsp;'
            elif zope.schema.interfaces.ITitledTokenizedTerm.providedBy(term):
...
class SingleCheckBoxInlineWidget(NoWidgetLabelMixin, SingleCheckBoxWidget):
...
class SingleCheckBoxInlinePickerWidget(NoWidgetLabelMixin,
    SingleCheckBoxPickerWidget):
    """SingleCheckBoxWidget using a span wrapper for option tags"""
