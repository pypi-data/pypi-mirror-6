...
class Addition(object):
    ...
    def merge(self, other):
        for key, entity_ids in other._append_to_lst_registry.iteritems():
            self._append_to_lst_registry[key].extend(entity_ids)
        for key, value in other._update_inc_registry.iteritems():
    ...
    def apply(self, txn, graph):
        ...
        for key, entity_ids in self._append_to_lst_registry.iteritems():
        ...
        self._applied = True
        # Increasing value
        for key, value in self._update_inc_registry.iteritems():
...
class Removal(object):
    ...
    def merge(self, other):
        for key, entity_ids in other._remove_from_lst_registry.iteritems():
            self._remove_from_lst_registry[key].extend(entity_ids)
        for key, value in other._update_inc_registry.iteritems():
            self._update_inc_registry[key] += value
        for key, value in other._update_dec_registry.iteritems():
    ...
    def apply(self, txn, graph):
        ...
        for key, entity_ids in self._remove_from_lst_registry.iteritems():
            graph._bulk_remove_from_lst(txn, key, entity_ids)
        # Increasing value
        for key, value in self._update_inc_registry.iteritems():
            graph._update_inc(txn, key, value)
        # Decreasing value
        for key, value in self._update_dec_registry.iteritems():
