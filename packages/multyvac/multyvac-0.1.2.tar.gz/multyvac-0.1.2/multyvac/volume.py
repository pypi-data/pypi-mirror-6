...
class MultyvacSyncError(MultyvacError):
    pass
...
class Volume(MultyvacModel):
    ...
    def get_file(self, remote_path, local_path):
        """
        Copies the file at :param remote_path: in the volume to the
        :param local_path:.
        """
    ...
    def put_file(self, local_path, remote_path, target_mode=None):
        """
        Copies the file at :param local_path: to the volume's
        :param remote_path:.
        """
        # FIXME: Make more memory efficient
    ...
    def sync_up(self, local_path, remote_path, max_runtime=3600):
        ...
        jid = self.multyvac.job.shell_submit(
            'sleep %s' % max_runtime,
            _name='volume sync down (%r -> %r:%r)' % (local_path,
                                                      self.name,
                                                      remote_path),
            _vol=[self.name]
        )
        ...
        try:
            if not job.wait_for_open_port(22):
                raise MultyvacSyncError('Failed waiting for job to open port')
    ...
    def sync_down(self, remote_path, local_path, max_runtime=3600):
        ...
        jid = self.multyvac.job.shell_submit(
            'sleep %s' % max_runtime,
            _name='volume sync down (%r:%r -> %r)' % (self.name,
                                                      remote_path,
                                                      local_path),
            _vol=[self.name]
        )
        ...
        try:
            ...
            if p.returncode:
                raise MultyvacSyncError(stderr)
        finally:
            job.kill()
