...
import sqlserver_ado.adodbapi.apibase as api
import sqlserver_ado.adodbapi.process_connect_string
from sqlserver_ado.adodbapi.apibase import apilevel, threadsafety, paramstyle
from sqlserver_ado.adodbapi.apibase import Warning, Error, InterfaceError, DatabaseError, DataError, OperationalError, IntegrityError
from sqlserver_ado.adodbapi.apibase import InternalError, ProgrammingError, NotSupportedError, FetchFailedError
from sqlserver_ado.adodbapi.apibase import NUMBER, STRING, BINARY, DATETIME, ROWID
...
if sys.version[0] >= '3': #python 3.x
    makeByteBuffer = api.makeByteBuffer
else:                   #python 2.x
...
class Connection(object):
    ...
    def close(self):
        ...
        self.proxy = None                             #v2.4.2.2 fix subtle timeout bug
    ...
    def commit(self):
        ...
        try:
            ...
            raise api.DatabaseError('Error doing COMMIT')
    ...
    def _rollback(self):
        ...
        try:
            self.proxy.rollback()
        except ImportError:  # Pyro trying to send a Windows COM error
            self._raiseConnectionError(api,OperationalError, 'Error during rollback.')
    ...
    def __getattr__(self, item):
        ...
        elif item in ('dbms_name', 'dbms_version', 'connection_string'):
            return self.getIndexedValue(item)
        else:
            raise AttributeError('No remote access for attribute="%s"' % item)
...
def fixpickle(x):
    ...
    if isinstance(x, dict):
        ...
        for arg,val in x.items():
            if isinstance(val, api.memoryViewType):
    ...
    newargs = []
    for arg in x:
        if isinstance(arg, api.memoryViewType):
