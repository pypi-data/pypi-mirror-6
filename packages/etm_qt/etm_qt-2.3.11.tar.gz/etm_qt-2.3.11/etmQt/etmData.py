...
#         mesg = """\
# Could not parse date = '%s', timezone = '%s', fmt = '%s'""" % (dt, timezone, fmt)
#         print(mesg)
...
""")
    ...
    def do_d(self, arg_str):
        ...
        print("{0:-^{4}}\n{1}\n{2:-^{4}}\n{3}".format(tr(' Details '),
              hsh['entry'], tr(" from "), filetext, self.line_length))
...
""")
            return(False)
        f, begline, endline = item_hsh['fileinfo']
...
Usage
    e INT
If there is an item number INT among those displayed
by the previous 'c' or 's' command then open it for editing.
The entry will be validated and any errors can be corrected.
...
etm shell loop commands:
    ...
    Note: commands C, e, E, n (without ARGS), o and r require
    that 'edit_cmd' be set in etm.cfg. Additionally, if 'hg_commit'
    is specified in etm.cfg, any changes will be committed.\
""")
        else:
            print("""\
etm commands:
