...
class Player(GloballyUnique, Base):
    ...
    idplayer = Column(
        ...
        info=dict(label=N_(u'Player ID'),
                  hint=N_(u'Unique ID of the player.')))
    """Primary key."""

    firstname = Column(
        name_t,
        nullable=False,
        info=dict(label=N_(u'First name'),
                  hint=N_(u'First name of the player.')))
    """Player's first name."""

    lastname = Column(
        name_t,
        nullable=False,
        info=dict(label=N_(u'Last name'),
                  hint=N_(u'Last name of the player.')))
    """Player's last name."""
    ...
    nickname = Column(
        ...
        nullable=False,
        default='',
        info=dict(label=N_(u'Nickname'),
                  hint=N_(u'Nickname of the player, for '
                          u'login and to disambiguate homonyms.')))
    ...
    _password = Column(
        ...
        nullable=False,
        default='*',
        info=dict(label=N_(u'Password'),
                  hint=N_(u'Login password of the player.')))
    """Crypted password."""

    sex = Column(
        flag_t,
        info=dict(label=N_(u'Sex'),
                  hint=N_(u'Male or Female.'),
                  dictionary=dict(F=N_(u'Female'),
                                  M=N_(u'Male'))))
    """To compute different championship: `F` means *female*, `M` means *male*."""

    birthdate = Column(
        date_t,
        info=dict(label=N_(u'Birthdate'),
                  hint=N_(u'Date of birth of the player.')))
    """Date of birth of the player."""

    nationality = Column(
        nationality_t,
        info=dict(label=N_(u'Country'),
                  hint=N_(u'The country the player plays for.')))
    ...
    citizenship = Column(
        ...
        nullable=False,
        default=True,
        info=dict(label=N_(u'Citizenship'),
                  hint=N_(u'Whether the player belongs legally to the given'
                          u' country or not.')))
    """Whether the player belongs legally to the given country or not."""

    language = Column(
        language_t,
        info=dict(label=N_(u'Language code'),
                  hint=N_(u'The code of the preferred language by the player.')))
    """The `ISO code <http://en.wikipedia.org/wiki/ISO_639-1>`_ of the preferred
       language of the player."""

    email = Column(
        email_t,
        info=dict(label=N_(u'Email'),
                  hint=N_(u'Email address of the player.')))
    """Email address of the player."""

    phone = Column(
        phone_t,
        info=dict(label=N_(u'Phone'),
                  hint=N_(u'Phone number of the player.')))
    """Phone number of the player."""

    idclub = Column(
        intid_t, ForeignKey('clubs.idclub'),
        info=dict(label=N_(u'Club ID'),
                  hint=N_(u'ID of the club the player is member of.')))
    """Membership club's ID."""

    idfederation = Column(
        intid_t, ForeignKey('clubs.idclub'),
        info=dict(label=N_(u'Federation ID'),
                  hint=N_(u'ID of the federation the player is associated with.')))
    """Membership federation's ID."""

    portrait = Column(
        filename_t,
        info=dict(label=N_(u'Portrait'),
                  hint=N_(u'File name of the PNG, JPG or GIF '
                          u'portrait of the player.')))
    ...
    def delete(self):
        ...
        if nt > 0:
            raise OperationAborted(ngettext(
                u'Deletion not allowed: $player is a competitor in $count tourney!',
                u'Deletion not allowed: $player is a competitor in $count tourneys!',
    ...
    def caption(self, html=None):
        ...
        if self.shouldOmitNickName():
            ...
            if html is None or html:
                format = N_(u'<b>$lastname</b> $firstname')
            else:
                format = N_(u'$lastname $firstname')
        else:
            nickname = u'“%s”' % self.nickname
            if html is None or html:
                format = N_(u'<b>$lastname</b> $firstname $nickname')
            else:
                format = N_(u'$lastname $firstname $nickname')
        return _(format, mapping=dict(lastname=self.lastname,
    ...
    def mergePlayers(self, other_players):
        ...
        for t in tourneys:
            ...
            if self.idplayer in allplayers:
                raise OperationAborted(_(u'Cannot merge given people '
                                         u'because "$player" is present in '
                                         u'tourney "$tourney"',
            ...
            if count > 1:
                raise OperationAborted(_(u'Cannot merge given people '
                                         u'because $count of them are '
                                         u'playing in tourney "$tourney"',
        ...
        for i in sourceids:
            ...
            replaced.append(p.caption(False))
            s.delete(p)

        msg = ngettext('Player $names has been merged into $player',
                       'Players $names has been merged into $player',
                       len(replaced),
                       mapping=dict(names=njoin(replaced),
                                    player=self.caption(False)))
        logger.info(msg)
