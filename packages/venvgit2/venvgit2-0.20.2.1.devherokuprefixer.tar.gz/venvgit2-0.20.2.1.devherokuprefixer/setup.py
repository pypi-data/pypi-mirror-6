...
class Dirs(object):
    ...
    def __init__(self):
        self.prefix = os.environ.get('VIRTUAL_ENV', os.environ.get('PREFIX'))
        if self.prefix is None:
            warn('this does not look like a virtualenv and no prefix was found')
            warn('trying with /usr/local (may fail with permission error)...')
        ...
        with catch_warnings(record=True) as warnings:
            try:
                if not cmake_is_installed():
                    warn("cmake does not appear to be installed...")
                if not pythondev_is_installed():
                    warn("python development headers do not appear to be installed...")
                with Dirs() as dirs:
                ...
                raise SystemExit('Install failed :(\n'
                    ...
                    '\n'.join(str(w.message) for w in warnings), process_error))
...
setup(
    ...
    version="0.20.2.1.devherokuprefixer",
    cmdclass={
