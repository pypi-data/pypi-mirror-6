...
class AsyncApi(object):
    ...
    def GetJobs(self, userId, **kwargs):
        ...
        Args:
            ...
            actions, str: Actions (optional)
            excludedActions, str: Excluded actions (optional)
            jobName, str: Foltred job name (optional)
            
        Returns: GetJobsResponse
        """
        if( userId == None ):
            raise ApiException(400, "missing required parameters")
        allParams = ['userId', 'pageIndex', 'pageSize', 'datetime', 'statusIds', 'actions', 'excludedActions', 'jobName']
        ...
        resourcePath = '/async/{userId}/jobs?page={pageIndex}&count={pageSize}&date={date}&statusIds={statusIds}&actions={actions}&excluded_actions={excludedActions}&jobName={jobName}'.replace('*', '')
        pos = resourcePath.find("?")
        if pos != -1:
            resourcePath = resourcePath[0:pos]
        resourcePath = resourcePath.replace('{format}', 'json')
