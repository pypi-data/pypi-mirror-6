...
class Match(Base):
    ...
    idmatch = Column(
        ...
        primary_key=True,
        nullable=False,
        info=dict(label=N_(u'Match ID'),
                  hint=N_(u'Unique ID of the match.')))
    """Primary key."""
    ...
    turn = Column(
        smallint_t,
        nullable=False,
        info=dict(label=N_(u'Turn #'),
                  hint=N_(u'Turn number.')))
    """Turn of the match."""

    board = Column(
        smallint_t, nullable=False,
        info=dict(label=N_(u'Board #'),
                  hint=N_(u'Board where this match is played.')))
    """The number of the carromboard this match is played on."""

    idcompetitor1 = Column(
        intid_t, ForeignKey('competitors.idcompetitor'),
        nullable=False,
        info=dict(label=N_(u'1st competitor ID'),
                  hint=N_(u'ID of the first competitor.')))
    """First competitor's ID."""

    idcompetitor2 = Column(
        intid_t, ForeignKey('competitors.idcompetitor'),
        info=dict(label=N_(u'2nd competitor ID'),
                  hint=N_(u'ID of the second competitor.')))
    ...
    score1 = Column(
        ...
        nullable=False,
        default=0,
        info=dict(label=N_(u'1st score'),
                  hint=N_(u"Score of the first competitor."),
                  min=0, max=25))
    ...
    score2 = Column(
        ...
        nullable=False,
        default=0,
        info=dict(label=N_(u'2nd score'),
                  hint=N_(u"Score of the second competitor."),
                  min=0, max=25))
    ...
    def caption(self, html=None):
        ...
        comp1 = self.competitor1.caption(html)
        if self.competitor2:
            comp2 = self.competitor2.caption(html)
        else:
        ...
        if html is None or html:
            if self.tourney.championship.playersperteam > 1:
                ...
                format = N_(u'$comp1<br/><i>vs.</i><br/>$comp2')
            else:
                # TRANSLATORS: this is used to format the description
                # of a match for single events
                format = N_(u'$comp1 <i>vs.</i> $comp2')
        else:
            format = N_(u'$comp1 vs. $comp2')
        return _(format, mapping=dict(comp1=comp1, comp2=comp2))
    ...
    def competitor1FullName(self):
        c1 = self.competitor1
        return c1.description if c1 else _(u'Player NOT assigned yet!')

    @property
    def competitor2FullName(self):
        c2 = self.competitor2
        return c2.description if c2 else _(u'Phantom')
    ...
    def results(self):
        ...
        if self.competitor2 is None:
            return self.competitor1, None, 25
            ...
            raise OperationAborted(
                _(u'How could match "$match" end without result?!?',
