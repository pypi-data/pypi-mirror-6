...
class GitCommand(Resource):
    ...
    def render(self, request):
        ...
        log.msg("Spawning git command: %s with args: %r" % (self.cmd, self.args))
    ...
    def render_GET(self, request):
        if 'service' not in request.args:
            ...
            request.setHeader('Content-Type',
                              'application/x-git-' + rpc + '-advertisement')
            request.write(git_packet('# service=git-' + rpc + '\n') + git_packet())
            cmd = self.gitcommand
...
class GitResource(Resource):
    ...
    def getChild(self, path, request):
        ...
        if (len(pathparts) >= 2 and
            ...
            pathparts[-1] == 'refs' and
            path_info['repository_fs_path'] is not None):
        ...
        elif (len(pathparts) >= 1 and pathparts[-1] == 'git-upload-pack' and
              path_info['repository_fs_path'] is not None):
            cmd = 'git'
        ...
        elif (len(pathparts) >= 1 and pathparts[-1] == 'git-receive-pack' and
              path_info['repository_fs_path'] is not None):
            writerequired = True
            cmd = 'git'
        ...
        # static files as specified in file_headers or fallback webfrontend
            ...
            if filename is not None and path_info['repository_fs_path'] is not None:
