...
class Client(object):
    ...
    def __init__(self, application_id, login, auth_token, sandbox=False, password=None, debug=False):
    ...
    def _fetch(self, url, args):
        ...
        ts = time.time()
        if self.debug:
            logging.debug("Fetching %s, args: %s" % (url, args))
        data = urllib2.urlopen(url, args, 60).read()
        if self.debug:
            logging.debug("Fetching took %f seconds." % (time.time() - ts))
        return data
