...
class Tourney(GloballyUnique, Base):
    ...
    idtourney = Column(
        ...
        primary_key=True,
        nullable=False,
        info=dict(label=N_(u'Tourney ID'),
                  hint=N_(u'Unique ID of the tourney.')))
    """Primary key."""

    idchampionship = Column(
        intid_t, ForeignKey('championships.idchampionship'),
        nullable=False,
        info=dict(label=N_(u'Championship ID'),
                  hint=N_(u'ID of the championship the tourney belongs to.')))
    """Related championship's ID."""

    idrating = Column(
        intid_t, ForeignKey('ratings.idrating'),
        info=dict(label=N_(u'Rating ID'),
                  hint=N_(u'ID of the rating this tourney uses.')))
    ...
    date = Column(
        ...
        info=dict(label=N_(u'Date'),
                  hint=N_(u'Date of the event.')))
    """Event date."""

    description = Column(
        description_t,
        nullable=False,
        info=dict(label=N_(u'Tourney'),
                  hint=N_(u'Description of the tourney.')))
    """Event description."""

    location = Column(
        description_t,
        info=dict(label=N_(u'Location'),
                  hint=N_(u'Location of the tourney.')))
    """Event location."""
    ...
    duration = Column(
        ...
        nullable=False,
        default=45,
        info=dict(label=N_(u'Duration'),
                  hint=N_(u'Duration in minutes of each turn.'),
                  min=0))
    ...
    prealarm = Column(
        ...
        nullable=False,
        default=5,
        info=dict(label=N_(u'Prealarm'),
                  hint=N_(u'Prealarm before the end of the'
                          u' turn, usually no more games'
                          u' after that.'),
                  min=0))
    ...
    couplings = Column(
        ...
        info=dict(label=N_(u'Pairing method'),
                  hint=N_(u'Method used to couple competitors at each turn.'),
                  dictionary=dict(serial=N_(u'Ranking order'),
                                  dazed=N_(u'Cross ranking order'))))
    ...
    currentturn = Column(
        ...
        nullable=False,
        default=0,
        info=dict(label=N_(u'Current turn'),
                  hint=N_(u'Current turn number.')))
    """The current turn."""
    ...
    rankedturn = Column(
        ...
        nullable=False,
        default=0,
        info=dict(label=N_(u'Ranked turn'),
                  hint=N_(u'To which turn the ranking is up-to-date with.')))
    ...
    prized = Column(
        ...
        nullable=False,
        default=False,
        info=dict(label=N_(u'Prized'),
                  hint=N_(u'Whether the final prizes has been assigned.')))
    ...
    def caption(self, html=None):
        return _(u'$tourney $championship, $date', mapping=dict(
            tourney=self.description,
            championship=self.championship.caption(html),
    ...
    def ranking(self):
        ...
        if self.currentturn != self.rankedturn:
            try:
                ...
                logger.debug(u'Match with no result, returning old ranking: %s', e)
    ...
    def updateRanking(self):
        ...
        if self.prized:
            raise OperationAborted(
                _(u'Cannot update rankings after prize-giving!'))
    ...
    def makeNextTurn(self):
        ...
        if self.prized:
            raise OperationAborted(
                _(u'Cannot create other turns after prize-giving!'))
    ...
    def _makeFirstTurn(self):
        ...
        while l > 0:
            ...
            m = Match(turn=1, board=b, competitor1=c1, competitor2=c2,
                      score1=c2 is None and 25 or 0, score2=0)
    ...
    def _combine(self, competitors, done, _level=0):
        ...
        if logger.isEnabledFor(logging.DEBUG):
            def debug(msg, *args):
                logger.debug(u"%sL%02d "+msg, "="*_level, _level, *args)
            def C(c):
    ...
    def assignPrizes(self):
        ...
        if self.prized:
            raise OperationAborted(
                _(u'Cannot update prizes after prize-giving!'))
    ...
    def replay(self, date):
        ...
        new = Tourney(date=date,
                      description=_(u'Replica of $tourney',
        ...
        if not self.championship.closed:
            ...
            if championship is None:
                raise OperationAborted(
                    _(u'Cannot replicate tourney, no open championships!'))
