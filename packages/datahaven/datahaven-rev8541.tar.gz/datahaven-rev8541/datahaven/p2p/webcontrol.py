    ...
    def renderPage(self, request):
        ...
        if True:
            ...
            if len(transport_tcp.started_connections()) > 0:
                ...
                src += '</table>\n'
        if transport_control._TransportCSpaceEnable:
            src += '<p>opened CSpace sending streams: <b>%d</b></p>\n' % (
                len(transport_cspace.opened_sending_streams_list()))
            if len(transport_cspace.opened_sending_streams_list()) > 0:
                src += '<table width=100% cellspacing=0 cellpadding=2 border=0>\n'
                src += '<tr bgcolor="#000000">\n'
                ...
                src += '</tr>\n'
                i = 0
                for item in transport_cspace.opened_sending_streams_list():
                    keyID, state, btes = item.split(':')
                    if i % 2: 
                        src += '<tr>\n'
                    else:
                        src += '<tr bgcolor="#f0f0f0">\n'
                    ...
                    src += '<td nowrap>%s</td>' % diskspace.MakeStringFromBytes(btes)
                    src += '</tr>\n'
                src += '</table>\n'
            src += '<p>opened CSpace receiving streams: <b>%d</b></p>\n' % (
                len(transport_cspace.opened_receiving_streams_list()))
            if len(transport_cspace.opened_receiving_streams_list()) > 0:
                src += '<table width=100% cellspacing=0 cellpadding=2 border=0>\n'
                src += '<tr bgcolor="#000000">\n'
                src += '<td align=left nowrap><b><font color="#ffffff">key ID</font></b></td>\n'
                src += '<td align=left nowrap><b><font color="#ffffff">state</font></b></td>\n'
                ...
                src += '</tr>\n'
                i = 0
                for item in transport_cspace.opened_receiving_streams_list():
                    keyID, state, btes = item.split(':')
                    if i % 2: 
                        src += '<tr>\n'
                    else:
