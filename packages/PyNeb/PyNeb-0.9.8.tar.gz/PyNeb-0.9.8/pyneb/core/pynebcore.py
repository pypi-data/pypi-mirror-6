...
class Atom(object):
    ...
    def __init__(self, elem=None, spec=None, atom=None, OmegaInterp='Cheb'):
        ...
        if self.collFileType == 'fits':
            self.CollData = CollDataFits(elem=self.elem, spec=self.spec, atom=self.atom, OmegaInterp=OmegaInterp)
        elif self.collFileType == 'dat':
            self.CollData = CollDataAscii(elem=self.elem, spec=self.spec, atom=self.atom, OmegaInterp='Linear')
        try:
