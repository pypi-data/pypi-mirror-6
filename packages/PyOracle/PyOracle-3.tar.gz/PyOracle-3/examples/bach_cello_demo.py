...
def main():
    s = music21.converter.parse('/Users/Wang/Downloads/Suite No 1 for Cello M1-Prelude.mxl')
    ...
    if len(sys.argv) == 1:
        b, kend, ktrace = gen.generate_symbolic(bo, len(note_obj_seq), 0.0, 0, LRS = 0, weight='max')
    else:
        ...
        LRS = int(sys.argv[4])
        weight = sys.argv[5]
        b, kend, ktrace = gen.generate_symbolic(bo, seq_len, p, k, LRS = LRS, weight = weight)
