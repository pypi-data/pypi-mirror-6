...
def valid_extension(filename, dirs=None):
...
def generate_invenio_files_list(invenio_path=None):
    """Generates the list of all the source files to be monitored"""
    if invenio_path is None:
        invenio_path = config.SRC_PATH
...
def select_destination_path(filename, install_path=None, dests=None):
    ...
    for regexp in config.IGNORE_FILES_RE:
    ...
    for regexp, dest_dir in config.SPECIAL_DIRS_RE:
        m = regexp.match(rel_path)
        if m:
        ...
        for filename in set(files):
            ...
            if old_time is None:
                ...
                if dest and os.path.isfile(dest):
                    ...
                    if dest:
                        print 'missing file', filename, dest
                    old_time = mtime
            ...
            if mtime > old_time:
                ...
                if os.path.basename(filename) in (config.CONFIG_FILENAME,
                                                 ...
                                                 config.AUTOTOOLS_CONFIG_FILENAME):
...
def print_traceback(sig, frame):  # pylint: disable=W0613
    traceback.print_stack(frame)


def start_server(options, server_socket, static_files=None):
    ...
    if options.use_pdb:
        ...
        def pdb_on_error(f, *args, **kwargs):
            try:
                ...
                # We catch on purpose all errors for debugging
                pdb.post_mortem()
    ...
    run_simple(server_socket,
               ...
               use_evalex=True,
               static_files=static,
               threaded=not options.single_threaded,
               processes=1)
...
def _main():
    """Script entrance"""
    setup_logging()
    options, dummy_args = parse_cli_options()
