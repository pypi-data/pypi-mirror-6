...
@expose(Competitor,
        ...
        metadata=dict(
    player1FullName=dict(label=_(u'Player'),
                         hint=_(u'Full name of the player.'),
                         lookup=dict(url='/tourney/players?sort=lastname,firstname',
                                     ...
                                     width=200,
                                     pageSize=12)),
    player2FullName=dict(label=_(u'2nd player'),
                         hint=_(u'Full name of the second player.'),
                         lookup=dict(url='/tourney/players?sort=lastname,firstname',
                                     idField='idplayer',
                                     ...
                                     width=200,
                                     pageSize=12)),
    player3FullName=dict(label=_(u'3rd player'),
                         hint=_(u'Full name of the third player.'),
                         lookup=dict(url='/tourney/players?sort=lastname,firstname',
                                     idField='idplayer',
                                     ...
                                     width=200,
                                     pageSize=12)),
    player4FullName=dict(label=_(u'4th player'),
                         hint=_(u'Full name of the fourth player.'),
                         lookup=dict(url='/tourney/players?sort=lastname,firstname',
                                     idField='idplayer',
                                     ...
                                     width=200,
                                     pageSize=12)),
    player1Nationality=dict(label=_(u'Nationality'),
                            hint=_('First player nationality.'),
                            hidden=True,
                            readOnly=True),
    player1Sex=dict(label=_(u'Sex'),
                    hint=_(u'First player sex'),
                    hidden=True,
                    readOnly=True),
    ...
    player1LastName=dict(label=_("First player's surname"),
                         ...
                         hidden=True,
                         readOnly=True),
    rate=dict(label=_(u'Rate'),
              hint=_(u'Most recent Glicko rate value of the competitor.'),
              hidden=True,
              readonly=True)
    ))
...
@expose(Match,
        ...
        metadata=dict(
    description=dict(label=_(u'Match'),
                     hint=_(u'Competitor 1 vs Competitor 2.'),
                     sortable=False,
                     readonly=True,
                     flex=1),
...
@expose(Query([Competitor])
        .order_by(_competitors_t.c.prize.desc(),
                  _competitors_t.c.points.desc(),
                  _competitors_t.c.bucholz.desc(),
                  _competitors_t.c.netscore.desc(),
                  _competitors_t.c.totscore.desc()),
        fields=('description,points,bucholz,netscore,prize,totscore,'
                'rate,player1Nationality,idcompetitor').split(','),
        metadata=dict(
    description=dict(label=_(u'Competitor'),
                     hint=_(u'Full name of the players.'),
                     sortable=False,
                     readonly=True,
                     flex=1),
    ...
    rate=dict(label=_('Rate'),
              ...
              readonly=True),
    player1Nationality=dict(label=_(u'Nationality'),
                            hint=_(u'Nationality of the competitor.'),
                            hidden=True,
                            readonly=True)
    ))
def ranking(request, results):
    ...
    if 'metadata' in results:
        results['metadata']['fields'].insert(0, {
            'label': t(_(u'Rank')),
            'hint': t(_(u'Position in the ranking.')),
            'width': 30,
            'readonly': True,
            'name': 'rank',
            'align': 'right',
        ...
        i = 1
        for r in sorted(results['root'], key=itemgetter('rate'), reverse=True):
            r['rank'] = i
            i += 1
    return results
...
def deleteFromTurn(request):
    ...
    try:
        ...
        get_request_logger(request, logger).critical(
            t(_("Couldn't delete turns: $message",
...
def newTurn(request):
    ...
    try:
        ...
        message = str(e)
        get_request_logger(request, logger).critical(
            t(_("Couldn't create next turn: $message",
                mapping=dict(message=message))))
        success = False
    except Exception as e:
        message = str(e)
        get_request_logger(request, logger).exception(
            t(_("Couldn't create next turn!")))
        success = False
    return dict(success=success, message=message,
                currentturn=tourney.currentturn,
                rankedturn=tourney.rankedturn,
...
def updateRanking(request):
    ...
    try:
        ...
        message = str(e)
        get_request_logger(request, logger).critical(
            t(_("Couldn't update the ranking: $message",
                mapping=dict(message=message))))
        success = False
    return dict(success=success, message=message,
                currentturn=tourney.currentturn,
...
def assignPrizes(request):
    ...
    try:
        ...
        message = str(e)
        get_request_logger(request, logger).critical(
            t(_("Couldn't assign prizes: $message",
                mapping=dict(message=message))))
        success = False
    return dict(success=success, message=message,
                currentturn=tourney.currentturn,
...
def resetPrizes(request):
    ...
    try:
        ...
        message = str(e)
        get_request_logger(request, logger).critical(
            t(_("Couldn't reset prizes: $message",
                mapping=dict(message=message))))
        success = False
    return dict(success=success, message=message,
                currentturn=tourney.currentturn,
...
def replayToday(request):
    ...
    try:
        ...
        get_request_logger(request, logger).error(
            t(_("Couldn't duplicate tourney: $message",
                mapping=dict(message=message))))
        success = False
    except Exception as e:
        message = str(e)
...
def clock(request):
    ...
    try:
        ...
        get_request_logger(request, logger).critical(
            t(_("Couldn't show the clock: $message",
