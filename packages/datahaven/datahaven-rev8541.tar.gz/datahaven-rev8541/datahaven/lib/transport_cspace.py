...
_OpenedSendingStreams = []
_OpenedReceivingStreams = []
...
def opened_sending_streams_list():
    global _OpenedSendingStreams
    return _OpenedSendingStreams

def opened_receiving_streams_list():
    global _OpenedReceivingStreams
    return _OpenedReceivingStreams
...
def serviceResponse(command, data):
    ...
    if command == 'sent':
        ...
        try:
            _OpenedSendingStreams, _OpenedReceivingStreams = data 
        except:
            dhnio.Dprint(2, 'transport_cspace.serviceResponse ERROR data=%s' % str(data))
            dhnio.DprintException()
            return
    ...
    def set_receiving_streams(self, num):
        ...
        if server is not None:
            try:
                ...
                dhnio.Dprint(6, 'transport_cspace.set_receiving_streams %s' % str(num))
                return True
            except:
                dhnio.Dprint(6, 'transport_cspace.set_receiving_streams ERROR connecting to XMLRPC')
                RestartService()
        else:
