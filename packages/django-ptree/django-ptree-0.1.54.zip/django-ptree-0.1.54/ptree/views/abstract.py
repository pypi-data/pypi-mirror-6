...
def create_match(view):
    match = view.MatchClass(treatment = view.treatment,
                            experiment = view.experiment)
    # need to save it before you assign the participant.match ForeignKey
    match.save()
    return match

def add_participant_to_match(view):
    view.participant.index_among_participants_in_match = view.match.participant_set.count()
    view.participant.match = view.match

def configure_match(view):
    """
    Find the participant and associate him with an existing or new match.
    """

    if view.participant.match:
        view.match = view.participant.match
    else:
        view.match = view.treatment.next_open_match() or create_match(view)
        add_participant_to_match(view)

    assert view.match
    assert view.match.treatment
