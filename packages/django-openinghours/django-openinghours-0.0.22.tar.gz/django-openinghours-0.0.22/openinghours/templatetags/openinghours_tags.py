...
@register.filter(expects_localtime=True)
def toWeekday(dateObj):

    if dateObj.isoweekday() == datetime.datetime.now().isoweekday() and (dateObj.isoweekday() - datetime.datetime.now().isoweekday()).days == 0:
        return _("today")
    for w in WEEKDAYS:
        if w[0] == int(d):
            return w[1]
...
def getCompanyNextOpeningHour(companySlug, attr=None):
    ...
    if obj is False:
        return False
    if type(attr) == tuple() and attr[1] == "TS":
        return ts
