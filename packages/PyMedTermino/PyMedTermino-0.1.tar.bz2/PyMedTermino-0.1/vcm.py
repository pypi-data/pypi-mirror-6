...
class VCM(pymedtermino.Terminology):
  ...
  def icons_from_lexs(self, lexs, debug = 0):
    ...
    # # Retirer les icônes qui introduisent de nouveaux concepts
    # if cons:
    #   icones_avec_de_nouveaux_concepts = [icone for icone in icones if not _icon_compatible_with_concepts(icone, cons, debug)]
    #   for icone_nc in icones_avec_de_nouveaux_concepts:
    #     icones3 = icones[:]
    #     icones3.remove(icone_nc)
        
    #     icones3_mods = set()
    #     for icone3 in icones3: icones3_mods.update(icone3.modificateurs_de_forme)
    #     tous_les_mods = (modificateurs_etiologies | modificateurs_processus | modificateurs_localisations_secondaires | modificateurs_quantitatifs)
    #     if tous_les_mods.difference(icones3_mods):
    #       if debug:
    #         print "Retrait annulé : cela ferait disparaître les modificateurs de forme", tous_les_mods.difference(icones3_mods)
    #     else:
    #       icones = icones3
    ...
    #   if debug:
    #     print()
    #     print("retirer les icônes qui introduisent de nouveaux concepts :", len(icones))
...
# def _icon_compatible_with_concepts(icone, cons_orig, debug = 0):
#   lexs = icone.lexs
#   cons = icone.concepts
  
#   pictogramme_central = lexs.find(VCM_LEXICON.CENTRAL_PICTOGRAM)
#   cons_du_pictogramme_central = (VCM_LEXICON >> VCM_CONCEPT)(pictogramme_central)
#   cons_du_pictogramme_central = cons_du_pictogramme_central & cons
#   if cons_du_pictogramme_central and (not pictogramme_central is vcm_lexique[u"Aucun_pictogramme_central"]):
#     for con in cons_du_pictogramme_central:
#       for con_orig in cons_orig:
#         if con_orig.est_un(con): break
#       else: continue
#       break
#     else:
#       if debug:
#         print(file = sys.stderr)
#         print("Retrait de l'icône", icone, "car le  picto central", pictogramme_central, "correspond aux concepts", cons_du_pictogramme_central, " dont aucun n'est présent dans les concepts originaux", cons_orig, file = sys.stderr)
#       return 0
    
#   modificateurs = lexs.extract(vcm_lexique[u"Modificateur"])
#   for modificateur in modificateurs:
#     if modificateur is vcm_lexique[u"Modificateur_patho"]: continue
#     cons_du_modificateur = CORRESPONDANCES[vcm_lexique, vcm_concept].traduit(modificateur)
#     cons_du_modificateur = cons_du_modificateur & cons
#     if cons_du_modificateur:
#       for con in cons_du_modificateur:
#         for con_orig in cons_orig:
#           if con_orig.est_un(con): break
#         else: continue
#         break
...
#         if debug:
#           print(file = sys.stderr)
#           print(CORRESPONDANCES[vcm_lexique, vcm_concept].traduit(modificateur), file = sys.stderr)
#           print(cons, file = sys.stderr)
#           print("Retrait de l'icône", icone, "car le  modificateur", modificateur, "correspond aux concepts", cons_du_modificateur, " dont aucun n'est présent dans les concepts originaux", cons_orig, file = sys.stderr)
#         return 0
...
class VCMIcon(pymedtermino.MultiaxialConcept, pymedtermino._StringCodeConcept):
  ...
  #if sys.version[0] == "2":
  #  def __unicode__  (self): return u'%s[u"%s"]' % (self.terminology.name, self.code)
  #  def __repr__     (self): return unicode(self).encode("utf8")
  #else:
  #  def __repr__     (self): return u'%s[u"%s"]' % (self.terminology.name, self.code)
...
# class VCMIcons(pymedtermino.Concepts):
#   def keep_most_graphically_specific(self, add_shadow = 0, on_del = None):
#     clone = self.copy()
#     for t1 in clone:
#       for t2 in clone:
...
#   def keep_most_graphically_generic(self, add_shadow = 0, on_del = None):
#     clone  = self.copy()
#     clone2 = self.copy()
#     for t1 in clone:
#       for t2 in clone2:
...
def generalize_icons(icons, on_del = None, fail_if_too_much_information_is_lost = 0):
  ...
  top_right_colors = pymedtermino.Concepts(icon.top_right_color for icon in icons)
  if len(top_right_colors) == 1: top_right_color = tuple(top_right_colors)[0]
  elif top_right_colors == pymedtermino.Concepts([VCM_LEXICON[3, u"traitement"  ], VCM_LEXICON[3, u"antecedent_traitement"]]):
    top_right_color = VCM_LEXICON[3, u"traitement"  ]
  elif top_right_colors == pymedtermino.Concepts([VCM_LEXICON[3, u"surveillance"], VCM_LEXICON[3, u"antecedent_surveillance"]]):
    top_right_color = VCM_LEXICON[3, u"surveillance"]
  top_right_color = VCM_LEXICON[3, u"empty"] # Conflit => on généralise en supprimant l'exposant
...
def combine(icons, on_del = None):
  if len(icons) == 1: return icons[0]
  r = generalize(icons)
  if on_del:
    for icon in icons: on_del(r, icon)
  return [r]
  
def garder_les_plus_precises(icons, on_del = None):
  icons  = list(set(icons))
  icons2 = icons[:]
  for iconA in icons2:
    for iconB in icons2:
      if (not iconA is iconB) and iconA.est_un(iconB) and (iconB in icons):
        if on_del: on_del(iconA, iconB)
        icons.remove(iconB)
  return icons

def garder_les_plus_generales(icons, on_del = None):
  icons  = icons[:]
  for i in range(len(icons)):
    iconA = icons[i]
    if iconA:
      for j in range(len(icons)):
        if i != j:
          iconB = icons[j]
          if iconB and iconA and iconB.est_un(iconA):
            if (iconA.ombre == Ombre.CODES[u"ombre"]) or (not iconA in icons):
              if on_del: on_del(iconA, iconB)
              icons[j] = None
            else:
              iconA_avec_ombre = iconA.modifie(ombre = Ombre.CODES[u"ombre"])
              if on_del:
                on_del(iconA_avec_ombre, iconA)
                on_del(iconA_avec_ombre, iconB)
              icons[i] = iconA_avec_ombre
...
def supprimer_doublons_sans_ombre(icons, on_del = None):
  icons  = list(set(icons))
  icons2 = icons[:]
  ombre_pours = set()
  ...
  for iconA in icons2:
    for iconB in icons2:
      if not iconA is iconB:
        if (
          (iconA.central_color               == iconB.central_color              ) and
          (iconA.pictogramme_central            == iconB.pictogramme_central           ) and
          (iconA.modificateurs_de_forme         == iconB.modificateurs_de_forme        ) and
          (iconA.top_right_color            == iconB.top_right_color           ) and
          (iconA.top_right_pictogram        == iconB.top_right_pictogram       ) and
          (iconA.pictogramme_en_second_exposant == iconB.pictogramme_en_second_exposant) and
         ((iconA.ombre == iconB.ombre) or (iconA.ombre == Ombre.CODES[u"ombre"]))
          ):
          if iconB in icons:
            if on_del: on_del(iconA, iconB)
            icons.remove(iconB)
...
def supprimer_doublons(icons, on_del = None):
  icons  = icons[:]
      ...
      if (i != j):
        ...
        if iconB and iconA and (
         ...
         ((iconA.ombre == iconB.ombre) or (iconA.ombre == Ombre.CODES[u"ombre"]))
          ...
          if iconA.ombre == Ombre.CODES[u"ombre"]: # Déjà fait
            if on_del: on_del(iconA, iconB)
            icons[j] = None
          else:
            iconA_avec_ombre = iconA.modifie(ombre = Ombre.CODES[u"ombre"])
...
_priorite_central_color = {
  CouleurCentrale.CODES[u"antecedent"] : 1,
  CouleurCentrale.CODES[u"en_cours"  ] : 2,
  CouleurCentrale.CODES[u"risque"    ] : 3,
  }
_priorite_top_right_color = {
  CouleurEnExposant.CODES[u"antecedent_traitement"  ] : 1,
  CouleurEnExposant.CODES[u"traitement"             ] : 2,
  CouleurEnExposant.CODES[u"antecedent_surveillance"] : 3,
  CouleurEnExposant.CODES[u"surveillance"           ] : 4,
  }
def trier(icons, on_del = None):
  def icon_cmp(a, b):
    return (
      cmp(_priorite_central_color   .get(a.central_color   , 999), _priorite_central_color   .get(b.central_color   , 999)) or
      cmp(_priorite_top_right_color.get(a.top_right_color, 999), _priorite_top_right_color.get(b.top_right_color, 999)) or
      cmp(a.pictogramme_central.code, b.pictogramme_central.code)
      )
  icons = list(icons)
  icons.sort(icon_cmp)
  return icons
