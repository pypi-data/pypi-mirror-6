...
def get_files(files):
    ...
    :param files: If a falsy value, returns an empty list.
                  ...
                  If a list of strings, each string is ran through glob.glob (or formic) and
                  returned as a list containing all of the unique paths returned from glob.glob.
                  If list of dicts, each dict is ran through _get_files (or formic) and
                  returns the list of unique paths retrieved from all the calls.
                  If dict is ran through _get_files, or formic if no 'path' key exists
    ...
    if isinstance(files, list):
        if isinstance(files[0], basestring):
            ...
            files = [list(FileSet(f)) if '**' in f else glob.glob(f) for f in files]
            ...
            files = [_get_files(**f) if 'path' in f else list(FileSet(**f)) for f in files]
            return list(set(itertools.chain(*files)))

    if isinstance(files, dict):
        return _get_files(**files) if 'path' in files else list(FileSet(**files))
