...
def minify_js(args, scripts):
    ...
    with open(concat_fname, 'w') as concat:
...
def minify_css(args, styles, map):
    ...
    if args.debug:
        ...
        concat_output = open(concat_fname, 'w')
...
def absolutize(paths, map):
    """Transform each partial path into its absolute full path.

    :param paths: a list of partial paths
    :param map: a dictionary, where each key is a possible prefix and its value
...
def main():
    ...
    with open(args.manifest) as f:
        sources = absolutize(load(f), prefix_map)

    scripts = [script for script in sources if script.endswith('.js')]
    styles = [script for script in sources if script.endswith('.css')]
