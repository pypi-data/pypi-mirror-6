...
def Experiments(f,samples,params,paramUpdate,action):
    ...
    def iterF(f,samples,params,paramUpdate,action):
        sols = []
#        LOG_FILE_BASE = "MonteCarlo"
#        i = 0
#        while ( os.path.isfile("%s-Run%d-Proc%d.log" % (LOG_FILE_BASE,i,myrank)) ):
#            i = i + 1
#        LOG_FILE = "%s-Run%d-Proc%d.log" % (LOG_FILE_BASE,i,myrank)
        for i in xrange(0,samples.shape[0]):
            params = paramUpdate(params,samples[i])
            sol = f(params)
            print "Proc %d run %d/%d" % (myrank,i+1,len(samples))
#            log = open(LOG_FILE,'a')
#            log.write("Proc %d run %d/%d\n" % (myrank,i+1,len(samples)))
#            log.close()
            #sols.append( sol )
