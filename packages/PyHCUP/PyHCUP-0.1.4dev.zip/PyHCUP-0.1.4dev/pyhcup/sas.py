...
def meta_from_sas(target):
    ...
    description_patterns = ['((?P<field>\S+)\s+LENGTH=\$?(?P<length>\d+)\s*(FORMAT=(?P<format>\S*))?\s*LABEL=\s*"(?P<label>[^"]+)")',
                            '(?P<field>\S+)\s*=\'(?P<label>[^\']*)\'',
                           ]
    ...
    for description_pattern_candidate in description_patterns:
        if description_regex is None:
            ...
            if re.search(description_regex_candidate, concatenated):
                print 'Used pattern %s' % description_pattern_candidate
    ...
    if 'length' in v_df.columns:
        sans_length = v_df.drop('length', axis=1)
    else:
        sans_length = v_df
    meta_df = i_df.merge(sans_length, on='field',
