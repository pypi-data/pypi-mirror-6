...
class ColorfulParser:
    ...
    def foreground_color_exists(cls, color):
        return hasattr(cls._forecolors, color)

    @classmethod
    def background_color_exists(cls, color):
        return hasattr(cls._backcolors, color)
...
class ColorfulMeta(type):
    class _ColorfulOut(type):
        def __getattr__(cls, attr):
            def _decoratedText(text):
                print(ColorfulParser.encode(ColorfulParser.parse_attr(attr) % text))
                stdout.flush()
            return _decoratedText
    ...
    def has_forecolor(self, color):
        return ColorfulParser.foreground_color_exists(color)

    def has_backcolor(self, color):
        return ColorfulParser.background_color_exists(color)

    def exists(self, attr):
        try:
            ColorfulParser.parse_attr(attr)
            return True
        except ANSISequenceNotFoundError:
            return False

    def get(self, attr):
        def _decoratedText(text):
            return ColorfulParser.encode(ColorfulParser.parse_attr(attr) % text)
        return _decoratedText

colorful = ColorfulMeta("colorful", (object, ), {})  # use metaclass ColorfulMeta for Colorful class
