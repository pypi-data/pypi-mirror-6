...
def add_apt_repository(repository):
    ...
    print('sudo privileges are required for PPA installation')
...
def start_ssh_agent():
    ...
    retcode, output, error = call('/usr/bin/ssh-agent')
    if retcode != 0:
        raise OSError(error.encode('utf-8'))
    ...
    print('ssh-agent has been started.\n\n'
          ...
          'finishes, please\nrun the following in a terminal to start '
          'ssh-agent:\n\n  eval `ssh-agent`\n')
...
def convert_bundle_url(bundle_url):
    ...
    """
    bundle_id = bundle_url.split(':', 1)[1].rstrip('/')
    if not bundle_id:
...
def get_juju_version():
    ...
    Return a (major:int, minor:int, patch:int) tuple, including
