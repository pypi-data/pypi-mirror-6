...
class Task():
    ...
    def run(self):
        sourcePath = backup_fs.ToPath(self.pathID)
        if sourcePath is None:
            ...
            return 
        itemInfo = backup_fs.GetByID(self.pathID)
        if itemInfo is None:
            dhnio.Dprint(4, 'backup_control.Task.run ERROR can not get info for %s in the index' % self.pathID)
            # self.defer.callback('error', self.pathID)
            return
        ...
        if itemInfo.type == backup_fs.PARENT:
            dirsize.ask(sourcePath)
        # self.defer.callback('started', backupID)
        reactor.callLater(0, FireTaskStartedCallbacks, self.pathID, dataID)
        dhnio.Dprint(4, 'backup_control.Task.run %s [%s]' % (self.pathID, dataID))
