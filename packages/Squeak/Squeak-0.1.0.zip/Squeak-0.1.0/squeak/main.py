...
t_test_effect_size(t, df):
	"""Gives Cohen's d, and Pearson's r, given t value and df from t test
	
		Parameters
	----------
	t, df : int, float
		t and degress of freedom from t test.
	...
	Not strictly speaking part of Squeak, but useful anyway"""
	r = np.sqrt( (t**2) / ( (t2**2) +(df) ) )
	d = (t*2) / ( np.sqrt(df)
	return (r, d)
