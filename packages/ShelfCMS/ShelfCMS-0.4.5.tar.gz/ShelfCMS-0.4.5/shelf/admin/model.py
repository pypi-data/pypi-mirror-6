...
class LocalizedString(db.Model):
    ...
    trad = db.relationship('LocalizedString', innerjoin=True, lazy="joined", backref=db.backref("parent", remote_side=[id]))
...
class LocalizedText(db.Model):
    ...
    trad = db.relationship('LocalizedText', innerjoin=True, lazy="joined", backref=db.backref("parent", remote_side=[id]))
...
class Page(db.Model):
    ...
    title = db.relationship('LocalizedString', innerjoin=True, lazy="joined", foreign_keys=(title_id,))
    description_id = db.Column(db.Integer, db.ForeignKey('localized_string.id'))
    description = db.relationship('LocalizedString', innerjoin=True, lazy="joined", foreign_keys=(description_id,))
