...
def actionking(att,lck, tempf, stocklist, dbase, attmt,actionlist,cycle,timeatpresent,timetotake,sqlstatements):
    ...
    if (time.clock() - timeatpresent) > timetotake:
      with lck:
        thread.start_newthread(goldsaxpersist.push,(lck,dbase,sqlstatements))
        return 0
    ...
    gc.collect()
    print(len(stocklist),"bytes sent, ",len(f),"bytes recieved")
    if sqlstatements!="":
      with lck:
        thread.start_newthread(goldsaxpersist.push,(lck,dbase,sqlstatements))
        print(len(sorter)," stocks changed and pushed to persistance")
      sqlstatements=""
    cycle = cycle + 1
    sorter=""
