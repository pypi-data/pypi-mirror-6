...
class JSButtonAction(action.Action, JSButtonWidget,
    ...
    def dataLoading(self):
        return self.field.getDataLoading(self, self.request)
...
class TriggerMixin(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'trigger' in kwargs:
            ...
            if self.trigger is True:
                # use default trigger event type
    ...
    def getDataLoading(self, action, request):
        if self.loading:
            return zope.i18n.translate(self.loading, context=request)
        else:
            return None
