...
def kamada_kawai_layout(graph, side=10.00, threshold=0.001, K=2.0):
    ...
    lengths = copy.deepcopy(graph.paths)
    for v1 in graph.vertices:
        for v2 in graph.vertices:
            lengths[v1][v2] *= L
    ...
    partial_derivs = [_partial_derivatives(v, vertices, strengths, lengths)
                      for v in vertices]
    ...
    while max_dm > threshold:
        while max_dm > threshold:
            ...
            dx, dy = _compute_deltas(
                v, vertices, strengths, lengths, partial_derivs)
            v.x += dx
            v.y += dy

            partial_derivs[worst_v_index] = _partial_derivatives(
                v, vertices, strengths, lengths)
...
def plot_graph(graph, layout_algorithm=kamada_kawai_layout, side=10.00, **kwargs):
    ...
    **kwargs: the rest of the keyword parameters for a layout_algorithm.
    ...
    if len(components) == 1:
        return layout_algorithm(graph, **kwargs)
    vertices = []
    x_start = 0
