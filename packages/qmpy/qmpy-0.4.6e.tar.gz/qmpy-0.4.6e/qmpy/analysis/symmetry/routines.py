...
if not qmpy.FOUND_SPGLIB:
    print 'Must install spglib to be able to do symmetry analysis'
    exit(-2)
...
zdict = dict( (e['z'], e['symbol']) for e in data.elements.values() )
...
def find_structure_symmetry(structure, method='spglib',
    ...
    if not any(magmoms):
        num_sym = spg.symmetry(rotation,
                               ...
                               structure.coords.copy(),
                               np.array(structure.atomic_numbers, dtype='intc'),
                               symprec,
                               angle_tolerance)
    else:
        num_sym = spg.symmetry_with_collinear_spin(rotation,
                                                   ...
                                                   lattice,
                                                   positions,
                                                   np.array(structure.atomic_numbers,
                                                       dtype='intc'),
                                                   magmoms,
                                                   symprec,
                                                   angle_tolerance)
...
def refine_cell(structure, symprec=1e-5, angle_tolerance=-1.0):
    ...
    positions[:num_atom] = structure.coords.copy()
...
def find_primitive(structure, symprec=1e-5, angle_tolerance=-1.0):
    ...
    """
    lattice = structure.cell.T.copy()
    positions = structure.coords.copy()
    numbers = np.array(structure.atomic_numbers, dtype='intc')
