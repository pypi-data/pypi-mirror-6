...
class InputEvents(object):
  ...
  def __init__(self, keyboardHandler=None, mouseHandler=None, joystickHandler=None, synHandler=None, unhandledHandler=None, wantKeyboard=True, wantMouse=True, wantJoystick=True):
    ...
    if USE_X11:
      ...
      di = self.DISPLAY.opengl
      a, b, c = self.X_mouse_position() 
      wantMouse = 0
    ###################################################################
    if wantKeyboard:
  ...
  ###################################################################
  def X_mouse_position(self):
    di = self.DISPLAY.opengl
    aa, ab =  Window(), Window()
    ac, ad, ae, af = c_int(), c_int(), c_int(), c_int()
    ag = c_uint()
    pp = xlib.XQueryPointer(di.d, di.window, aa, ab, ac, ad, ae, af, ag)
    return (ae.value, af.value, ag.value)
  ###################################################################
