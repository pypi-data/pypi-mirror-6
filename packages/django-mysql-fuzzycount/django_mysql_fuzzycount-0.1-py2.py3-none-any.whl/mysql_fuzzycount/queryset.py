...
class LimitedCountQuerySet(QuerySet):
    _count_limit = None
    ...
    def count(self):
        if self._count_limit is None:
            return super(LimitedCountQuerySet, self).count()

        sql = 'SELECT COUNT(*) FROM (%s) as l' % (
            self.only('pk').order_by()[:self._count_limit].query)
        return list(self.model.objects.raw(sql).query)[0][0]
