...
class theTVDB(object):
  ...
  def __init__(self, series_name, api_key, language=None):
    ...
    self.series_name = str(series_name.encode('utf-8'))
    if language is None:
  ...
  def set_language(self, language):
    ...
    if language in AVAILABLE_LANGUAGES:
      ...
      raise theTVDBError('This language is not supported by theTVDB. Available languages are: '+str(self.get_languages()).strip('[]')+'.')
  ...
  def set_series(self, language=None):
    ...
    if language == ALL_LANGUAGES:
      self.set_simple_series()
    else:
      self.set_full_series(language)
  
  def set_simple_series(self):
    """
    ...
    theTVDBError if something went wrong
    """
    series_name = urllib2.quote(self.series_name)
    ...
    self.series_id = series_infos['seriesid']
  ...
  def set_full_series(self, language):
    """
    Set the series info when no language has been specified
    ...
    series_name = urllib2.quote(self.series_name)
    url = ROOT_URL+'{tvdb_api_key}/series/{series_id}/{series_language}.xml'.format(tvdb_api_key=self.api_key, series_id=self.series_id, series_language=language)
    
    #Get the TVDB xml file
    try:
      logger.debug('Trying to open the url {url}...'.format(url=url))
      series_tree = get_xml_tree(url).find('Series')
    except theTVDBAPIConnectionError as error:
      raise theTVDBAPIError('Could not connect to theTVDB. {error_text}'.format(error_text=error))
      return
    except theTVDBAPIXMLError as error:
      raise theTVDBAPIError('There was an error with the XML retrieved from thetvdb.com. {error_text}'.format(error_text=error))
      return
    
    if series_tree is None:
      raise theTVDBAPIError('This series does not exists at thetvdb.com.')
    ...
    self.series_infos.update(series_infos)
  ...
  def get_episode(self, episode_season, episode_number, language=None):
    ...
    url = ROOT_URL+'{tvdb_api_key}/series/{series_id}/default/{episode_season}/{episode_number}/{series_language}.xml'.format(tvdb_api_key=self.api_key, series_id=self.series_id, episode_season=episode_season, episode_number=episode_number, series_language=language)
  ...
  def get_banners(self):
    ...
    theTVDBError if something went wrong
    """
    url = ROOT_URL+'{tvdb_api_key}/series/{series_id}/banners.xml'.format(tvdb_api_key=self.api_key, series_id=self.series_id)
  ...
  def get_actors(self):
    ...
    theTVDBError if something went wrong
    """
    url = ROOT_URL+'{tvdb_api_key}/series/{series_id}/actors.xml'.format(tvdb_api_key=self.api_key, series_id=self.series_id)
