...
class ConnectionBuilder:
  ...
  def __init__(self, api_key, log_group_name, log_name, endpoint=None):
    ...
    self.api_key = u(api_key).encode('utf-8')
    self.log_group_name = u(log_group_name).encode('utf-8')
    self.log_name = u(log_name).encode('utf-8')
    self.protocol,\
        self.host,\
        self.port,\
...
class SlickLogRemoteLogger:
  ...
  def log(self, message):
    ...
    try:
      ...
      log('message put int queue: {0}', bytes)
