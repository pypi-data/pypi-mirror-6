...
def get_adaptor_for_type(type):
    """Create an adaptor for the given type.

    :param type: an SQLAlchemy ``TypeEngine``
    :rtype: a function
    ...
    elif isinstance(type, (Date, DateTime)):
        def coerce_date(s):
            ...
            if isinstance(s, basestring):
                ...
                res = s
            if isinstance(type, Date) and isinstance(s, datetime):
                res = s.date()
            return res
...
def compare_field(field, value):
    ...
    *sequence* (i.e. ``list`` or ``tuple``), the expression will use
    ...
    try:
        coerce_value = get_adaptor_for_type(field.type)
    ...
    if only_cols:
        ...
        if not isinstance(query, Query):
            cols = [col for col in [col_by_name(query, c) for c in only_cols]
                    if col is not None]
            if not cols:
                assert False, "No valid column in only_cols='%s'" % only_cols
...
def apply_sorters(query, sort, dir):
    ...
    if isinstance(sort, basestring):
        if sort.startswith('['):
            ...
            sort = [{'property': sort, 'direction': dir}]
