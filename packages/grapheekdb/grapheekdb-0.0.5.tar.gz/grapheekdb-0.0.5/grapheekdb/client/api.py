...
class ProxyEntityIterator(ProxyIteratorCommon):
    ...
    def agg(self, agg='+', asc=False):
        command = ('agg', [], dict(agg=agg, asc=asc))
...
class ProxyGraph(GraphReadWrite):
    ...
    def add_node_index(self, *fields):
        return self._add_entity_index(KIND_VERTEX, *fields)

    def add_edge_index(self, *fields):
        return self._add_entity_index(KIND_EDGE, *fields)
