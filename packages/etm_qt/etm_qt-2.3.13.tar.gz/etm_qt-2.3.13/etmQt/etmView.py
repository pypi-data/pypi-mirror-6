...
from etmData import has_sip, has_qt
if not has_sip or not has_qt:
    print("""
Both sip and Qt are required for the etm-qt GUI but could not be imported.
""")
    sys.exit()
...
from etmQt import etm_rc  # THIS IS NECESSARY NO MATTER WHAT PEP8 SAYS!!!
    ...
    def writeLinesToFile(self, filename, lines, senddone=True):
        ...
        if 'hg_commit' in self.parent().options:
            command = self.parent().options['hg_commit'].format(
                repo=self.parent().options['datadir'],
                mesg="{0}: {1}".format(self.mode, filename))
            process = QProcess()
