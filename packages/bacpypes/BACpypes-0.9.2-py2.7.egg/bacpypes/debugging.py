...
#               file.write("%s%s = 0x%X\n" % ('    ' * indent, attr, value))
    ...
    def dict_contents(self, use_dict=None, as_class=dict):
        ...
        # loop through the attributes
        for attr in attrs:
            # rip off the common suffix patterns
            if attr.endswith("-"):
                attr = attr[:-1]
            elif attr.endswith("*"):
                attr = attr[:-1]
            ...
            elif isinstance(value, types.DictType) and value:
                mapped_value = as_class()
                for key, elem in value.items():
                    if hasattr(elem, 'dict_contents'):
                        mapped_value.__setitem__(key, elem.dict_contents(as_class=as_class))
                    else:
                        mapped_value.__setitem__(key, elem)
        ...
        ownFn.reverse()
        for klass in ownFn:
            klass.dict_contents(self, use_dict=use_dict, as_class=as_class)
