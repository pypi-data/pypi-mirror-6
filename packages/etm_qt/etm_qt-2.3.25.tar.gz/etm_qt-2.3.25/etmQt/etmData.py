...
def get_week(dt):
    ...
    theweek = "{0} {1}: {2}".format(tr('Week'), wn, header)
    return theweek
...
if qt_version == 5:
    ...
    def d_to_str(qlcl, d, s):
        ret = str(qlcl.toString(QDate(d), s))
        if 'w' in ret:
            week = get_week(d)
            # week = tr("Week {0}").format(d.strftime("%W"))
            ret = ret.replace('w', week)
        return ret
    ...
    def d_to_str(qlcl, d, s):
        if python_version == 2:
            ret = unicode(qlcl.toString(QDate(d), s).toUtf8().data(), term_encoding)
        else:
            ret = qlcl.toString(QDate(d), s)
        if 'w' in ret:
            week = get_week(d)
            # week = tr("Week {0}").format(d.strftime("%W"))
            ret = ret.replace('w', week)
    ...
    def dt_to_str(qlcl, dt, s):
        if python_version == 2:
            return(unicode(qlcl.toString(QDateTime(dt), s).toUtf8().data(), term_encoding))
        else:
            return(qlcl.toString(QDateTime(dt), s))
...
trailingzeros = re.compile(r'(:00)(?=[aApP])')
...
def get_options(etmdir=''):
    ...
    default_ampm = ('AP' in QLocale.timeFormat(qlcl))
    ...
    # set the global encoding values
...
def fmt_time(dt, omitMidnight=False, options={}):
    ...
    if 'ampm' in options and options['ampm']:
        ...
        # drop trailing :00 if followed by a, A, p or P
...
class ETMCmd(cmd.Cmd):
    ...
    """
    def __init__(self, options={}, loop=False):
...
""")
    ...
    def do_t(self, arg):
        ...
        if self.number:
            term_print("item numbers on")
        else:
            term_print("item numbers off")
...
def main(etmdir='', argv=[]):
    ...
    if len(argv) > 1:
        if argv[1] in ['a', 'c', 's', 'm', 'n', 'v', '?']:
            ...
            c = ETMCmd(options=options, loop=True)
            # c.loop = True
            c.cmdloop()
