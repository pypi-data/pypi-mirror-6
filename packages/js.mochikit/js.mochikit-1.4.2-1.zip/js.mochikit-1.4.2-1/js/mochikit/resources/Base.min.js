...
MochiKit.Base.VERSION="1.4.2";MochiKit.Base.NAME="MochiKit.Base";MochiKit.Base.update=function(self,obj){if(self===null||self===undefined){self={};}
...
return res;},extend:function(self,obj,skip){
 if(!skip){skip=0;}
if(obj){
var l=obj.length;if(typeof(l)!='number'){if(typeof(MochiKit.Iter)!="undefined"){obj=MochiKit.Iter.list(obj);l=obj.length;}else{throw new TypeError("Argument not an array-like and MochiKit.Iter not present");}}
if(!self){self=[];}
...
var im_func=func.im_func;var im_preargs=func.im_preargs;var im_self=func.im_self;var m=MochiKit.Base;if(typeof(func)=="function"&&typeof(func.apply)=="undefined"){
...
return me.im_func.apply(self,args);};newfunc.im_self=im_self;newfunc.im_func=im_func;newfunc.im_preargs=im_preargs;return newfunc;},bindLate:function(func,self){var m=MochiKit.Base;if(typeof(func)!="string"){return m.bind.apply(this,arguments);}
var im_preargs=m.extend([],arguments,2);var newfunc=function(){var args=arguments;var me=arguments.callee;if(me.im_preargs.length>0){args=m.concat(me.im_preargs,args);}
var self=me.im_self;if(!self){self=this;}
return self[me.im_func].apply(self,args);};newfunc.im_self=self;newfunc.im_func=func;newfunc.im_preargs=im_preargs;return newfunc;},bindMethods:function(self){var bind=MochiKit.Base.bind;for(var k in self){var func=self[k];if(typeof(func)=='function'){self[k]=bind(func,self);}}},registerComparator:function(name,check,comparator,override){MochiKit.Base.comparatorRegistry.register(name,check,comparator,override);},_primitives:{'boolean':true,'string':true,'number':true},compare:function(a,b){if(a==b){return 0;}
...
return s;},serializeJSON:function(o){var objtype=typeof(o);if(objtype=="number"||objtype=="boolean"){return o+"";}else if(o===null){return"null";}else if(objtype=="string"){var res="";for(var i=0;i<o.length;i++){var c=o.charAt(i);if(c=='\"'){res+='\\"';}else if(c=='\\'){res+='\\\\';}else if(c=='\b'){res+='\\b';}else if(c=='\f'){res+='\\f';}else if(c=='\n'){res+='\\n';}else if(c=='\r'){res+='\\r';}else if(c=='\t'){res+='\\t';}else if(o.charCodeAt(i)<=0x1F){var hex=o.charCodeAt(i).toString(16);if(hex.length<2){hex='0'+hex;}
res+='\\u00'+hex.toUpperCase();}else{res+=c;}}
return'"'+res+'"';} 
var me=arguments.callee;
var newObj;if(typeof(o.__json__)=="function"){newObj=o.__json__();if(o!==newObj){return me(newObj);}}
...
var compareKeys=m.extend(null,arguments);return function(a,b){var rval=0;
 for(var i=0;(rval===0)&&(i<compareKeys.length);i++){var key=compareKeys[i];rval=compare(a[key],b[key]);}
...
for(var name in namespace){var o=namespace[name];if(typeof(o)=='function'&&typeof(o.NAME)=='undefined'){try{o.NAME=base+name;}catch(e){}}}},queryString:function(names,values){

if(typeof(MochiKit.DOM)!="undefined"&&arguments.length==1&&(typeof(names)=="string"||(typeof(names.nodeType)!="undefined"&&names.nodeType>0))){var kv=MochiKit.DOM.formContents(names);names=kv[0];values=kv[1];}else if(arguments.length==1){ if(typeof(names.length)=="number"&&names.length==2){return arguments.callee(names[0],names[1]);}
...
if(MochiKit.__export__){compare=MochiKit.Base.compare;compose=MochiKit.Base.compose;serializeJSON=MochiKit.Base.serializeJSON;mean=MochiKit.Base.mean;median=MochiKit.Base.median;}
