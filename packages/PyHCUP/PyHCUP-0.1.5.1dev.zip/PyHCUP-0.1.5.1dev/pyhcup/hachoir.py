...
def discover(root_path, sources=None, nixfs=True):
    ...
    sources should be a list containing dictionaries that specify str name and list patterns. Each value in patterns must be a regular expression representing a file naming convention.
    ...
    Here is an example of a source dictionary.
        ...
        'name': 'HCUP SID CORE',
        'patterns': [
            '(?P<state_abbr>[A-Z]{2})_(SID|SIDC)_(?P<year>[0-9]{4})_(?:CORE)\.(?P<file_extension>(asc|zip|exe|sas))',
            ]
        }
    ...
    if sources is None:
        ...
        sources = [
            {
            'name': 'HCUP SID CORE',
            'description': 'Healthcare Cost and Utilization Project State Inpatient Database CORE dataset',
            ...
            'patterns': [
                '(?P<state_abbr>[A-Z]{2})_(SID|SIDC)_(?P<year>[0-9]{4})_(?:CORE)\.(?P<file_extension>asc)',
                ]
            },
            {
            'name': 'HCUP SID CORE',
            ...
            'patterns': [
                '(?P<state_abbr>[A-Z]{2})_(SID|SIDC)_(?P<year>[0-9]{4})_(?:CORE)\.(?P<file_extension>(exe|zip))',
                ]
            },
            {
            'name': 'HCUP SID CORE',
            ...
            'patterns': [
                '(?P<state_abbr>[A-Z]{2})_(SID|SIDC)_(?P<year>[0-9]{4})_(?:CORE)\.(?P<file_extension>sas)',
                ],
            },
            ]
