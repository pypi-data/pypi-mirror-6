...
class Report(object):
    ...
    def stop(self):
        ...
        print("Skipping redundant matches...", file=sys.stderr)
        ...
        print("... %d redundant matches dropped" % skipped_count, file=sys.stderr)
...
""")
    ...
    def item(self, book1, book2, observations, merge_safe):
        ...
        print(("""<tr class="%s first"><td>%s</td><td>%s</td><td>%s</td></tr>""" % (
            (cls,) + rows[0])).encode("utf-8"))
        for i in range(1, len(rows)-1):
            print(("""<tr class="%s internal"><td>%s</td><td>%s</td><td>%s</td></tr>""" % (
                (cls,) + rows[i])).encode("utf-8"))
        print(("""<tr class="%s last"><td>%s</td><td>%s</td><td>%s</td></tr>""" % (
...
def run():
    ...
    print("Loading books info...", file=sys.stderr)
    ...
    print("... book list loaded", file=sys.stderr)

    likely_identical = set()  # pary id

    print("Looking for identical files...", file=sys.stderr)
    ...
    print("... analyzed, %d possible pairs found" % identical_count, file=sys.stderr)

    print("Looking for matching authors and titles...", file=sys.stderr)

    all_authors = books_by_author.keys()

    for author, books in six.iteritems(books_by_author):
    ...
    print("... analyzed, %d new pairs found" % (
        len(likely_identical) - identical_count), file=sys.stderr)
