...
from glams.website.database.forms import getMouseForm, getCageForm, getInnerCageForm, getGeneticFilterForm, getResident, getStrainList, getLabMemberList, getResidentsFilterForm
...
def makequery(viewtype,c,sortby):
    ...
    if viewtype=='mouse':
        ...
        w.append("(housing.currentcage=1 OR (housing.currentcage=0 AND housing.start_date= (SELECT max(h.start_date) FROM housing AS h WHERE h.mouse_id=m.id)) OR housing.currentcage IS NULL)")
        if len(w)>0:
