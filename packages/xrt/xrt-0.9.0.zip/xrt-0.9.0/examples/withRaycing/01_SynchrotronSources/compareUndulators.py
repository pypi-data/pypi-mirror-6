...
#    fig.subplots_adjust(right=0.88)
    ...
    plotOwn(ax, '1u_urgent3-n-monoE-.pickle', '-m', 'ray-traced, xrt with Urgent')
#    plotOwn(ax, '1u_xrt3-n-monoE-BW.pickle', '.b', 'ray-traced, xrt, distE=BW')
    ...
    E, fl = np.loadtxt("urgentOpt.out", skiprows=32, usecols=(0,2), unpack=True,
        converters={2: lambda s: float(s.translate(transFortranD))},
        comments=" TOTAL")
    ax.plot(E*1e-3, fl, '-y', lw=2, label='XOP Urgent, icalc=1, mode4')
#    E, fl = np.loadtxt("urgent_icalc2.out", skiprows=32, usecols=(0,2), unpack=True,
#        converters={2: lambda s: float(s.translate(transFortranD))},
#        comments=" TOTAL")
#    ax.plot(E*1e-3, fl, '-g', lw=2, label='XOP Urgent, icalc=2, mode4')

#    E, fl = np.loadtxt("us-mode1.out", skiprows=23, usecols=(0,1), unpack=True)
#    ax.plot(E*1e-3, fl, '-g', lw=2, label='XOP XUS, mode1')
    E, fl = np.loadtxt("us-mode3.out", skiprows=23, usecols=(0,1), unpack=True)
    ax.plot(E*1e-3, fl, '-c', lw=2, label='XOP US, mode3')

    E, fl = np.loadtxt("Spectra_FluxZeroESpread.dc0", skiprows=10, usecols=(0,1), unpack=True)
    ax.plot(E*1e-3, fl, '--r', lw=2, label='Spectra, no E spread')
    E, fl = np.loadtxt("Spectra_Flux.dc0", skiprows=10, usecols=(0,1), unpack=True)
