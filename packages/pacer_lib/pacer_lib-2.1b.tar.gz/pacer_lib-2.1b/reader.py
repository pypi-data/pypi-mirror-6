...
class docket_processor():
    ...
    * ``output_path`` points to the folder where you would like output to be
    ...
    def write_all_matches(self):
        """
        Writes all of the matches found in the *self.hit_list* dictionary to 
        a single .csv file (**all_match.csv**) in the *self.output_path*. 
        ...
        (For version 2.1) We should allow for suffixing and also have an
        overwrite option that defaults to False to prevent accidental overwrites.

        Returns nothing.
        """
        with open(self.output_path + '/all_match.csv', 'w') as f:
    ...
    def write_individual_matches(self, suffix):
        """
        Writes all of the matches in the *self.hit_list* dictionary to one .csv
        file per docket sheet (determined by the source .csv). To distinguish
        ...
        overwrite previous searches. 

        (For version 2.1) We should have an overwrite option that defaults to 
        False to prevent accidental overwrites. If someones wants to overwrite
        a search, we also want to delete all files with that suffix.

        Returns nothing.
        """
        #Writes matches a new .csv file for each document within the search directory
        for key in self.hit_list.keys():
            with open(self.output_path + '/' + '^' + key + '_' + suffix +'.csv', 'w') as f:
                 writer = csv.writer(f, dialect='excel')
