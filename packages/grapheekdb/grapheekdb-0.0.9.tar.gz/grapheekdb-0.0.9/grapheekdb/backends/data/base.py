...
class EntityAggregate(object):
    ...
    def _get_results(self):
        ...
        assert(agg_type in ['+', '%'])
        field_path = self._agg[1:]
        try:
            alias, field_name = field_path.split('.')
        except:
            alias = None
        ...
        for entity_id in self._entity_iterator._iterate(on_item=False):
            if field_name == '':
                ...
                if alias is None:
                    obj = entity = entity_cache.get(entity_id, self._graph._item_from_id(self._entity_kind, entity_id))
                    entity_cache[entity_id] = entity
                else:
                    obj = self._entity_iterator._context[alias]
