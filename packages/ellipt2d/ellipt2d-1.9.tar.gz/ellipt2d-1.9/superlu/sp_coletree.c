...
static int	*first_next_kid_stack_v, *first_next_kid_stack_w, first_next_kid_stack_pos;	/* stack to change recursive nature of etdfs function (was exceeding max recursion limit)	*/
...
{
	int	w;
	first_next_kid_stack_pos=-1;
	w = first_kid[v];
	do{
		while( w != -1 ) {
			//etdfs (w);
			first_next_kid_stack_pos++;
			first_next_kid_stack_v[first_next_kid_stack_pos] = v;
			first_next_kid_stack_w[first_next_kid_stack_pos] = next_kid[w];
			v = w;
			w = first_kid[v];
				
		}
	//fprintf(fetd_out,"%d %d     slen=%d\n", v, postnum, first_next_kid_stack_pos);
		post[v] = postnum++;    /* Modified by X.Li on 2/14/95 */
		if(first_next_kid_stack_pos>=0){
			v=first_next_kid_stack_v[first_next_kid_stack_pos];
			w=first_next_kid_stack_w[first_next_kid_stack_pos];
			//printf("pop  %d  %d\n", v, w);
		}
		first_next_kid_stack_pos--;
	}while(first_next_kid_stack_pos>=-1 || w!=-1); 
}
...
)
{
	...
	first_next_kid_stack_v = 	mxCallocInt (n+1);
	first_next_kid_stack_w = 	mxCallocInt (n+1);
	...
	postnum = 0;
	//fetd_out=fopen("c:\\temp\\etdfs2_new.dat", "wb");
	etdfs_new(n);
	//etdfs_orig(n);
	//fclose(fetd_out);
	SUPERLU_FREE (first_kid);
	SUPERLU_FREE (next_kid);
	SUPERLU_FREE (first_next_kid_stack_v);
	SUPERLU_FREE (first_next_kid_stack_w);
	return post;
}
