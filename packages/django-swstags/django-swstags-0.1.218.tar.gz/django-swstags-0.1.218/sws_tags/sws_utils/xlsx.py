...
def ExportFile(param_export_file):
	...
	def writeBillInStream():
		...
		try:
			...
			sheet.merge_range('D2:E4', 'FACTURA: '+param_export_file['data_bill']['invoice_number'], red_format)
		except:
			pass
			# print traceback.format_exc()
		...
		sheet.write(row, col_max-2, total['sum_total_minutes'],grey_bg)	
		sheet.write(row, col_max-1, 'Total',grey_bg)
		sheet.write(row, col_max, total['sum_total_income'],money_grey_bg)	

		row = row+1
		sheet.write(row, col_max-1,'IVA '+ str(param_export_file['data_bill']['VAT']) +'%',grey_bg)
		iva = float(total['sum_total_income']) * float(param_export_file['data_bill']['VAT'])/100

		sheet.write(row, col_max,iva,money_grey_bg)

		row = row+1
		sheet.write(row, col_max-1,'Total + IVA',money_grey_bg)
		total_iva = float(total['sum_total_income']) * (1 + float(param_export_file['data_bill']['VAT'])/100)
	...
	if param_export_file['format'] == 'excel':
		...
		return render_to_pdf('tags/pdf_format.html', dictionary=None, context_instance=param_export_file, header_template=None, footer_template=None)
