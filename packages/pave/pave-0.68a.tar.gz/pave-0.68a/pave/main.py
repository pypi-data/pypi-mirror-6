# -*- coding: utf-8 -*-
'''
    %%prog v%s - (C) 2012-2014 Mike Miller.
    License: GNU GPLv3+
...
if True: # foldable
    ...
    opts            = None
    lcmds           = None
    tmpdir, logdir  = None, None  # set in setup()
...
def setup():
    ...
    if opts.skel: # output skeleton file
        ...
        try:    # this translates newlines, unlike shutil
            with open(join(dirname(__file__), 'skeleton.yml')) as infile:
    ...
    # find cache and config folders
    try:
        import xdg.BaseDirectory  # modern linux
        tempdir = join(xdg.BaseDirectory.xdg_cache_home, __progname)
        confdir = join(xdg.BaseDirectory.xdg_config_home, __progname)
    except ImportError:
        xdg = None
        tempdir = join(expanduser('~'), '.cache', __progname)
        confdir = join(expanduser('~'), '.config', __progname)
    logdir = join(tempdir, 'logs')
    libdir = join(confdir, 'lib')
    sys.path.append(libdir)

    # create folders
    folders = (tempdir, 0750), (logdir, 0750), (confdir, 0750), (libdir, 0750)
    for folder, perms in folders:
        try:
            os.makedirs(folder)  # perms incorrect w/makedirs
            os.chmod(folder, perms)
            log.debug('created "%s" (%o)', folder, perms)
        except OSError, e:
