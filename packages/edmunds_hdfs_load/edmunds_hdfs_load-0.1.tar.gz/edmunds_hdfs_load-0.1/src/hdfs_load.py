...
import time
import csv
import src.create_hive_table_sql as cht
import ConfigParser
...
def move_folder(sftp, dest_folder, folder, recursive=True, remove_headers=False):
    ...
    for f in os.listdir(folder):
        ...
        if(os.path.isdir(f_path) and recursive):
            move_folder(sftp,dest_path,f_path,recursive)
        else:
            print "Copying {0} to {1}".format(f_path, dest_path)
            if remove_headers:
                f_path, table_name, headers = preprocess_csv(f_path)
...
def preprocess_csv(csv_file, delimiter = ","):
    ...
    with open(csv_file,'rb') as inp:
        with open(n_file,'wb') as out:
            csv_inp = csv.reader(inp,delimiter=delimiter)
...
def create_hive_ddl(sql_folder, table_name, fields, base_path, overwrite, delimiter):
    config_dict = cht.parse_program_input(table_name, fields, ["STRING" for f in fields], base_path+"/"+table_name, ["year","month","day"])
    config_dict[cht.FDELIM] = delimiter
    output_f_location = os.path.join(sql_folder,table_name+".sql")
    cht.create_creation_sql(config_dict, output_f_location, overwrite)
    
def create_hive_tables(sftp,ssh,sql_folder, dest_sql_folder):
    move_folder(sftp,dest_sql_folder,sql_folder,False,False)
...
def main():
    ...
    sql_folder = config.get(LOCAL_PATHS,"local_sql_dir")
    sql_dest_folder = config.get(REMOTE_PATHS,"sql_dest_dir")
    ...
    info_dict = move_folder(sftp,dest,src,True,True)
    try:
        os.mkdir(sql_folder)
    except:
    ...
    if create_tables:
        for tn in info_dict:
            create_hive_ddl(sql_folder,tn,info_dict[tn],hdfs_dest,overwrite_existing, delimiter)
