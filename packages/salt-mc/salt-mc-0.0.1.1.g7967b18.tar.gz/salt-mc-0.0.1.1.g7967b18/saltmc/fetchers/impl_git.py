...
from saltmc.fetchers import VCSFetcher
...
class GitFetcher(VCSFetcher):
    ...
    dirname = '.git'
    repo_name = 'clone'
    schemes = ('git', 'git+http', 'git+https', 'git+ssh', 'git+git',
               'git+file')
    ...
    def __init__(self, url=None, *args, **kwargs):
        ...
        if url:
            ...
            if scheme.endswith('file'):
                ...
                newpath = initial_slashes + url2pathname(path).\
                    replace('\\', '/').lstrip('/')
        ...
        super(GitFetcher, self).__init__(url, *args, **kwargs)

    def export(self, location):
        """Export the Git repository at the url to the destination location"""
        temp_dir = tempfile.mkdtemp('-export', 'saltmc-')
    ...
    def check_rev_options(self, rev, dest, rev_options):
        ...
        if origin_rev in revisions:
            ...
            msg = "Could not find a tag or branch '%s', assuming commit."
            LOG.warn(msg % rev)
            return rev_options
    ...
    def update(self, dest, rev_options):
        ...
        utils.call_subprocess([self.cmd, 'fetch', '-q'], cwd=dest)
    ...
    def fetch(self, dest):
        ...
        if self.check_destination(dest, url, rev_options, rev_display):
            LOG.debug('Cloning %s%s to %s' %
                      (url, rev_display, utils.display_path(dest)))
            utils.call_subprocess([self.cmd, 'clone', '-q', url, dest])
    ...
    def get_url(self, location):
        url = utils.call_subprocess(
            [self.cmd, 'config', 'remote.origin.url'],
            show_stdout=False, cwd=location)
        return url.strip()

    def get_revision(self, location):
        current_rev = utils.call_subprocess(
            [self.cmd, 'rev-parse', 'HEAD'], show_stdout=False, cwd=location)
        return current_rev.strip()

    def get_refs(self, location):
        """Return map of named refs (branches or tags) to commit hashes."""
        output = utils.call_subprocess([self.cmd, 'show-ref'],
                                 show_stdout=False, cwd=location)
        rv = {}
    ...
    def get_src_requirement(self, dist, location, find_tags):
        ...
        if current_rev in names_by_commit:
            # It's a tag
            full_egg_name = '%s-%s' % \
                (egg_project_name, names_by_commit[current_rev])
        else:
    ...
    def get_url_rev(self):
        ...
        if not '://' in self.url:
            ...
            url, rev = super(GitFetcher, self).get_url_rev()
            url = url.replace('ssh://', '')
        else:
            url, rev = super(GitFetcher, self).get_url_rev()
    ...
    def update_submodules(self, location):
        if not os.path.exists(os.path.join(location, '.gitmodules')):
            return
        utils.call_subprocess([self.cmd, 'submodule', 'update', '--init',
                        '--recursive', '-q'],
                        cwd=location)
