...
class CachedOcAdminClient(OcClient):
    ...
    def _getValueUseCache(self, cache_key, func, account, skip_cache=False, **params):
    ...
    def getPrincipalInfo(self, vendor, account,  pid,  skip_cache=False):
        ...
        value = self._getValueUseCache(key, self.admin.principal_info, account, skip_cache=skip_cache, user_id=pid) 
        if not value:
    ...
    def listPrincipalInfo(self, vendor, account, pids, skip_cache=False):
        ...
        if len(users) == 1:
            ...
            infos = self.admin.list_principal_info(account=account, users=users)
            values.extend(infos)
            for info in infos:
    ...
    def listUserGroups(self, vendor, account, user_id):
        ...
        remote_groups= self._getValueUseCache(key, self.admin.list_user_groups, account=account, key=user_id) or {}
    ...
    def listGroupMembers(self, vendor, account, group_id):
        ...
        return self._getValueUseCache(key, self.admin.list_group_members, account=account, key=group_id) or []

    def listOrgStructure(self, vendor, account):
        key = "orgstr:%s.%s.%s" % (CACHE_PREFIX, vendor, account)

        org_structure = self._getValueUseCache(key, self.admin.list_org_structure, account=account) or {}
        return org_structure

    def listCompanies(self, vendor, account):
        key = "companies:%s.%s.%s" % (CACHE_PREFIX, vendor, account)
        return self._getValueUseCache(key, self.admin.list_companies, account=account) or []
    ...
    def listInstances(self, vendor,  account, skip_cache=False):
        ...
        value = self._getValueUseCache(key, self.admin.list_instances, account, skip_cache=skip_cache)
        return value
    ...
    def getOUDetail(self, vendor, account, ou_id, include_disabled=False, skip_cache=False):
        ...
        value = self._getValueUseCache(key, self.admin.ou_detail, account=account, ou_id=ou_id, include_disabled=include_disabled, skip_cache=skip_cache) or {}
        return value
