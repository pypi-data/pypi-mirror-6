...
def spectrogram(cnt, timeaxis=-2):
    ...
    specgram = stft(cnt.data, width, timeaxis=timeaxis)
    freqs = sp.fftpack.fftfreq(width, 1/cnt.fs)[1:width/2]
    print freqs
    return specgram, freqs


def stft(x, width, timeaxis=-2):
    ...
    fourier = np.array([sp.fftpack.fft(x[i:i+width] * window)[1:width/2] for i in range(0, len(x)-width)])#, width//2)])
    amps = 2 * fourier / width
    amps = np.abs(amps)
    return amps
