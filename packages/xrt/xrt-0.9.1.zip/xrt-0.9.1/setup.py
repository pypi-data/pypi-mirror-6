...
* Synchrotron sources. Bending magnet, wiggler,
  undulator and elliptic undulator are calculated internally within xrt (new in
  version 0.9). There is also a legacy approach to sampling synchrotron sources
  using the codes `ws` and `urgent` which are parts of XOP package. Please look
  the section `Comparison of synchrotron sources` for the comparison between
  the two implementations. If the photon source is one of the synchrotron
  sources, the total flux in the beam is reported not just in number of rays
  but in physical units of ph/s. The total power or absorbed power can be opted
  instead of flux and is reported in W. The power density can be visualized by
  isolines.
...
* *Energy dispersive elements*. Implemented are crystals in dynamical
  ...
  reflection or in transmission. The two-field polarization phenomenaare fully
  preserved, also within the Darwin diffraction plateau, thus enabling the ray 
  tracing of crystal-based phase retarders. As compared to XOP/Shadow, xrt works
  correctly for asymmetric crystals in transmission regime.

* *Materials*. The material properties are incorporated using three
  different tabulations of the scattering factors, with a wide energy range
  (>100 keV) or finer energy mesh. Refraction index and absorption coefficient
  are calculated from the scattering factors. Two-surface bodies, like plates or
...
Dependencies
------------
The histogramming is done by means of numpy; matplotlib is used for plotting."""
...
setup(name='xrt',
      ...
      package_data={'xrt.backends.raycing': ['data/*.*',
                                             'data/XOP-Reflectivities/*.*',
                                             'data/XOP-RockingCurves/*.*']},
