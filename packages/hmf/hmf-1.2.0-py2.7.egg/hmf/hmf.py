...
#        dndm:          The comoving number density of halos in mass interval M [h**4/(M_sun * Mpc**3)]
...
#                      transfer_fit=transfer_fit, cut_fit=cut_fit, z2=z2, nz=nz, ** kwargs)
    ...
    def update(self, **kwargs):
        ...
        cp = {k:v for k, v in kwargs.iteritems() if k in self._cpdict and v != self._cpdict[k]}
        if cp:
            self._cpdict.update(cp)
...
#             dndm = fsigma * self.cosmo_params['mean_dens'] * np.abs(dlnsdlnm) / np.exp(m_lower) ** 2
    ...
    def how_big(self):
        """ 
        Size of simulation volume in which to expect one halo of mass M, ``len=len(M)`` [units :math:`Mpc^{3}h^{-3}`]
        """
