...
VERSION = "0.1"
instances = [ 'I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII','XIII','XiV','XV','XVI','XVII','XVIII','XIX','XX']
...
class LyMaker(object):
    ...
    def __init__(self,name,quarters=4,beats=[],onbeat =False):
        ...
        self.chord = []
        self.chord.append(self.notes[0])
        if len(self.notes) > 2:
            self.chord.append(self.notes[2])
        if len(self.notes) > 4:
            self.chord.append(self.notes[4])
        self.chord2 = []
        if len(self.notes) > 2:
            self.chord.append(self.notes[2])
        if len(self.notes) > 4:
    ...
    def asXml(self):
        ...
        text += "<key>%s</key>\n" % tmp
        text += "<tempo>%d</tempo>\n" % self.tempo
        text += "<structure>AAAA</structure>\n"
        text += "<downbeats>1,3</downbeats>\n"
        on = 0
        if self.onbeat == True:
            on = 1
        text += "<onbeat>%d</onbeat>\n" % on
        text += "<part>\n"
        text += "<partname>A</partname>\n"
        text += "<ptempo>%d</ptempo>\n" % self.tempo
        text += "<progressions>"
        text += "c,e,g|c,e,g|a,c,e|a,c,e;c,e,g|d,f,a|d,f,a|f,a,c|f,a,c"
        text += "</progressions>\n"
        text += "<groove>0</groove>\n"
    ...
    def addHarmonies(self,name,chords):
        text = '\n%s = {\n' % name
    ...
    def addScorePart(self,instanceCount,harmonies):
            text = harmonies
    ...
    def makeSong(self,chords):
        result = self.addIntro()
        riff = ""
        riff2 = ""
        if len(chords) > 0:
            riff = self.getRiff(chords[0][0])
        if len (chords) > 1:    
            riff2 = self.getRiff(chords[1][0])
    ...
    def makePart(self,name,chords,instance,bassmode,drummode):
        result = ""    
        self.chord = chords[0]
        if len(chords) > 1:
        ...
        for bar in chords:
                ...
                resultH += "% " + barstr                
                if self.melody:
                    resultH += hm.process(c,d)
                else:
                    resultH += hm.makeChord(c,d)    
                resultH += " | \n"
                if bassmode != 5:
                    resultB += "% " + barstr                
    ...
    def makeScore(self,instances,harmonies):
        result = ""
        result += self.addScorePart(instances,harmonies)
        return result
#
#
# HARMONYMAKER
...
class HarmonyMaker(object):
        ...
        def makeChord(self,chord,chord2):
            result = ""
            cnt = 0
            while cnt < self.quarters:
                pos = cnt * 16
                if pos in self.beats:
                    if pos > 0:
                        if len(chord2):
                            chord = chord2
        ...
        def process(self,chord,chord2):
            ...
            while mysum < (self.quarters * 16):
                if mysum == 0:  # beat 1
                    ...
                    mysum += dur
                    continue
                elif beat+1 < len(self.beats)  and mysum >= (self.beats[beat+1] - 8): # one eighth before next downbeat
                    ...
                    play = random.randint(0,1)
                elif mysum == self.beats[beat+1]: # next downbeat reached
...
def main():
    ...
    if automatic:
            ...
            mm.setTempo(r.getTempo())
            mm.setKey(r.getKey())
            mm.setDownbeats(r.getDownbeats())
            mm.setQuarters(r.getQuarters())
            mm.setOnbeat(r.isOnbeat())
            harmonies = ""
            callstring =  '\nharmonies = {\n    '
            i = 0
            while i < count:
                ...
                if i == 0:
                    text = mm.makeSong(chords)        
                i += 1
                harmonies += mm.addHarmonies(part.name,chords)
            ...
            for s in structure:
                ...
                i += 1
                text += mm.makePart(part.name,chords,instance,bassmode,drummode)
                callstring += mm.addStructure(part.name)
            
            callstring += '}\n'
            harmonies += callstring
            text += mm.makeScore(i,harmonies)
            outf = None
            try:
