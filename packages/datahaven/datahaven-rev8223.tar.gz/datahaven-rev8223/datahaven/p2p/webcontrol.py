...
class MainPage(Page):
    ...
    def _body(self, request):
        ...
        if self.newPathAdded is not None:
            if self.newPathAdded[2] == 'diradd':
                src += html_comment('new folder was added:')
            elif self.newPathAdded[2] == 'diraddrecursive':
                src += html_comment('new folder tree was added:')
            else:
                src += html_comment('new file was added:')
            src += html_comment('  %s %s' % (self.newPathAdded[0].ljust(27), self.newPathAdded[1].ljust(70)))
            self.newPathAdded = None
            
        if self.newBackupStarted is not None:
            src += html_comment('  %s : backup started' % self.newBackupStarted)
            self.newBackupStarted = None
    ...
    def _action(self, request):
        ...
        if action == 'diradd':
            ...
            if opendir:
                ...
                self.newPathAdded = ( newPathID, opendir, action )
        ...
        #---fileselected---
            ...
            if openfile:
                ...
                self.listExpandedDirs = None
                self.listExpandedVersions = None
                self.newPathAdded = ( newPathID, openfile, action )
        ...
        #---list---
            ...
            for pathID, localPath, item in backup_fs.IterateIDs():
                ...
                for version, vinfo in item.get_versions().items():
                    if vinfo[1] >= 0:
                        szver = diskspace.MakeStringFromBytes(vinfo[1]) + ' / ' + diskspace.MakeStringFromBytes(vinfo[1]/contacts.numSuppliers()) 
                    else:
                        szver = '?'
        ...
        #---idlist---
            ...
            src = ''
            for backupID, versionInfo, localPath in backup_fs.ListAllBackupIDsFull():
                szver = diskspace.MakeStringFromBytes(versionInfo[1]) if versionInfo[1] >= 0 else '?'
                src += html_comment('  %s %s %s' % (backupID.ljust(28), szver.ljust(15), localPath))
            return html(request, body=str(src),)
        else:
            return None
