...
class MatchWindowTask(object):
    ...
    def _prepare_match_data(self, region, tag, force_full_page_screenshot, ignore_mismatch):
        ...
        # We need the initial screenshot anyway
        ...
        if force_full_page_screenshot:
            # If the screenshot is smaller then the page size, we only took a partial screenshot
            if (self._current_screenshot.width < self._driver.extract_full_page_width()) and \
                    (self._current_screenshot.height < self._driver.extract_full_page_height()):
                # We need to take a complete screenshot
