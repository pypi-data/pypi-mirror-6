...
class Basket(models.Model):
    ...
    @property
    def signature(self):
        """return an md5 of the basket and contents"""
        hashes = ''.join([b.signature for b in self.basketitem_set.all()])
        sig = hashlib.md5()
        sig.update(hashes)
    ...
    def total_price(self):
        ...
        if cached_total_price is not None:
    ...
    def total_items(self):
        ...
        # if there is cache return that
        if cached_total_items is not None:
    ...
    def add(self, item, **kwargs):
        ...
        """
        # if item is None then content object has been removed loop over
        # basket items & remove any with None as a contentobject
        if item is None:
            for basketitem in self.basketitem_set.all():
                if basketitem.content_object is None:
        ...
        try:
            ...
            raise Exception('Quantity is not an integer' % quantity)
...
class BasketItem(models.Model):
    ...
    def signature(self):
        sig_string = '{}{}{}{}'.format(
            ...
            self.object_id,
            self.quantity,
            self.get_options_hash(self.options.all())
        )
        sig = hashlib.md5()
        sig.update(sig_string)
        return sig.hexdigest()
