...
def createTWS(*args): 
    ...
    creteTWS creates a table workspace and allows different ways of use,
    you can choose the one that fits the best your needs.
    ...
    Method 2:
        ...
        workspace (always using forward slashes "/" to specify the
        location).if you only specify the location and name of the table
        workspace the password will be sent to blank ("").
    ...
    Method 3:
        You can specify only the name and password. Using this method
        the table workspace location will be on "./", and just like in
        the method 2 if you only specify the name the password will be
        set blank ("").
...
def useTWS(route, password,user=None,logs='disable'):
    '''
    Return an object with the table workspace connection if the TWS
    exist and the password matches with the one given otherwise raises
    an exception.
...
class connection():    
    ...
    def createTable(self, name, columns):
        '''
        createTable allows to create tables into a table workspace, by
        receiving the name of the table and a dictionary with the fields
        names and data types. The possible data types are: 'str', 'char',
        'bool', 'float', 'int', 'date' and 'file'. To specify the key field
        this must be preceded by a star "*".
    ...
    def insertInto(self, table, values):
        '''
        insertInto allows to add registries to an specified table by
        receiving the table name and a dictionary with the fields and their
        corresponding values. It's necessary that the key field is present
        to add a registry, and that all values correspond to the data type
        of the field. Date type fields must be on ISO 8601 format.
    ...
    def __getRegs(self,table):
        '''
        __getRegs is a hidden method that returns all registries from a
        given table.
    ...
    def fetchAll(self, *args):
        '''
        fetchAll returns a list of lists with all registries from a
        table according to a set of filters, default filter is None
        which return all registries from the given table.
        ...
        eg3:
           ...
           Registries order by an specific field ('d' is descending and 'a'
           is ascending)
        ...
        eg6:
            ...
            this get all registries from table1 which name is 'name1'
            in a descending order by age.
    ...
    def fetchAsDict(self,*args):
        '''
        fetchAsDict returns a list of dictionaries with all registries from
        a table according to a set of filters, default filter is None
        which return all registries from the given table.

        eg1:
         tws.fetchAsDict('table1')

        eg2:
         tws.fetchAsDict('table1', {'id':1})

        eg3:
         tws.fetchAsDict('table1', {'name':'name1', 'lastname':'lastName1'})

          Registries order by an specific field ('d' is descending and 'a'
          is ascending)

        eg4: 
         tws.fetchAsDict('table1','name')

        eg5:
         tws.fetchAsDict('table1','name','d')

        eg6:
         tws.fetchAsDict('table1', {'name':'name1'}, 'age','d')

         this get all registries from table1 which name is 'name1'
         in a descending order by age.
        '''

        regs=self.fetchAll(*args)
        fields=[x[0] if x[0][0]!="*" else x[0][1:] for x in self.getSchema(args[0])]
        if fields==[]: return []
        listOfDicts=[]
        for reg in regs:
            tmp={}
            for field in range(len(fields)):
                tmp[fields[field]]=reg[field]
    ...
    def deleteFrom(self, table, keyvalue):
        '''
        deleteFrom, deletes a registry from a table, given the key value
    ...
    def updateFrom(self, table, keyvalue, values):
        '''
        updateFrom allows to modify data from an existing registry.
        It works by receiving the name of the table in which is located the
        registry, the Key Value of the registry and finally a dictionary
        with the fields and values to modify.

        eg:
        tws.updateFrom('table1', 'id1',
    ...
    def match(self, keyvalue, tables):
        '''
        match return all registries that matches between different tables
        given one key value.
        eg:
    ...
    def __log(self, action):
        '''
        _log register an action on the log file
        '''
