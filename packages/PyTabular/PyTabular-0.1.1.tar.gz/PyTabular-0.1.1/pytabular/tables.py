...
class TabularCell(TabularBase):
    ...
    ** Attributes **
    ...
    def _handle_content(self, content):
        ...
        Issues
        ------
        confuses math environment ($ $) as unprefixed
        special characters when the count of '$' in content
        is odd
        
        certain combinations of restricted characters are permissible,
        not accounting for these scenarios        
        
        will probably need regular expressions
        '''
        ...
        if isinstance(content, str):
            for char in escapeChars2:
                if char in content:
                    msg = '"{}" replaced with {} in cell content'.format(char, '')
                    warnings.warn(msg, UserWarning)
                    content = content.replace(char, '') 
            for char in escapeChars1:
                if char in content:
                    if (char=='$') & (content.count('$')%2 == 0) & (not '$$' in content):
                        continue
                    if (char=='{') & ('{}' in content):
                        continue
                    if (char=='}') & ('{}' in content):
                        continue
                    msg = 'leading "\\" placed before {} in cell content'.format(char)
                    warnings.warn(msg, UserWarning)
                    content = content.replace('\\{}'.format(char), char)
                    content = content.replace(char, '\\{}'.format(char))
