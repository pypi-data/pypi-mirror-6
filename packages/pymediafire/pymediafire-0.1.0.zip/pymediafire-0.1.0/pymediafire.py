...
if __name__ == "__main__":
    ...
    mf = MediaFireSession('schampailler@...com','password','appid','appkey',mainlog)

    folder = mf.load_directory()

    # Look for the back up directory
    backup_directory_found = False
    for entry in folder:
        if isinstance(entry,MediaFireFolder) and entry.name == 'backup-pl':
            backup_directory_found = entry
            break

    # If the backup directory is not found we create it
    if backup_directory_found == False:
        backup_directory_found = mf.create_folder("backup-pl")
    else:
        print("Backup folder is already there")


    # upload(token,backup_directory_found,"precision_test1.sql.bz2.aes")
    mf.upload(backup_directory_found,"mediafire.py")
