...
class Users(object):
    ...
    ::
     ...
     . . .
    """
    def __init__(self, output, shadow=True):
        self._users = {}
        ...
        for record in (l.strip().split(':') for l in output):
            try:
                ...
                time_since_change = timedelta(days=int(record[2]))
                # If we're reading from /etc/passwd, make last_change the epoch
                if not shadow:
                    time_since_change = 0

                last_change = (epoch + time_since_change).isoformat()
...
class SSHProfile(Profile):
    ...
    @property
    def _input(self):
        return "(cat /etc/shadow || sudo -n cat /etc/shadow) 2>/dev/null",
...
class BackupSSHProfile(Profile):
    """
    """
    service = SSHService
    ...
    @property
    def _input(self):
        return "(cat /etc/passwd || sudo -n cat /etc/passwd) 2>/dev/null",

    @staticmethod
    def _parse(output):
        return Users(output, shadow=False)
