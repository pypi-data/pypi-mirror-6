...
class UiWindow(QMainWindow, MainWindow_Dialog):
    def __init__(self, parent=None, options={}):
        ...
        # if self.widths and qt_version == 5:
        #     for tree in [self.dayTree, self.nowTree, self.nextTree,
        #                  self.dayTree, self.folderTree, self.keywordTree,
        #                  self.tagTree]:
...
from your 'emt.cfg': %s.""" % ", ".join(["'%s'" % x for x in missing])))
        ...
        if self.alerts:
            ...
            # self.alertsButton.setToolTip(self.tr(
            #     "Remaining alerts for today: {0}".format(len(self.alerts))))
            self.alertsButton.show()
        else:
            self.alertsButton.hide()
...
def main(etmdir=''):
    ...
    # app.setWindowIcon(QIcon(QPixmap(":/etm.png")))
