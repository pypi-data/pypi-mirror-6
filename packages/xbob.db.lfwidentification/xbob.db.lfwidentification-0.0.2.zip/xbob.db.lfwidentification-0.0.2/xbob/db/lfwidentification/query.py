...
LFW Identification database in the most obvious ways.
"""
...
class Database(xbob.db.verification.utils.SQLiteDatabase):
  ...
  def clients(self, protocol=None, groups=None):
    ...
    protocol
      The protocol to consider ('P0', 'P0set')
  ...
  def objects(self, protocol=None, purposes=None, model_ids=None, groups=None,
    ...
    protocol
      One of the LFW Identification protocols ('P0','P0set').
    ...
    # Sort by (client_id, id) and remove duplicates
    sorted_files = sorted(retval, cmp=lambda x,y: cmp((x.client_id, x.id), (y.client_id, y.id)))
    retval = [f for i,f in enumerate(sorted_files) if not i or sorted_files[i-1].id != f.id]
    return retval 

  def object_sets(self, protocol=None, purposes=None, model_ids=None, groups=None,
      classes=None):
    """Returns a set of Fileset's for the specific query by the user.
    WARNING: Files used as impostor access for several different models are
    only listed one and refer to only a single model
    ...
    if ('dev' in groups or 'eval' in groups):
      if('probe' in purposes):
        if('client' in classes):
          q = self.query(Fileset).join(Client).join((ProtocolPurpose, Fileset.protocolPurposes)).join(Protocol).\
                filter(and_(Protocol.name.in_(protocol), ProtocolPurpose.sgroup.in_(groups), ProtocolPurpose.purpose == 'probe'))
          if model_ids:
            q = q.filter(Client.id.in_(model_ids))
          q = q.order_by(Fileset.client_id)
          retval += list(q)
        ...
        if('impostor' in classes):
          q = self.query(Fileset).join(Client).join((ProtocolPurpose, Fileset.protocolPurposes)).join(Protocol).\
                filter(and_(Protocol.name.in_(protocol), ProtocolPurpose.sgroup.in_(groups), ProtocolPurpose.purpose == 'probe'))
          if len(model_ids) == 1:
            q = q.filter(not_(Fileset.client_id.in_(model_ids)))
          q = q.order_by(Fileset.client_id)
          retval += list(q)
  ...
  def has_fileset(self, protocol_name):
    """Tells if a certain protocol is using fileset's"""

    p = self.query(Protocol).filter(Protocol.name==protocol_name)
    if p.count() == 0: return False
    else: return p.one().fileset
