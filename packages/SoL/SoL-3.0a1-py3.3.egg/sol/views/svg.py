...
def ratingChart(request):
    ...
    chart.title = t(_(u'Rating “$rating”',
...
def playersDistribution(request):
    ...
    orig_pygal_countries = dict(pygal.i18n.COUNTRIES)
    try:
        ...
        new_pygal_countries = {}
        for alpha2 in pygal.i18n.COUNTRIES:
            try:
                c = countries.get(alpha2=alpha2.upper())
            except KeyError:
                new_pygal_countries[alpha2] = pygal.i18n.COUNTRIES[alpha2]
            else:
                new_pygal_countries[alpha2] = ct(c.name)
        pygal.i18n.COUNTRIES.update(new_pygal_countries)

        t = translator(request)

        chart = pygal.Worldmap(show_legend=False, width=width, height=height)
        chart.title = t(_(u'Players distribution around the globe'))
        ...
        chart.add(t(_(u'Females')), women)
        chart.add(t(_(u'Males')), men)

        response = request.response
        response.body = chart.render()
        response.content_type = 'image/svg+xml'
    finally:
        pygal.i18n.COUNTRIES.update(orig_pygal_countries)
