...
class search_agent():
    """
    search_agent() is an interface for the PACER case locator. It will
    query and download both dockets and documents. It is a modified
    requests.sessions object.
    """
    ...
    def refresh_login(self):
        """
        Login to the PACER system.
        Login to the PACER system using the login and password provided at
        the initialization of the search_agent. This will create a Requests
        session that will allow you to query the PACER system. This function
        will raise an error if you cannot login.
        """
    ...
    def query_case_locator(self, payload):
        """
        Simple interface to pass queries for PACER Case Locator. Returns the 
        HTML of the search results page.

        What are the acceptable payloads?
        """
        #Locator
    ...
    'default_form':'b'}):
        ...
        results. Each search result is a dictionary with these attributes 
        (searched_case_no, result_no, case_name, listed_case_no, court_id, nos, date_filed, date_closed, query_link)
        """
    ...
    def request_docket_sheet(self, docket_link, other_options={}):
        """
        Input the link to a docket report sheet, parse the docket report page 
        for parameters and pass it to PACER. Returns the HTML of the docket 
        sheet.
        """
