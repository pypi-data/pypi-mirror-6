...
class Eyes(object):
    ...
    _DEFAULT_VIEWPORT_SCREENSHOT_BROWSERS = [u"chrome", u"safari"]
    ...
    def open(self, driver, app_name, test_name, viewport_size=None,
        ...
        if self.is_open():
            ...
            # FIXME Daniel, do we want to throw an exception? or just continue the test?
    ...
    def _check_window(self, region, tag):
        ...
        force_full_page_screenshot = self.force_full_page_screenshot and \
            (self._driver.driver.desired_capabilities["browserName"] in
             self._DEFAULT_VIEWPORT_SCREENSHOT_BROWSERS)
        ...
        if not self._running_session:
            ...
            self._match_window_task = MatchWindowTask(self, self._agent_connector,
                                                      ...
                                                      self.match_timeout)
        as_expected = self._match_window_task.match_window(region, tag, force_full_page_screenshot,
    ...
    def _reset_driver(self):
        # If we have an existing driver, we try to remove the replaced methods.
        if self._driver is not None:
            # noinspection PyBroadException
            try:
                # noinspection PyProtectedMember
                self._driver._reset_driver()
            except:
                logger.debug("Failed to restore original methods to driver!")
            self._driver = None
