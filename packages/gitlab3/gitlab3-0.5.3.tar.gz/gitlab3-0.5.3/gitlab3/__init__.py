...
def list_fn(api, parent, url, limit=None, page=None,
            per_page=None, addl_keys=[], **data):
    print "list_fn:", url
    print "list_fn:", addl_keys 
    ret = []
    ...
    if page or per_page:
        ...
        objs = parent._get(url, data=data)
        for obj in objs:
...
def _add_list_fn(api, api_definition, parent):
    ...
    def fn(limit=None, page=None, per_page=None, **data):
        return flexible_list_fn(api, parent, api._uq_url,
                                limit, page, per_page, **data)
...
def _add_extra_fn(api, action_def, parent):
...
class GitLab(_GitLabAPI):
    ...
    def __init__(self, gitlab_url, token=None, convert_dates=True,
        ...
        for action_def in _GitLabAPIDefinition.extra_actions:
            _add_extra_fn(GitLab, action_def, self)
