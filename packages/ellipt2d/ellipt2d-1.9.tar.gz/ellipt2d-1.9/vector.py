#!/usr/bin/env python
# $Id: vector.py,v 1.13 2009/07/13 19:52:53 pletzer Exp $
# a python vector class
# A. Pletzer 5 Jan 00/11 April 2002
#
"""
A list based vector representation (real)
"""
...
class vector(list):
	...
	def __rdiv__(self, other):
	    ...
	    """
	    try:
		    return vector(map(operator.div, other, self))
	    except:
		    # other is a const
...
def random(n, lmin=0.0, lmax=1.0):
    ...
    gen = rand.random()
    dl = lmax-lmin
    return vector(map(lambda x: dl*rand.random(),
		       range(n)))
