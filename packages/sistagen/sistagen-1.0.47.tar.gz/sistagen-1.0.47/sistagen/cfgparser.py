...
def create_grammar():
    ...
    dtreg = r'((?P<year_prefix>\d{2})?(?P<year>\d{2})?(?P<month>\d{2})(?P<day>\d{2}))?(?P<hour>\d{2})(?P<min>\d{2})(?P<sec>\d{2})?'
    vtreg = r'(?P<relative>\d+[smhd])'
    ...
    cmds = White(' \t') + \
        (
            (
                ...
                Keyword('colors', caseless=True) + Names
            ) | (
                Keyword('title', caseless=True) + String
            ) | (
                Keyword('size', caseless=True) + OneOrMore(Regex('[0-9]+x[0-9]+'))
            ) | (
                Keyword('date', caseless=True) + OneOrMore(
                        Group(
                            Optional(Group((Regex(dtreg)|Regex(vtreg)) + (Word('-'))))
                             + Group(Regex(dtreg)|Regex(vtreg))
                        )
                    )
            ) | (
