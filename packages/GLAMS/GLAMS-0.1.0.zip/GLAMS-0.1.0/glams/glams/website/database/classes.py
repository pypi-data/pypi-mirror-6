...
class Mouse:
    ...
    def transfer(self,newcage):
        ...
        if cage_id==[]:
            ...
            return "{0} placed in {1} on {2}".format(self.d['mousename'],newcage,date2str(today))
        else:
            oldcage=cage_id[0][0]
            if oldcage==newcage:
                return "{} still in cage {}".format(self.d['mousename'],newcage)
            start_date=cage_id[0][1]
            if date2str(start_date)==date2str(today):
            ...
            return "{0} transfered from {1} to {2} on {3}".format(self.d['mousename'],oldcage,newcage,date2str(today))
        ...
        for cage in d.keys():
            if cage.startswith('oldcagename'):
                ...
                if d['oldcagename'+n] is not None:
                    if d['startDate'+n] is not None and d['endDate'+n] is not None:
            ...
            if reserve.startswith('reserve_lab_member'):
                ...
                if d['reserve_lab_member'+n] is not None:
                    ...
                    if d['reserve_date'+n] is None or d['reserve_description'+n] is None or d['reserve_status'+n] is None:
                        ...
                        queries.append(["INSERT INTO experiments SET mouse_id=(SELECT id FROM mice WHERE name=%s), lab_member_id=(SELECT id FROM lab_members WHERE name=%s), date=%s, description=%s, status=%s, notes=%s, filenames=%s",(d['mousename'],d['reserve_lab_member'+n],d['reserve_date'+n],d['reserve_description'+n], d['reserve_status'+n], d['reserve_notes'+n], d['filenames'+n])])                        
