...
class DataPlot(object):
    ...
    def plotAllA(self, save=False, figsize=[18, 12], fignum=None):
        """
        Plot the log of the A values of each data set 
        ...
        plt.clf()
        max_y_ticks = 6
        ticks = range(len(self.atom_data)+1)[1:] 
        
        A = np.zeros([len(self.atom_data), self.atom_n_max, self.atom_n_max])
        color=[]
        ...
        for j in range(2, self.atom_n_max+1):
            for i in range(1, j):
                x = ticks
                y = lgA[:, j-1, i-1]
                ax = plt.subplot(self.atom_n_max - 1, self.atom_n_max - 1, (self.atom_n_max-1)*(j-2) + i,
                                 axisbg='#FFFFE0')
                ax.set_xticks(x)
                plt.xlim((min(x)-0.5, max(x)+0.5))
                if ((j == self.atom_n_max) and (i == 1)):
                ...
                plt.draw()
                try:
                    plt.scatter(x, y, color=color, label='_nolegend_')
                    lbl = "(" + str(j) + r"$\to$" + str(i) + ")"
                    ax.text(0.95, 0.95, lbl, fontsize=10, color="#660066", transform=ax.transAxes, ha="right", va="top")                                    
                except:
                    pn.log_.warn('Problem with plotting a subplot', calling=self.calling)
        ...
        if save:
            plt.figure(figsize=[18, 12])
            plt.savefig(self.atom + "_all_As.pdf")
            ...
            for i in range(1, self.coll_n_max):
                for j in range(i + 1, self.coll_n_max + 1):                
                        ...
                        ax = plt.subplot(self.coll_n_max - 1, self.coll_n_max - 1, (self.coll_n_max - 1) * (j - 2) + i,
                                         axisbg='#FFFFE0')
                        try:
                            ax.plot((np.log10(tem_points)), data['atom'].getOmega(tem_points,j, i), color=data['color'],
