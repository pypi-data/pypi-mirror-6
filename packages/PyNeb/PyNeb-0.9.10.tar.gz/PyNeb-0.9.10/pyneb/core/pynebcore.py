    ...
    def printSources(self):
        ...
        for com in self.comments:
            if 'SOURCE' in com:
                ...
                sources[number] = self.comments[com]
            if 'NOTE' in com:
    ...
    def getOmega(self, tem, lev_i= -1, lev_j= -1):
        ...
        if (lev_i == -1) and (lev_j == -1):
            ...
            if self.noExtrapol or pn.config.get_noExtrapol():
                ...
                rigthExtrapol = OmegaArray[-1]
            Omega = np.interp(tem_in_file_units, self.getTemArray(), OmegaArray,
                              left=leftExtrapol, right=rigthExtrapol)
...
class Atom(object):
    ...
    def __init__(self, elem=None, spec=None, atom=None, OmegaInterp='Cheb', noExtrapol = False):
        ...
        if self.atomFileType == 'fits':
            self.AtomData = AtomDataFits(elem=self.elem, spec=self.spec, atom=self.atom)
        elif self.atomFileType == 'dat':
            self.AtomData = AtomDataAscii(elem=self.elem, spec=self.spec, atom=self.atom)
        ...
        if self.collFileType == 'fits':
            self.CollData = CollDataFits(elem=self.elem, spec=self.spec, atom=self.atom, 
                                         OmegaInterp=OmegaInterp, noExtrapol = noExtrapol)
        elif self.collFileType == 'dat':
            self.CollData = CollDataAscii(elem=self.elem, spec=self.spec, atom=self.atom, 
    ...
    def getOmega(self, tem, lev_i= -1, lev_j= -1, wave= -1):
        ...
        if 'O_UNIT' in self.CollData.comments:
            if self.CollData.comments['O_UNIT'] == 'RATE COEFF':
                to_return /= pn.CST.KCOLLRATE / tem ** 0.5 / self.getStatWeight(lev_i)
            if self.CollData.comments['O_UNIT'] == 'COOLING':
