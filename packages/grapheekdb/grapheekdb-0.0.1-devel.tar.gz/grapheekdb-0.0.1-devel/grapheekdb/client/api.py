...
class ProxyEntityIterator(object):
    ...
    def _raw(self, method, *args, **kwargs):
        command = [method, args, kwargs]
        response = self._graph._request(self._commands + [command])
        return unmarshall(self._graph, response)

    def _jump(self, traversal, *args, **kwargs):
        return self._do(traversal, *args, **kwargs)
    ...
    def call(self, fname, *aliases, **params):
        return self._raw('call', fname, *aliases, **params)

    def request(self, fname, *aliases, **params):
        return self._raw('request', fname, *aliases, **params)


class ProxyEntity(object):

    def __init__(self, graph, entity_id, **data):
        check_valid_data(data)
        self._entity_id = entity_id
        self._graph = graph
        self._commands = [[self._kind, [entity_id], {}]]
        self._data = data
    ...
    def map(self):
        return self._data or self._raw('map')
...
class ProxyNode(ProxyEntity):
    ...
    def __init__(self, graph, node_id, **data):
        self._kind = KIND_VERTEX
        super(ProxyNode, self).__init__(graph, node_id, **data)
...
class ProxyEdge(ProxyEntity):
    ...
    def __init__(self, graph, edge_id, **data):
        self._kind = KIND_EDGE
        super(ProxyEdge, self).__init__(graph, edge_id, **data)
