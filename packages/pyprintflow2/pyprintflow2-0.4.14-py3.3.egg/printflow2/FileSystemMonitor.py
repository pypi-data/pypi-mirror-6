...
class FileSystemMonitor(pyinotify.ProcessEvent):
    ...
    def process_zip_file(self, workgroup, project, specification, file_path, file_name, file_pathname):
        client = None
        guid = str(uuid.uuid4())
        uploaded_by = file_path.split('/')[-4]
        ext = file_name.split('.')[-1]
        if ext != 'zip':
            self.logger.error('file should have extension "zip" - file name given is: '+file_name)
            proj = project['noosh_project']
            spec = specification['noosh_specification']
            db_file, db_file_path = self.register_failed_zip_file(client, file_path, file_name, file_pathname, project, specification, guid, uploaded_by)
        ...
        db_file, db_file_path = self.upload_zip_file_to_cumulus(client, file_path, file_name, file_pathname, project, specification, '2', guid, uploaded_by)
        proj = project['noosh_project']
        spec = specification['noosh_specification']
    ...
    def upload_zip_file_to_cumulus(self, client, file_path, file_name, file_pathname, project, spec, guid, uploaded_by):
    ...
    def post_file_to_noosh(self, client, workgroup_id, project_id, spec_id, file_object, uploaded_by):
        ...
        if 'noosh_nge_enabled' in client and client['noosh_nge_enabled'] is True:
            ...
            if config_upload_by is not None:
                ...
                noosh_post_data['uploaded_by'] = data['uploaded_by']
