...
class Client(object):
    ...
    def __init__(self, application_id, login, auth_token, sandbox=False, password=None, debug=False, timeout=3600, logger=None):
    ...
    def call_method(self, method, param=None):
        ...
        for attempt in range(5):
            try:
                ...
                delay = 5 * (attempt + 1)
                logging.exception("Error, will retry in %u seconds: %s" % (delay, e))
                time.sleep(delay)
    ...
    def _fetch(self, url, args):
        ...
        ts = time.time()
        if self.logger:
            self.logger.debug("Fetching %s, args: %s" % (url, args))
        data = urllib2.urlopen(url, args, self.timeout).read()
        if self.logger:
            self.logger.debug("Fetching took %f seconds." % (time.time() - ts))
        return data
    ...
    def Ping(self, login=None, token=None):
        ...
        try:
            ...
            logging.error("Could not ping the API: %s" % e)
            return False
        finally:
            self.login = old_login
            self.auth_token = old_token

    def GetBanners(self, campaign_ids=None, banner_ids=None, archive=None):
        _filter = {}
        if archive == True:
    ...
    def StopCampaign(self, CampaignID):
        logging.info("Stopping campaign %s" % CampaignID)
        self.call_method("StopCampaign", {"CampaignID": CampaignID})

    def ResumeCampaign(self, CampaignID):
        logging.info("Resuming campaign %s" % CampaignID)
