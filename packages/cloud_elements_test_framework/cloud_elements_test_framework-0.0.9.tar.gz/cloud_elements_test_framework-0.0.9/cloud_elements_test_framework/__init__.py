...
class ElementsConnectorTest(cloud_elements.ElementsConnector):
    ...
    def invoke(self, http_method, hub_name, api, version='1', params=None, payload=None, files=None, headers=None):
        if headers is None:
            headers = {'elementToken': self.element_token}
        return cloud_elements.ElementsConnector.invoke(self, httpMethod=http_method, providerName=hub_name,
                                                       headers=headers, apiMethodName=api,
    ...
    def make_get_request(self, api, params):
        return self.invoke(http_method='get', hub_name=self.provider_name, api=api, params=params)

    def make_put_request(self, api, params, payload, files):
        return self.invoke(http_method='put', hub_name=self.provider_name, api=api, params=params,
                           payload=payload, files=files)

    def make_delete_request(self, api, params):
        return self.invoke(http_method='delete', hub_name=self.provider_name, api=api, params=params)
