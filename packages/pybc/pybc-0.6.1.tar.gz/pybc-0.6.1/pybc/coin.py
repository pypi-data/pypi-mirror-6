...
import struct, hashlib, traceback, time, threading, logging, itertools

try:
    import pyelliptic
except:
    # pyelliptic didn't load. Either it's not installed or it can't find OpenSSL
...
class CoinState(State):
    ...
    def apply_transaction(self, transaction):
        ...
        for i, output in enumerate(transaction.outputs):
            ...
            self.add_unused_output((transaction_hash, i, amount, destination))
    ...
    def remove_transaction(self, transaction):
        ...
        for i, output in enumerate(transaction.outputs):
            ...
            self.remove_unused_output((transaction_hash, i, amount, 
                destination))
    ...
    def step_forwards(self, block):
        ...
        for transaction_bytes in transactions.unpack_transactions(
            block.payload):
            ...
            logging.debug("Applying transaction with {} inputs and {} "
                "outputs".format(len(transaction.inputs),
        ...
        if self.get_hash() != block.state_hash:
            ...
            raise Exception("Stepping forward to state {} instead produced "
                "state {}.".format(pybc.util.bytes2string(block.state_hash), 
                pybc.util.bytes2string(self.get_hash())))
    ...
    def step_backwards(self, block):
        ...
        for transaction_bytes in transactions.unpack_transactions(
            ...
            logging.debug("Rewinding transaction with {} inputs and {} "
                "outputs".format(len(transaction.inputs), 
                len(transaction.outputs)))
...
class CoinBlockchain(TransactionalBlockchain):
    ...
    def __init__(self, block_store, minification_time=None):
        """
        ...
        super(CoinBlockchain, self).__init__(PowAlgorithm(), block_store, 
            state=CoinState(filename=block_store, table="state"),
        ...
        for i, (amount, destination) in enumerate(transaction.outputs):
            ...
            if state is not None:
                if state.has_unused_output(unused_output):
                    ...
                    logging.warning("Transaction trying to create a duplicate "
                        "unused output")
                    return False
            else:
                ...
                logging.debug("Not checking for duplicate output, since we "
                    "have no State.")
        ...
        for source in transaction.inputs:
            if state is not None:
                ...
                if not state.has_unused_output(source):
                    ...
                    # already spent.
                    logging.warning("Transaction trying to use spent or "
                        "nonexistent input")
                    return False
            else:
                logging.debug("Not checking for re-used input, since we have "
                    "no State.")
    ...
    def make_block(self, destination, min_fee = 1):
        ...
        with self.lock:
            ...
            # Don't forget to use the reward transaction in the new State.
            next_state.apply_transaction(reward_transaction)
            ...
            block = super(CoinBlockchain, self).make_block(next_state,
                transactions.pack_transactions(
...
class Wallet(object):
    ...
    def __init__(self, blockchain, filename):
        ...
        self.spendable = CoinState(filename=filename, table="spendable")
        ...
        if (not self.wallet_metadata.has_key("blockchain_state") or
        ...
        with self.lock:
            if event == "forward":
                ...
                for transaction_bytes in transactions.unpack_transactions(
                    ...
                    for spent_output in transaction.inputs:
                        if self.spendable.has_unused_output(spent_output):
                            ...
                            logging.debug("Our available output was spent")
                    
                    for i, output in enumerate(transaction.outputs):
                        # Unpack each output tuple
                        amount, destination = output
                        
                        if self.keystore.has_key(destination):
                            # This output is spendable by us           
                            self.spendable.add_unused_output((transaction_hash,
                                i, amount, destination))
                            logging.debug("New available output!")
                ...
                for transaction_bytes in transactions.unpack_transactions(
                    ...
                    for spent_output in transaction.inputs:
                        if self.keystore.has_key(spent_output[3]):
                            # This output we spent got unspent
                            self.spendable.add_unused_output(spent_output)
                        logging.debug("Our spent output is available again")
                    ...
                    for i, output in enumerate(transaction.outputs):
                        ...
                        if self.spendable.has_unused_output(spent_output):
                            ...
                            logging.debug("Lost available output!")
                ...
                for unused_output in argument.get_unused_outputs():
                    if self.keystore.has_key(unused_output[3]):
                        ...
                        logging.debug("\t{} to {}".format(unused_output[2], 
                            pybc.util.bytes2string(unused_output[3])))
                        found += 1
...
if __name__ == "__main__":
    ...
    def generate_block(blockchain, destination, min_fee = 1):
        ...
        for transaction in transactions.unpack_transactions(block.payload):
